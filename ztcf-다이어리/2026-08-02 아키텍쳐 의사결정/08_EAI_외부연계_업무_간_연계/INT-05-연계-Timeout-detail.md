# INT-05 연계 Timeout 상세 설명서

> 문서 성격: **이해·검토용 상세 초안**  
> 상태: **ARB 승인 전** · 영역: **EAI·외부연계·업무 간 연계** · 우선순위: **P0**

## 1. 이 문서를 먼저 읽는 이유

이 문서는 요약본 [INT-05-연계-Timeout.md](./INT-05-연계-Timeout.md)의 내용을 처음 접하는 사람도 INT-05의 의미, 필요한 이유, 실제로 결정할 항목과 완료 판단 방법을 이해할 수 있도록 풀어서 설명한다.

INT-05의 핵심 주제는 **연계 Timeout**이다. 쉽게 말하면 **다른 WAR·기간계·외부기관을 호출할 때 계약, 장애격리, 중복방지와 데이터 정합성을 보장하는 영역**에서 `연결·응답·전체 Timeout 구분`에 관한 공통 기준을 정하는 일이다. 이 기준을 정하지 않으면 팀마다 서로 다른 판단을 구현하고, 통합·보안·운영 단계에서 뒤늦게 충돌할 가능성이 높다.
...
| TASK ID | INT-05 |
| 의사결정 사항 | 연계 Timeout |
| 이번에 확정할 범위 | 연결·응답·전체 Timeout 구분 |
| 권고 초안 | 연결·응답·전체 Timeout을 구분한다. |
| 이행 방향 | 상위 Timeout이 하위 Timeout보다 충분히 크도록 설정한다. |
| 주관 | EAI |
| 협의 | TA·AA·FW |
| 승인 | TA |
| 우선순위 | P0 |
...
EAI·외부연계·업무 간 연계는 **다른 WAR·기간계·외부기관을 호출할 때 계약, 장애격리, 중복방지와 데이터 정합성을 보장하는 영역**이다.
...
- 의사결정 주제: `연결·응답·전체 Timeout 구분`
- 책임 경계: 업무 Service/Facade → tcf-eai 또는 공통 Client → EAI/외부 시스템 → 응답·대사·보상
- 관련 구성요소: tcf-eai, 서비스 계약, 동기·비동기 Client, 멱등키, 재시도, Circuit Breaker, 대사·보상
- 설계·개발·시험·배포·운영 중 이 결정의 영향을 받는 산출물
- 기존 구현을 변경할 경우 호환성, 전환, Rollback과 폐기 계획

### 기본적으로 적용하지 않는 범위

- INT-05와 직접 관련 없는 코드 정리나 대규모 이름 변경
...
- 팀별로 `연결·응답·전체 Timeout 구분` 해석이 달라져 동일 기능이 여러 방식으로 구현된다.
...
대표 실패 유형은 **다른 WAR Java Import, 타 업무 DB 직접변경, 무조건 재시도, Timeout 역전, 부분성공 상태 미관리**이다. 단기적으로 기능이 동작해도 운영 안정성, 변경 용이성, 감사 가능성과 장애 격리 능력이 약해질 수 있다.
...
| 순서 | 결정 질문 | 답변에 포함할 내용 |
|---:|---|---|
| 1 | `연결·응답·전체 Timeout 구분`의 단일 기준은 무엇인가? | 허용·권고·금지 규칙과 적용 대상 |
| 2 | 기준은 어느 경계에서 실행되는가? | 업무 Service/Facade → tcf-eai 또는 공통 Client → EAI/외부 시스템 → 응답·대사·보상 |
| 3 | 누가 작성·구현·검증·승인·운영하는가? | EAI·TA·AA·FW·TA의 역할 분리 |
...
| 8 | 무엇으로 준수를 증명하는가? | 계약·Timeout·중복·재시도·부분실패·보상·외부 오류 매핑·대사·장애격리 시험 |
| 9 | 완료로 인정할 최소 증적은 무엇인가? | 연계 Timeout표, 테스트, 자동검증, 운영 확인 |
...
**상위 Timeout이 하위 Timeout보다 충분히 크도록 설정한다.**

이행 결과는 `연계 Timeout표` 하나만 제출하는 것으로 끝나지 않는다. 관련 소스·설정·테스트·자동검증·운영 절차가 같은 결정 ID로 추적되어야 한다.
...
```text
업무 Service/Facade → tcf-eai 또는 공통 Client → EAI/외부 시스템 → 응답·대사·보상
```
...
업무 WAR 간 호출은 serviceId 계약과 tcf-eai를 사용하고 변경 요청은 멱등키와 처리상태로 중복을 차단.
...
consulted: "TA·AA·FW"
approver: "TA"
decisionScope: "연결·응답·전체 Timeout 구분"
dueDate: "승인 회의에서 확정"
deliverable: "연계 Timeout표"
```
...
implementation: "상위 Timeout이 하위 Timeout보다 충분히 크도록 설정한다."
alternatives:
  - 중앙 표준·공통 강제
  - 영역 자율·경계 계약
  - 승인된 제한 예외
...
| 구분 | 기록할 값 |
|---|---|
| 요구사항·TASK·ADR | 요구사항 ID, INT-05, ADR ID |
| 설계·가이드 | 문서 경로와 승인 버전 |
...
- 계약·Timeout·중복·재시도·부분실패·보상·외부 오류 매핑·대사·장애격리 시험
...
- [ ] 처음 읽는 사람이 `연결·응답·전체 Timeout 구분`의 의미와 결정 이유를 설명할 수 있다.
...
- [ ] 주관 EAI, 협의 TA·AA·FW, 승인 TA의 행동이 구분되어 있다.
- [ ] `연계 Timeout표`의 작성자·검토자·기한이 있다.
...
INT-05의 목적은 **연계 Timeout**에 관한 결정을 문서 한 장으로 끝내는 것이 아니라, `연결·응답·전체 Timeout 구분` 기준을 프로젝트 전체가 같은 방식으로 이해하고 실행·검증하게 만드는 것이다.

최종 승인 시에는 **연결·응답·전체 Timeout을 구분한다.**이라는 방향을 구체적인 값과 경계로 확정하고, **상위 Timeout이 하위 Timeout보다 충분히 크도록 설정한다.**을 코드·설정·테스트·운영 증적으로 입증해야 한다. 그때 비로소 INT-05를 완료로 판정할 수 있다.

## 21. 참조 문서

- [zarchitecture/08-서비스-간-연동-아키텍처.md](../../../zarchitecture/08-서비스-간-연동-아키텍처.md)
- [zdocs-1/architecture/46-service-integration-contract.md](../../../zdocs-1/architecture/46-service-integration-contract.md)
- [tcf-eai/build.gradle](../../../tcf-eai/build.gradle)
