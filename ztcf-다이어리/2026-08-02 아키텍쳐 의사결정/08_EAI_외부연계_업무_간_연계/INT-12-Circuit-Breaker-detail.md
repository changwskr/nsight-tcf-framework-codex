# INT-12 Circuit Breaker 상세 설명서

> 문서 성격: **이해·검토용 상세 초안**  
> 상태: **ARB 승인 전** · 영역: **EAI·외부연계·업무 간 연계** · 우선순위: **P1**

## 1. 이 문서를 먼저 읽는 이유

이 문서는 요약본 [INT-12-Circuit-Breaker.md](./INT-12-Circuit-Breaker.md)의 내용을 처음 접하는 사람도 INT-12의 의미, 필요한 이유, 실제로 결정할 항목과 완료 판단 방법을 이해할 수 있도록 풀어서 설명한다.

INT-12의 핵심 주제는 **Circuit Breaker**이다. 쉽게 말하면 **다른 WAR·기간계·외부기관을 호출할 때 계약, 장애격리, 중복방지와 데이터 정합성을 보장하는 영역**에서 `차단조건·Half-open·복구`에 관한 공통 기준을 정하는 일이다. 이 기준을 정하지 않으면 팀마다 서로 다른 판단을 구현하고, 통합·보안·운영 단계에서 뒤늦게 충돌할 가능성이 높다.
...
| 이번에 확정할 범위 | 차단조건·Half-open·복구 |
| 권고 초안 | 반복 실패 연계에는 Circuit Breaker를 적용한다. |
| 이행 방향 | 차단·시험호출·복구 상태를 운영에서 확인 가능하게 한다. |
| 주관 | EAI |
| 협의 | TA·AA·OPS |
| 승인 | TA |
| 우선순위 | P1 |
...
- INT-12와 직접 관련 없는 코드 정리나 대규모 이름 변경
...
- 팀별로 `차단조건·Half-open·복구` 해석이 달라져 동일 기능이 여러 방식으로 구현된다.
...
| 순서 | 결정 질문 | 답변에 포함할 내용 |
|---:|---|---|
| 1 | `차단조건·Half-open·복구`의 단일 기준은 무엇인가? | 허용·권고·금지 규칙과 적용 대상 |
| 2 | 기준은 어느 경계에서 실행되는가? | 업무 Service/Facade → tcf-eai 또는 공통 Client → EAI/외부 시스템 → 응답·대사·보상 |
| 3 | 누가 작성·구현·검증·승인·운영하는가? | EAI·TA·AA·OPS·TA의 역할 분리 |
...
| 9 | 완료로 인정할 최소 증적은 무엇인가? | 장애격리 기준, 테스트, 자동검증, 운영 확인 |
...
**반복 실패 연계에는 Circuit Breaker를 적용한다.**
...
**차단·시험호출·복구 상태를 운영에서 확인 가능하게 한다.**

이행 결과는 `장애격리 기준` 하나만 제출하는 것으로 끝나지 않는다. 관련 소스·설정·테스트·자동검증·운영 절차가 같은 결정 ID로 추적되어야 한다.
...
consulted: "TA·AA·OPS"
approver: "TA"
decisionScope: "차단조건·Half-open·복구"
dueDate: "승인 회의에서 확정"
deliverable: "장애격리 기준"
```
...
implementation: "차단·시험호출·복구 상태를 운영에서 확인 가능하게 한다."
alternatives:
  - 중앙 표준·공통 강제
  - 영역 자율·경계 계약
  - 승인된 제한 예외
...
| 구분 | 기록할 값 |
|---|---|
| 요구사항·TASK·ADR | 요구사항 ID, INT-12, ADR ID |
| 설계·가이드 | 문서 경로와 승인 버전 |
...
- [ ] 처음 읽는 사람이 `차단조건·Half-open·복구`의 의미와 결정 이유를 설명할 수 있다.
...
- [ ] 주관 EAI, 협의 TA·AA·OPS, 승인 TA의 행동이 구분되어 있다.
- [ ] `장애격리 기준`의 작성자·검토자·기한이 있다.
...
INT-12의 목적은 **Circuit Breaker**에 관한 결정을 문서 한 장으로 끝내는 것이 아니라, `차단조건·Half-open·복구` 기준을 프로젝트 전체가 같은 방식으로 이해하고 실행·검증하게 만드는 것이다.

최종 승인 시에는 **반복 실패 연계에는 Circuit Breaker를 적용한다.**이라는 방향을 구체적인 값과 경계로 확정하고, **차단·시험호출·복구 상태를 운영에서 확인 가능하게 한다.**을 코드·설정·테스트·운영 증적으로 입증해야 한다. 그때 비로소 INT-12를 완료로 판정할 수 있다.
