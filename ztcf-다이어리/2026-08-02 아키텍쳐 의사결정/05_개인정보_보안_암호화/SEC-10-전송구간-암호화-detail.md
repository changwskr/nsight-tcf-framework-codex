...
이 문서는 요약본 [SEC-10-전송구간-암호화.md](./SEC-10-전송구간-암호화.md)의 내용을 처음 접하는 사람도 SEC-10의 의미, 필요한 이유, 실제로 결정할 항목과 완료 판단 방법을 이해할 수 있도록 풀어서 설명한다.

SEC-10의 핵심 주제는 **전송구간 암호화**이다. 쉽게 말하면 **민감정보가 저장·전송·표시·로그·파일 어디에서도 불필요하게 노출되지 않도록 통제하는 영역**에서 `TLS 적용구간·인증서·암호군`에 관한 공통 기준을 정하는 일이다. 이 기준을 정하지 않으면 팀마다 서로 다른 판단을 구현하고, 통합·보안·운영 단계에서 뒤늦게 충돌할 가능성이 높다.
...
| TASK ID | SEC-10 |
| 의사결정 사항 | 전송구간 암호화 |
| 이번에 확정할 범위 | TLS 적용구간·인증서·암호군 |
| 권고 초안 | 모든 주요 통신구간에 TLS를 적용한다. |
| 이행 방향 | TLS와 필드 암호화를 별도 통제로 관리한다. |
| 주관 | TA |
| 협의 | SEC·MCA·EAI |
| 승인 | SEC |
| 우선순위 | P0 |
...
- SEC-10와 직접 관련 없는 코드 정리나 대규모 이름 변경
...
- 팀별로 `TLS 적용구간·인증서·암호군` 해석이 달라져 동일 기능이 여러 방식으로 구현된다.
...
| 순서 | 결정 질문 | 답변에 포함할 내용 |
|---:|---|---|
| 1 | `TLS 적용구간·인증서·암호군`의 단일 기준은 무엇인가? | 허용·권고·금지 규칙과 적용 대상 |
| 2 | 기준은 어느 경계에서 실행되는가? | 데이터 분류 → 권한 확인 → 서버 마스킹/암복호화 → 응답·로그·파일 → 감사 |
| 3 | 누가 작성·구현·검증·승인·운영하는가? | TA·SEC·MCA·EAI·SEC의 역할 분리 |
...
| 9 | 완료로 인정할 최소 증적은 무엇인가? | TLS 적용설계, 테스트, 자동검증, 운영 확인 |
...
**TLS와 필드 암호화를 별도 통제로 관리한다.**

이행 결과는 `TLS 적용설계` 하나만 제출하는 것으로 끝나지 않는다. 관련 소스·설정·테스트·자동검증·운영 절차가 같은 결정 ID로 추적되어야 한다.
...
| 협의 SEC·MCA·EAI | 영역 요구와 부작용 검토, 테스트·운영 조건 제시 | 검토 의견, 영향도, 조건부 조치 |
...
priority: P0
owner: TA
consulted: "SEC·MCA·EAI"
approver: "SEC"
decisionScope: "TLS 적용구간·인증서·암호군"
dueDate: "승인 회의에서 확정"
deliverable: "TLS 적용설계"
```
...
implementation: "TLS와 필드 암호화를 별도 통제로 관리한다."
alternatives:
  - 중앙 표준·공통 강제
  - 영역 자율·경계 계약
  - 승인된 제한 예외
...
| 구분 | 기록할 값 |
|---|---|
| 요구사항·TASK·ADR | 요구사항 ID, SEC-10, ADR ID |
| 설계·가이드 | 문서 경로와 승인 버전 |
...
- [ ] 처음 읽는 사람이 `TLS 적용구간·인증서·암호군`의 의미와 결정 이유를 설명할 수 있다.
...
- [ ] 주관 TA, 협의 SEC·MCA·EAI, 승인 SEC의 행동이 구분되어 있다.
- [ ] `TLS 적용설계`의 작성자·검토자·기한이 있다.
...
SEC-10의 목적은 **전송구간 암호화**에 관한 결정을 문서 한 장으로 끝내는 것이 아니라, `TLS 적용구간·인증서·암호군` 기준을 프로젝트 전체가 같은 방식으로 이해하고 실행·검증하게 만드는 것이다.

최종 승인 시에는 **모든 주요 통신구간에 TLS를 적용한다.**이라는 방향을 구체적인 값과 경계로 확정하고, **TLS와 필드 암호화를 별도 통제로 관리한다.**을 코드·설정·테스트·운영 증적으로 입증해야 한다. 그때 비로소 SEC-10를 완료로 판정할 수 있다.
