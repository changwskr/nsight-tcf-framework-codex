# SEC-08 암복호화 수행 위치 상세 설명서

> 문서 성격: **이해·검토용 상세 초안**  
> 상태: **ARB 승인 전** · 영역: **개인정보·보안·암호화** · 우선순위: **P0**

## 1. 이 문서를 먼저 읽는 이유

이 문서는 요약본 [SEC-08-암복호화-수행-위치.md](./SEC-08-암복호화-수행-위치.md)의 내용을 처음 접하는 사람도 SEC-08의 의미, 필요한 이유, 실제로 결정할 항목과 완료 판단 방법을 이해할 수 있도록 풀어서 설명한다.

SEC-08의 핵심 주제는 **암복호화 수행 위치**이다. 쉽게 말하면 **민감정보가 저장·전송·표시·로그·파일 어디에서도 불필요하게 노출되지 않도록 통제하는 영역**에서 `UI·MCA·EAI·서버 Adapter 경계`에 관한 공통 기준을 정하는 일이다. 이 기준을 정하지 않으면 팀마다 서로 다른 판단을 구현하고, 통합·보안·운영 단계에서 뒤늦게 충돌할 가능성이 높다.
...
| TASK ID | SEC-08 |
| 의사결정 사항 | 암복호화 수행 위치 |
| 이번에 확정할 범위 | UI·MCA·EAI·서버 Adapter 경계 |
| 권고 초안 | 암복호화는 공통 Security Adapter에서 수행한다. |
| 이행 방향 | 업무 Service마다 암호화 코드를 중복 구현하지 않는다. |
| 주관 | AA |
| 협의 | SEC·FW·MCA |
| 승인 | SEC |
| 우선순위 | P0 |
...
개인정보·보안·암호화는 **민감정보가 저장·전송·표시·로그·파일 어디에서도 불필요하게 노출되지 않도록 통제하는 영역**이다.
...
- 의사결정 주제: `UI·MCA·EAI·서버 Adapter 경계`
- 책임 경계: 데이터 분류 → 권한 확인 → 서버 마스킹/암복호화 → 응답·로그·파일 → 감사
- 관련 구성요소: 개인정보 분류표, Security Adapter, Key Provider/KMS/HSM, 마스킹 정책, 감사로그, Secret Store
- 설계·개발·시험·배포·운영 중 이 결정의 영향을 받는 산출물
- 기존 구현을 변경할 경우 호환성, 전환, Rollback과 폐기 계획

### 기본적으로 적용하지 않는 범위

- SEC-08와 직접 관련 없는 코드 정리나 대규모 이름 변경
...
- 팀별로 `UI·MCA·EAI·서버 Adapter 경계` 해석이 달라져 동일 기능이 여러 방식으로 구현된다.
...
대표 실패 유형은 **업무별 암호화 구현, 소스·YAML 평문 Key, 화면만 마스킹, 암호문·Token 로그, 영구 복호화 예외**이다. 단기적으로 기능이 동작해도 운영 안정성, 변경 용이성, 감사 가능성과 장애 격리 능력이 약해질 수 있다.
...
| 순서 | 결정 질문 | 답변에 포함할 내용 |
|---:|---|---|
| 1 | `UI·MCA·EAI·서버 Adapter 경계`의 단일 기준은 무엇인가? | 허용·권고·금지 규칙과 적용 대상 |
| 2 | 기준은 어느 경계에서 실행되는가? | 데이터 분류 → 권한 확인 → 서버 마스킹/암복호화 → 응답·로그·파일 → 감사 |
| 3 | 누가 작성·구현·검증·승인·운영하는가? | AA·SEC·FW·MCA·SEC의 역할 분리 |
...
| 8 | 무엇으로 준수를 증명하는가? | 권한별 원문·부분·제거 응답, 키 교체, 복호화 실패, 로그·Dump·파일 노출과 서버 우회 보안시험 |
| 9 | 완료로 인정할 최소 증적은 무엇인가? | 암복호화 구조도, 테스트, 자동검증, 운영 확인 |
...
**암복호화는 공통 Security Adapter에서 수행한다.**
...
**업무 Service마다 암호화 코드를 중복 구현하지 않는다.**

이행 결과는 `암복호화 구조도` 하나만 제출하는 것으로 끝나지 않는다. 관련 소스·설정·테스트·자동검증·운영 절차가 같은 결정 ID로 추적되어야 한다.
...
```text
데이터 분류 → 권한 확인 → 서버 마스킹/암복호화 → 응답·로그·파일 → 감사
```
...
| 협의 SEC·FW·MCA | 영역 요구와 부작용 검토, 테스트·운영 조건 제시 | 검토 의견, 영향도, 조건부 조치 |
...
고객식별정보는 서버 정책이 사용자 목적과 권한을 확인해 원문·부분마스킹·필드제거 중 하나로 응답하고 원문조회는 감사.
...
priority: P0
owner: AA
consulted: "SEC·FW·MCA"
approver: "SEC"
decisionScope: "UI·MCA·EAI·서버 Adapter 경계"
dueDate: "승인 회의에서 확정"
deliverable: "암복호화 구조도"
```
...
implementation: "업무 Service마다 암호화 코드를 중복 구현하지 않는다."
alternatives:
  - 중앙 표준·공통 강제
  - 영역 자율·경계 계약
  - 승인된 제한 예외
...
| 구분 | 기록할 값 |
|---|---|
| 요구사항·TASK·ADR | 요구사항 ID, SEC-08, ADR ID |
| 설계·가이드 | 문서 경로와 승인 버전 |
...
- 권한별 원문·부분·제거 응답, 키 교체, 복호화 실패, 로그·Dump·파일 노출과 서버 우회 보안시험
...
- [ ] 처음 읽는 사람이 `UI·MCA·EAI·서버 Adapter 경계`의 의미와 결정 이유를 설명할 수 있다.
...
- [ ] 주관 AA, 협의 SEC·FW·MCA, 승인 SEC의 행동이 구분되어 있다.
- [ ] `암복호화 구조도`의 작성자·검토자·기한이 있다.
...
SEC-08의 목적은 **암복호화 수행 위치**에 관한 결정을 문서 한 장으로 끝내는 것이 아니라, `UI·MCA·EAI·서버 Adapter 경계` 기준을 프로젝트 전체가 같은 방식으로 이해하고 실행·검증하게 만드는 것이다.

최종 승인 시에는 **암복호화는 공통 Security Adapter에서 수행한다.**이라는 방향을 구체적인 값과 경계로 확정하고, **업무 Service마다 암호화 코드를 중복 구현하지 않는다.**을 코드·설정·테스트·운영 증적으로 입증해야 한다. 그때 비로소 SEC-08를 완료로 판정할 수 있다.

## 21. 참조 문서

- [zdocs-1/architecture/43-security-operations.md](../../../zdocs-1/architecture/43-security-operations.md)
- [zarchitecture/07-세션-인증-보안-아키텍처.md](../../../zarchitecture/07-세션-인증-보안-아키텍처.md)
- [xdoc/agents/security-agent.md](../../../xdoc/agents/security-agent.md)
