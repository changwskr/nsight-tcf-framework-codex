# DATA-13 Lock 처리 상세 설명서

> 문서 성격: **이해·검토용 상세 초안**  
> 상태: **ARB 승인 전** · 영역: **데이터·DB·SQL** · 우선순위: **P1**

## 1. 이 문서를 먼저 읽는 이유

이 문서는 요약본 [DATA-13-Lock-처리.md](./DATA-13-Lock-처리.md)의 내용을 처음 접하는 사람도 DATA-13의 의미, 필요한 이유, 실제로 결정할 항목과 완료 판단 방법을 이해할 수 있도록 풀어서 설명한다.

DATA-13의 핵심 주제는 **Lock 처리**이다. 쉽게 말하면 **데이터의 소유권·모델·접근·동시성·성능·보존과 변경 배포 방식을 정하는 영역**에서 `Lock Timeout·Deadlock·재시도`에 관한 공통 기준을 정하는 일이다. 이 기준을 정하지 않으면 팀마다 서로 다른 판단을 구현하고, 통합·보안·운영 단계에서 뒤늦게 충돌할 가능성이 높다.
...
| TASK ID | DATA-13 |
| 의사결정 사항 | Lock 처리 |
| 이번에 확정할 범위 | Lock Timeout·Deadlock·재시도 |
| 권고 초안 | Lock Timeout과 Deadlock 대응기준을 정의한다. |
| 이행 방향 | 자동 재시도 가능 거래와 불가능 거래를 구분한다. |
| 주관 | DBA |
| 협의 | DA·AA·OPS |
| 승인 | DA |
| 우선순위 | P1 |
...
데이터·DB·SQL는 **데이터의 소유권·모델·접근·동시성·성능·보존과 변경 배포 방식을 정하는 영역**이다.
...
- 의사결정 주제: `Lock Timeout·Deadlock·재시도`
- 책임 경계: 업무 도메인 → DAO/Mapper → 소유 데이터베이스와 승인된 조회·연계 계약
- 관련 구성요소: 논리·물리 모델, 표준용어, MyBatis Mapper, Bind SQL, Index, Lock, Migration, 보존·폐기 정책
- 설계·개발·시험·배포·운영 중 이 결정의 영향을 받는 산출물
- 기존 구현을 변경할 경우 호환성, 전환, Rollback과 폐기 계획

### 기본적으로 적용하지 않는 범위

- DATA-13와 직접 관련 없는 코드 정리나 대규모 이름 변경
...
- 팀별로 `Lock Timeout·Deadlock·재시도` 해석이 달라져 동일 기능이 여러 방식으로 구현된다.
...
대표 실패 유형은 **타 업무 테이블 직접변경, 문자열 결합 SQL, 무제한 조회, 실행계획 미검증, 스키마 선후관계·Rollback 누락**이다. 단기적으로 기능이 동작해도 운영 안정성, 변경 용이성, 감사 가능성과 장애 격리 능력이 약해질 수 있다.
...
| 순서 | 결정 질문 | 답변에 포함할 내용 |
|---:|---|---|
| 1 | `Lock Timeout·Deadlock·재시도`의 단일 기준은 무엇인가? | 허용·권고·금지 규칙과 적용 대상 |
| 2 | 기준은 어느 경계에서 실행되는가? | 업무 도메인 → DAO/Mapper → 소유 데이터베이스와 승인된 조회·연계 계약 |
| 3 | 누가 작성·구현·검증·승인·운영하는가? | DBA·DA·AA·OPS·DA의 역할 분리 |
...
| 8 | 무엇으로 준수를 증명하는가? | 모델·DDL·Mapper 정합성, SQL Injection, 실행계획, 페이징, Lock, Rollback, Migration 전·후 검증 |
| 9 | 완료로 인정할 최소 증적은 무엇인가? | Lock 대응기준, 테스트, 자동검증, 운영 확인 |
...
**Lock Timeout과 Deadlock 대응기준을 정의한다.**
...
**자동 재시도 가능 거래와 불가능 거래를 구분한다.**

이행 결과는 `Lock 대응기준` 하나만 제출하는 것으로 끝나지 않는다. 관련 소스·설정·테스트·자동검증·운영 절차가 같은 결정 ID로 추적되어야 한다.
...
```text
업무 도메인 → DAO/Mapper → 소유 데이터베이스와 승인된 조회·연계 계약
```
...
| 협의 DA·AA·OPS | 영역 요구와 부작용 검토, 테스트·운영 조건 제시 | 검토 의견, 영향도, 조건부 조치 |
...
소유 도메인만 테이블을 변경하고 다른 업무는 공개 계약으로 요청하며 Mapper는 파라미터 바인딩을 사용.
...
consulted: "DA·AA·OPS"
approver: "DA"
decisionScope: "Lock Timeout·Deadlock·재시도"
dueDate: "승인 회의에서 확정"
deliverable: "Lock 대응기준"
```
...
implementation: "자동 재시도 가능 거래와 불가능 거래를 구분한다."
alternatives:
  - 중앙 표준·공통 강제
  - 영역 자율·경계 계약
  - 승인된 제한 예외
...
| 구분 | 기록할 값 |
|---|---|
| 요구사항·TASK·ADR | 요구사항 ID, DATA-13, ADR ID |
| 설계·가이드 | 문서 경로와 승인 버전 |
...
- 모델·DDL·Mapper 정합성, SQL Injection, 실행계획, 페이징, Lock, Rollback, Migration 전·후 검증
...
- [ ] 처음 읽는 사람이 `Lock Timeout·Deadlock·재시도`의 의미와 결정 이유를 설명할 수 있다.
...
- [ ] 주관 DBA, 협의 DA·AA·OPS, 승인 DA의 행동이 구분되어 있다.
- [ ] `Lock 대응기준`의 작성자·검토자·기한이 있다.
...
DATA-13의 목적은 **Lock 처리**에 관한 결정을 문서 한 장으로 끝내는 것이 아니라, `Lock Timeout·Deadlock·재시도` 기준을 프로젝트 전체가 같은 방식으로 이해하고 실행·검증하게 만드는 것이다.

최종 승인 시에는 **Lock Timeout과 Deadlock 대응기준을 정의한다.**이라는 방향을 구체적인 값과 경계로 확정하고, **자동 재시도 가능 거래와 불가능 거래를 구분한다.**을 코드·설정·테스트·운영 증적으로 입증해야 한다. 그때 비로소 DATA-13를 완료로 판정할 수 있다.

## 21. 참조 문서

- [zarchitecture/09-데이터-DB-아키텍처.md](../../../zarchitecture/09-데이터-DB-아키텍처.md)
- [zdocs-1/architecture/07-DAO.md](../../../zdocs-1/architecture/07-DAO.md)
- [zdocs-1/architecture/26-mybatis.md](../../../zdocs-1/architecture/26-mybatis.md)
- [zdocs-1/architecture/47-data-governance.md](../../../zdocs-1/architecture/47-data-governance.md)
