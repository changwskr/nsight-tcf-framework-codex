# BFC-02 Batch Job 구조 상세 설명서

> 문서 성격: **이해·검토용 상세 초안**  
> 상태: **ARB 승인 전** · 영역: **배치·파일·캐시** · 우선순위: **P1**

## 1. 이 문서를 먼저 읽는 이유

이 문서는 요약본 [BFC-02-Batch-Job-구조.md](./BFC-02-Batch-Job-구조.md)의 내용을 처음 접하는 사람도 BFC-02의 의미, 필요한 이유, 실제로 결정할 항목과 완료 판단 방법을 이해할 수 있도록 풀어서 설명한다.

BFC-02의 핵심 주제는 **Batch Job 구조**이다. 쉽게 말하면 **대량·주기·파일·반복조회 작업을 온라인 거래와 분리하고 재시작·대사·일관성을 보장하는 영역**에서 `Job·Step·Tasklet·Reader·Writer`에 관한 공통 기준을 정하는 일이다. 이 기준을 정하지 않으면 팀마다 서로 다른 판단을 구현하고, 통합·보안·운영 단계에서 뒤늦게 충돌할 가능성이 높다.
...
|---|---|
| TASK ID | BFC-02 |
| 의사결정 사항 | Batch Job 구조 |
| 이번에 확정할 범위 | Job·Step·Tasklet·Reader·Writer |
| 권고 초안 | Batch는 Job·Step·Reader·Processor·Writer 구조를 사용한다. |
| 이행 방향 | 업무로직과 재시작 위치를 명확히 분리한다. |
| 주관 | AA |
| 협의 | FW·DA |
| 승인 | SA |
| 우선순위 | P1 |
...
배치·파일·캐시는 **대량·주기·파일·반복조회 작업을 온라인 거래와 분리하고 재시작·대사·일관성을 보장하는 영역**이다.
...
- 의사결정 주제: `Job·Step·Tasklet·Reader·Writer`
- 책임 경계: Scheduler/Job → Step/Chunk → DB·파일·외부연계, 그리고 Cache 원천·갱신·무효화
- 관련 구성요소: tcf-batch, tcf-cache, Job/Step, Checkpoint, 파일명·Hash·암호화, EhCache Region, TTL·Eviction
- 설계·개발·시험·배포·운영 중 이 결정의 영향을 받는 산출물
- 기존 구현을 변경할 경우 호환성, 전환, Rollback과 폐기 계획

### 기본적으로 적용하지 않는 범위

- BFC-02와 직접 관련 없는 코드 정리나 대규모 이름 변경
...
- 팀별로 `Job·Step·Tasklet·Reader·Writer` 해석이 달라져 동일 기능이 여러 방식으로 구현된다.
...
대표 실패 유형은 **전체 파일 Heap 적재, 중복 Job 실행, 재시작 위치 없음, 대사 누락, 캐시 원천 불명확·무효화 실패**이다. 단기적으로 기능이 동작해도 운영 안정성, 변경 용이성, 감사 가능성과 장애 격리 능력이 약해질 수 있다.
...
| 순서 | 결정 질문 | 답변에 포함할 내용 |
|---:|---|---|
| 1 | `Job·Step·Tasklet·Reader·Writer`의 단일 기준은 무엇인가? | 허용·권고·금지 규칙과 적용 대상 |
| 2 | 기준은 어느 경계에서 실행되는가? | Scheduler/Job → Step/Chunk → DB·파일·외부연계, 그리고 Cache 원천·갱신·무효화 |
...
| 8 | 무엇으로 준수를 증명하는가? | 재시작·중복·부분실패·대용량·파일 대사·암호화·캐시 Hit/Miss·TTL·무효화·장애복구 시험 |
| 9 | 완료로 인정할 최소 증적은 무엇인가? | Batch 설계기준, 테스트, 자동검증, 운영 확인 |
...
**Batch는 Job·Step·Reader·Processor·Writer 구조를 사용한다.**
...
**업무로직과 재시작 위치를 명확히 분리한다.**

이행 결과는 `Batch 설계기준` 하나만 제출하는 것으로 끝나지 않는다. 관련 소스·설정·테스트·자동검증·운영 절차가 같은 결정 ID로 추적되어야 한다.
...
```text
Scheduler/Job → Step/Chunk → DB·파일·외부연계, 그리고 Cache 원천·갱신·무효화
```
...
대용량 파일은 Streaming과 Checkpoint로 처리하고 완료 후 건수·금액·Hash를 대사하며 실패 파일을 격리.
...
consulted: "FW·DA"
approver: "SA"
decisionScope: "Job·Step·Tasklet·Reader·Writer"
dueDate: "승인 회의에서 확정"
deliverable: "Batch 설계기준"
```
...
implementation: "업무로직과 재시작 위치를 명확히 분리한다."
alternatives:
  - 중앙 표준·공통 강제
  - 영역 자율·경계 계약
  - 승인된 제한 예외
...
| 구분 | 기록할 값 |
|---|---|
| 요구사항·TASK·ADR | 요구사항 ID, BFC-02, ADR ID |
| 설계·가이드 | 문서 경로와 승인 버전 |
...
- 재시작·중복·부분실패·대용량·파일 대사·암호화·캐시 Hit/Miss·TTL·무효화·장애복구 시험
...
- [ ] 처음 읽는 사람이 `Job·Step·Tasklet·Reader·Writer`의 의미와 결정 이유를 설명할 수 있다.
...
BFC-02의 목적은 **Batch Job 구조**에 관한 결정을 문서 한 장으로 끝내는 것이 아니라, `Job·Step·Tasklet·Reader·Writer` 기준을 프로젝트 전체가 같은 방식으로 이해하고 실행·검증하게 만드는 것이다.

최종 승인 시에는 **Batch는 Job·Step·Reader·Processor·Writer 구조를 사용한다.**이라는 방향을 구체적인 값과 경계로 확정하고, **업무로직과 재시작 위치를 명확히 분리한다.**을 코드·설정·테스트·운영 증적으로 입증해야 한다. 그때 비로소 BFC-02를 완료로 판정할 수 있다.

## 21. 참조 문서

- [zarchitecture/11-캐시-아키텍처.md](../../../zarchitecture/11-캐시-아키텍처.md)
- [zarchitecture/12-배치-모니터링-아키텍처.md](../../../zarchitecture/12-배치-모니터링-아키텍처.md)
- [zdocs-1/architecture/13-batch.md](../../../zdocs-1/architecture/13-batch.md)
