...
이 문서는 요약본 [GOV-04-공식-기준문서-지정.md](./GOV-04-공식-기준문서-지정.md)의 내용을 처음 접하는 사람도 GOV-04의 의미, 필요한 이유, 실제로 결정할 항목과 완료 판단 방법을 이해할 수 있도록 풀어서 설명한다.

GOV-04의 핵심 주제는 **공식 기준문서 지정**이다. 쉽게 말하면 **누가, 언제, 어떤 근거로 기술 결정을 내리고 변경·폐기까지 책임질지를 정하는 영역**에서 `개발·설계 기준의 단일 기준원`에 관한 공통 기준을 정하는 일이다. 이 기준을 정하지 않으면 팀마다 서로 다른 판단을 구현하고, 통합·보안·운영 단계에서 뒤늦게 충돌할 가능성이 높다.
...
| TASK ID | GOV-04 |
| 의사결정 사항 | 공식 기준문서 지정 |
| 이번에 확정할 범위 | 개발·설계 기준의 단일 기준원 |
| 권고 초안 | 공식 문서는 단일 기준원에서 버전 관리한다. |
| 이행 방향 | 승인본만 기준문서 저장소에 게시하고 초안과 분리한다. |
| 주관 | SA |
| 협의 | AA·DA·TA·SEC |
| 승인 | ARB |
| 우선순위 | P0 |
...
- GOV-04와 직접 관련 없는 코드 정리나 대규모 이름 변경
...
- 팀별로 `개발·설계 기준의 단일 기준원` 해석이 달라져 동일 기능이 여러 방식으로 구현된다.
...
| 순서 | 결정 질문 | 답변에 포함할 내용 |
|---:|---|---|
| 1 | `개발·설계 기준의 단일 기준원`의 단일 기준은 무엇인가? | 허용·권고·금지 규칙과 적용 대상 |
| 2 | 기준은 어느 경계에서 실행되는가? | 프로젝트 책임자·ARB·SA·각 영역 아키텍트·PMO·QA 사이의 의사결정 권한과 증적 흐름 |
| 3 | 누가 작성·구현·검증·승인·운영하는가? | SA·AA·DA·TA·SEC·ARB의 역할 분리 |
...
| 9 | 완료로 인정할 최소 증적은 무엇인가? | 기준문서 목록, 테스트, 자동검증, 운영 확인 |
...
**승인본만 기준문서 저장소에 게시하고 초안과 분리한다.**

이행 결과는 `기준문서 목록` 하나만 제출하는 것으로 끝나지 않는다. 관련 소스·설정·테스트·자동검증·운영 절차가 같은 결정 ID로 추적되어야 한다.
...
요구사항·현행 문제
  ...
  → 대안과 영향도 비교
  → PoC 또는 대표 시나리오 검증
  → SA 권고안 작성 / AA·DA·TA·SEC 협의
  → ARB 승인 및 ADR Baseline
  → 개발표준·공통 구현·샘플 반영
  → 테스트·CI/CD·운영 검증
  → 적용 확인·예외 추적·정기 재검토
```
...
| 협의 AA·DA·TA·SEC | 영역 요구와 부작용 검토, 테스트·운영 조건 제시 | 검토 의견, 영향도, 조건부 조치 |
| 승인 ARB | 선택·기각 근거, 잔여 위험과 예외 수용 여부 결정 | 승인 기록, ADR Baseline |
...
priority: P0
owner: SA
consulted: "AA·DA·TA·SEC"
approver: "ARB"
decisionScope: "개발·설계 기준의 단일 기준원"
dueDate: "승인 회의에서 확정"
deliverable: "기준문서 목록"
```
...
implementation: "승인본만 기준문서 저장소에 게시하고 초안과 분리한다."
alternatives:
  - 중앙 표준·공통 강제
  - 영역 자율·경계 계약
  - 승인된 제한 예외
...
| 구분 | 기록할 값 |
|---|---|
| 요구사항·TASK·ADR | 요구사항 ID, GOV-04, ADR ID |
| 설계·가이드 | 문서 경로와 승인 버전 |
...
- [ ] 처음 읽는 사람이 `개발·설계 기준의 단일 기준원`의 의미와 결정 이유를 설명할 수 있다.
...
- [ ] 주관 SA, 협의 AA·DA·TA·SEC, 승인 ARB의 행동이 구분되어 있다.
- [ ] `기준문서 목록`의 작성자·검토자·기한이 있다.
...
GOV-04의 목적은 **공식 기준문서 지정**에 관한 결정을 문서 한 장으로 끝내는 것이 아니라, `개발·설계 기준의 단일 기준원` 기준을 프로젝트 전체가 같은 방식으로 이해하고 실행·검증하게 만드는 것이다.

최종 승인 시에는 **공식 문서는 단일 기준원에서 버전 관리한다.**이라는 방향을 구체적인 값과 경계로 확정하고, **승인본만 기준문서 저장소에 게시하고 초안과 분리한다.**을 코드·설정·테스트·운영 증적으로 입증해야 한다. 그때 비로소 GOV-04를 완료로 판정할 수 있다.
