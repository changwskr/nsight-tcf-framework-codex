# GOV-09 개발 완료 기준 상세 설명서

> 문서 성격: **이해·검토용 상세 초안**  
> 상태: **ARB 승인 전** · 영역: **거버넌스·아키텍처 관리** · 우선순위: **P0**

## 1. 이 문서를 먼저 읽는 이유

이 문서는 요약본 [GOV-09-개발-완료-기준.md](./GOV-09-개발-완료-기준.md)의 내용을 처음 접하는 사람도 GOV-09의 의미, 필요한 이유, 실제로 결정할 항목과 완료 판단 방법을 이해할 수 있도록 풀어서 설명한다.

GOV-09의 핵심 주제는 **개발 완료 기준**이다. 쉽게 말하면 **누가, 언제, 어떤 근거로 기술 결정을 내리고 변경·폐기까지 책임질지를 정하는 영역**에서 `설계·코드·설정·시험·운영 증적`에 관한 공통 기준을 정하는 일이다. 이 기준을 정하지 않으면 팀마다 서로 다른 판단을 구현하고, 통합·보안·운영 단계에서 뒤늦게 충돌할 가능성이 높다.
...
| TASK ID | GOV-09 |
| 의사결정 사항 | 개발 완료 기준 |
| 이번에 확정할 범위 | 설계·코드·설정·시험·운영 증적 |
| 권고 초안 | 개발 완료는 코드 작성이 아닌 증적 정합성으로 판단한다. |
| 이행 방향 | 설계·소스·설정·테스트·OM·로그를 완료조건에 포함한다. |
| 주관 | QA |
| 협의 | AA·DVO·OPS |
| 승인 | SA |
| 우선순위 | P0 |
| 필수 산출물 | Definition of Done |

## 3. 용어와 배경지식

### 3.1 영역의 의미

거버넌스·아키텍처 관리는 **누가, 언제, 어떤 근거로 기술 결정을 내리고 변경·폐기까지 책임질지를 정하는 영역**이다.
...
- 의사결정 주제: `설계·코드·설정·시험·운영 증적`
- 책임 경계: 프로젝트 책임자·ARB·SA·각 영역 아키텍트·PMO·QA 사이의 의사결정 권한과 증적 흐름
- 관련 구성요소: TASK 원장, ADR, RACI, Architecture Gate, 예외승인서, Risk·Gap·기술부채 원장
- 설계·개발·시험·배포·운영 중 이 결정의 영향을 받는 산출물
- 기존 구현을 변경할 경우 호환성, 전환, Rollback과 폐기 계획

### 기본적으로 적용하지 않는 범위

- GOV-09와 직접 관련 없는 코드 정리나 대규모 이름 변경
...
- 팀별로 `설계·코드·설정·시험·운영 증적` 해석이 달라져 동일 기능이 여러 방식으로 구현된다.
...
대표 실패 유형은 **구두 결정, 승인자 부재, 조건부 조치 미추적, 만료 없는 예외, 문서와 구현의 기준선 불일치**이다. 단기적으로 기능이 동작해도 운영 안정성, 변경 용이성, 감사 가능성과 장애 격리 능력이 약해질 수 있다.
...
| 순서 | 결정 질문 | 답변에 포함할 내용 |
|---:|---|---|
| 1 | `설계·코드·설정·시험·운영 증적`의 단일 기준은 무엇인가? | 허용·권고·금지 규칙과 적용 대상 |
| 2 | 기준은 어느 경계에서 실행되는가? | 프로젝트 책임자·ARB·SA·각 영역 아키텍트·PMO·QA 사이의 의사결정 권한과 증적 흐름 |
| 3 | 누가 작성·구현·검증·승인·운영하는가? | QA·AA·DVO·OPS·SA의 역할 분리 |
...
| 8 | 무엇으로 준수를 증명하는가? | 승인 이력과 Baseline, 대체 ADR 연결, Gate 차단항목, 조건부 조치의 책임자·기한·검증자 |
| 9 | 완료로 인정할 최소 증적은 무엇인가? | Definition of Done, 테스트, 자동검증, 운영 확인 |
...
**개발 완료는 코드 작성이 아닌 증적 정합성으로 판단한다.**
...
**설계·소스·설정·테스트·OM·로그를 완료조건에 포함한다.**

이행 결과는 `Definition of Done` 하나만 제출하는 것으로 끝나지 않는다. 관련 소스·설정·테스트·자동검증·운영 절차가 같은 결정 ID로 추적되어야 한다.
...
```text
프로젝트 책임자·ARB·SA·각 영역 아키텍트·PMO·QA 사이의 의사결정 권한과 증적 흐름
```
...
| 협의 AA·DVO·OPS | 영역 요구와 부작용 검토, 테스트·운영 조건 제시 | 검토 의견, 영향도, 조건부 조치 |
...
ServiceId 규칙 변경 시 SA가 TASK를 등록하고 FW·AA·OM·QA가 영향을 검토한 뒤 ARB가 호환기간과 자동검사를 포함해 승인.
...
priority: P0
owner: QA
consulted: "AA·DVO·OPS"
approver: "SA"
decisionScope: "설계·코드·설정·시험·운영 증적"
dueDate: "승인 회의에서 확정"
...
implementation: "설계·소스·설정·테스트·OM·로그를 완료조건에 포함한다."
alternatives:
  - 중앙 표준·공통 강제
  - 영역 자율·경계 계약
  - 승인된 제한 예외
...
| 구분 | 기록할 값 |
|---|---|
| 요구사항·TASK·ADR | 요구사항 ID, GOV-09, ADR ID |
| 설계·가이드 | 문서 경로와 승인 버전 |
...
- 승인 이력과 Baseline, 대체 ADR 연결, Gate 차단항목, 조건부 조치의 책임자·기한·검증자
...
- [ ] 처음 읽는 사람이 `설계·코드·설정·시험·운영 증적`의 의미와 결정 이유를 설명할 수 있다.
...
- [ ] 주관 QA, 협의 AA·DVO·OPS, 승인 SA의 행동이 구분되어 있다.
- [ ] `Definition of Done`의 작성자·검토자·기한이 있다.
...
GOV-09의 목적은 **개발 완료 기준**에 관한 결정을 문서 한 장으로 끝내는 것이 아니라, `설계·코드·설정·시험·운영 증적` 기준을 프로젝트 전체가 같은 방식으로 이해하고 실행·검증하게 만드는 것이다.

최종 승인 시에는 **개발 완료는 코드 작성이 아닌 증적 정합성으로 판단한다.**이라는 방향을 구체적인 값과 경계로 확정하고, **설계·소스·설정·테스트·OM·로그를 완료조건에 포함한다.**을 코드·설정·테스트·운영 증적으로 입증해야 한다. 그때 비로소 GOV-09를 완료로 판정할 수 있다.

## 21. 참조 문서

- [AGENTS.md](../../../AGENTS.md)
- [xdoc/agents/development-agent-guide.md](../../../xdoc/agents/development-agent-guide.md)
- [znsight-man/NSIGHT TCF 아키텍처 구축 방법론 - 3. Architecture Gate 단계별 체크리스트.md](../../../znsight-man/NSIGHT%20TCF%20아키텍처%20구축%20방법론%20-%203.%20Architecture%20Gate%20단계별%20체크리스트.md)
- [ztcf-book-capacity-md/부록/AC-아키텍처-의사결정-기록.md](../../../ztcf-book-capacity-md/부록/AC-아키텍처-의사결정-기록.md)
