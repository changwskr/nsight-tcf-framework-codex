# QLT-06 계층 자동검증 상세 설명서

> 문서 성격: **이해·검토용 상세 초안**  
> 상태: **ARB 승인 전** · 영역: **DevOps·CI/CD·품질검증** · 우선순위: **P1**

## 1. 이 문서를 먼저 읽는 이유

이 문서는 요약본 [QLT-06-계층-자동검증.md](./QLT-06-계층-자동검증.md)의 내용을 처음 접하는 사람도 QLT-06의 의미, 필요한 이유, 실제로 결정할 항목과 완료 판단 방법을 이해할 수 있도록 풀어서 설명한다.

QLT-06의 핵심 주제는 **계층 자동검증**이다. 쉽게 말하면 **표준 위반과 결함을 사람의 기억이 아니라 빌드·테스트·배포 Gate에서 반복 차단하는 영역**에서 `계층 우회·직접 Mapper 호출 차단`에 관한 공통 기준을 정하는 일이다. 이 기준을 정하지 않으면 팀마다 서로 다른 판단을 구현하고, 통합·보안·운영 단계에서 뒤늦게 충돌할 가능성이 높다.
...
| TASK ID | QLT-06 |
| 의사결정 사항 | 계층 자동검증 |
| 이번에 확정할 범위 | 계층 우회·직접 Mapper 호출 차단 |
| 권고 초안 | 계층 위반을 ArchUnit으로 차단한다. |
| 이행 방향 | Handler→Mapper, Controller→Service 등 금지 호출을 검출한다. |
| 주관 | FW |
| 협의 | AA·QA |
| 승인 | AA |
| 우선순위 | P1 |
...
- QLT-06와 직접 관련 없는 코드 정리나 대규모 이름 변경
...
- 팀별로 `계층 우회·직접 Mapper 호출 차단` 해석이 달라져 동일 기능이 여러 방식으로 구현된다.
...
| 순서 | 결정 질문 | 답변에 포함할 내용 |
|---:|---|---|
| 1 | `계층 우회·직접 Mapper 호출 차단`의 단일 기준은 무엇인가? | 허용·권고·금지 규칙과 적용 대상 |
| 2 | 기준은 어느 경계에서 실행되는가? | Commit/PR → Build → 정적·보안·구조·계약·통합 시험 → Artifact → 배포 승인 → Smoke/Health 검증 |
| 3 | 누가 작성·구현·검증·승인·운영하는가? | FW·AA·QA·AA의 역할 분리 |
...
| 9 | 완료로 인정할 최소 증적은 무엇인가? | ArchUnit 규칙, 테스트, 자동검증, 운영 확인 |
...
**Handler→Mapper, Controller→Service 등 금지 호출을 검출한다.**

이행 결과는 `ArchUnit 규칙` 하나만 제출하는 것으로 끝나지 않는다. 관련 소스·설정·테스트·자동검증·운영 절차가 같은 결정 ID로 추적되어야 한다.
...
consulted: "AA·QA"
approver: "AA"
decisionScope: "계층 우회·직접 Mapper 호출 차단"
dueDate: "승인 회의에서 확정"
deliverable: "ArchUnit 규칙"
```
...
implementation: "Handler→Mapper, Controller→Service 등 금지 호출을 검출한다."
alternatives:
  - 중앙 표준·공통 강제
  - 영역 자율·경계 계약
  - 승인된 제한 예외
...
| 구분 | 기록할 값 |
|---|---|
| 요구사항·TASK·ADR | 요구사항 ID, QLT-06, ADR ID |
| 설계·가이드 | 문서 경로와 승인 버전 |
...
- [ ] 처음 읽는 사람이 `계층 우회·직접 Mapper 호출 차단`의 의미와 결정 이유를 설명할 수 있다.
...
- [ ] 주관 FW, 협의 AA·QA, 승인 AA의 행동이 구분되어 있다.
- [ ] `ArchUnit 규칙`의 작성자·검토자·기한이 있다.
...
QLT-06의 목적은 **계층 자동검증**에 관한 결정을 문서 한 장으로 끝내는 것이 아니라, `계층 우회·직접 Mapper 호출 차단` 기준을 프로젝트 전체가 같은 방식으로 이해하고 실행·검증하게 만드는 것이다.

최종 승인 시에는 **계층 위반을 ArchUnit으로 차단한다.**이라는 방향을 구체적인 값과 경계로 확정하고, **Handler→Mapper, Controller→Service 등 금지 호출을 검출한다.**을 코드·설정·테스트·운영 증적으로 입증해야 한다. 그때 비로소 QLT-06를 완료로 판정할 수 있다.
