# QLT-07 ServiceId 중복검사 상세 설명서

> 문서 성격: **이해·검토용 상세 초안**  
> 상태: **ARB 승인 전** · 영역: **DevOps·CI/CD·품질검증** · 우선순위: **P0**

## 1. 이 문서를 먼저 읽는 이유

이 문서는 요약본 [QLT-07-ServiceId-중복검사.md](./QLT-07-ServiceId-중복검사.md)의 내용을 처음 접하는 사람도 QLT-07의 의미, 필요한 이유, 실제로 결정할 항목과 완료 판단 방법을 이해할 수 있도록 풀어서 설명한다.

QLT-07의 핵심 주제는 **ServiceId 중복검사**이다. 쉽게 말하면 **표준 위반과 결함을 사람의 기억이 아니라 빌드·테스트·배포 Gate에서 반복 차단하는 영역**에서 `소스·Handler·OM 중복·누락`에 관한 공통 기준을 정하는 일이다. 이 기준을 정하지 않으면 팀마다 서로 다른 판단을 구현하고, 통합·보안·운영 단계에서 뒤늦게 충돌할 가능성이 높다.
...
|---|---|
| TASK ID | QLT-07 |
| 의사결정 사항 | ServiceId 중복검사 |
| 이번에 확정할 범위 | 소스·Handler·OM 중복·누락 |
| 권고 초안 | ServiceId의 중복·미등록·Handler 불일치를 검사한다. |
| 이행 방향 | 소스와 OM Catalog를 자동 대조한다. |
| 주관 | FW |
| 협의 | AA·OM·QA |
| 승인 | AA |
| 우선순위 | P0 |
...
DevOps·CI/CD·품질검증는 **표준 위반과 결함을 사람의 기억이 아니라 빌드·테스트·배포 Gate에서 반복 차단하는 영역**이다.
...
- 의사결정 주제: `소스·Handler·OM 중복·누락`
- 책임 경계: Commit/PR → Build → 정적·보안·구조·계약·통합 시험 → Artifact → 배포 승인 → Smoke/Health 검증
- 관련 구성요소: Gradle Wrapper, CI Pipeline, ArchUnit, Checkstyle, SAST/SCA/Secret Scan, 테스트, Artifact Manifest, Drift 검사
- 설계·개발·시험·배포·운영 중 이 결정의 영향을 받는 산출물
- 기존 구현을 변경할 경우 호환성, 전환, Rollback과 폐기 계획

### 기본적으로 적용하지 않는 범위

- QLT-07와 직접 관련 없는 코드 정리나 대규모 이름 변경
...
- 팀별로 `소스·Handler·OM 중복·누락` 해석이 달라져 동일 기능이 여러 방식으로 구현된다.
...
대표 실패 유형은 **환경별 비재현 빌드, main 직접반영, 검사 실패 무시, 미등록 ServiceId 배포, 근거 없는 예외 승인**이다. 단기적으로 기능이 동작해도 운영 안정성, 변경 용이성, 감사 가능성과 장애 격리 능력이 약해질 수 있다.
...
| 순서 | 결정 질문 | 답변에 포함할 내용 |
|---:|---|---|
| 1 | `소스·Handler·OM 중복·누락`의 단일 기준은 무엇인가? | 허용·권고·금지 규칙과 적용 대상 |
| 2 | 기준은 어느 경계에서 실행되는가? | Commit/PR → Build → 정적·보안·구조·계약·통합 시험 → Artifact → 배포 승인 → Smoke/Health 검증 |
| 3 | 누가 작성·구현·검증·승인·운영하는가? | FW·AA·OM·QA·AA의 역할 분리 |
...
| 8 | 무엇으로 준수를 증명하는가? | 동일 JDK·Wrapper 재현, 필수 Gate 실패 차단, Artifact 추적, 배포 후 대표 거래·Health, Rollback 검증 |
| 9 | 완료로 인정할 최소 증적은 무엇인가? | ServiceId 검사, 테스트, 자동검증, 운영 확인 |
...
**ServiceId의 중복·미등록·Handler 불일치를 검사한다.**
...
**소스와 OM Catalog를 자동 대조한다.**

이행 결과는 `ServiceId 검사` 하나만 제출하는 것으로 끝나지 않는다. 관련 소스·설정·테스트·자동검증·운영 절차가 같은 결정 ID로 추적되어야 한다.
...
```text
Commit/PR → Build → 정적·보안·구조·계약·통합 시험 → Artifact → 배포 승인 → Smoke/Health 검증
```
...
| 협의 AA·OM·QA | 영역 요구와 부작용 검토, 테스트·운영 조건 제시 | 검토 의견, 영향도, 조건부 조치 |
...
PR에서 계층·ServiceId·보안·테스트 검사를 통과한 동일 Artifact만 환경별 설정을 주입해 순차 배포.
...
```yaml
taskId: QLT-07
title: "ServiceId 중복검사"
status: 분석-중
priority: P0
owner: FW
consulted: "AA·OM·QA"
approver: "AA"
decisionScope: "소스·Handler·OM 중복·누락"
dueDate: "승인 회의에서 확정"
deliverable: "ServiceId 검사"
```
...
implementation: "소스와 OM Catalog를 자동 대조한다."
alternatives:
  - 중앙 표준·공통 강제
  - 영역 자율·경계 계약
  - 승인된 제한 예외
...
| 구분 | 기록할 값 |
|---|---|
| 요구사항·TASK·ADR | 요구사항 ID, QLT-07, ADR ID |
| 설계·가이드 | 문서 경로와 승인 버전 |
...
- 동일 JDK·Wrapper 재현, 필수 Gate 실패 차단, Artifact 추적, 배포 후 대표 거래·Health, Rollback 검증
...
- [ ] 처음 읽는 사람이 `소스·Handler·OM 중복·누락`의 의미와 결정 이유를 설명할 수 있다.
...
- [ ] 주관 FW, 협의 AA·OM·QA, 승인 AA의 행동이 구분되어 있다.
- [ ] `ServiceId 검사`의 작성자·검토자·기한이 있다.
...
QLT-07의 목적은 **ServiceId 중복검사**에 관한 결정을 문서 한 장으로 끝내는 것이 아니라, `소스·Handler·OM 중복·누락` 기준을 프로젝트 전체가 같은 방식으로 이해하고 실행·검증하게 만드는 것이다.

최종 승인 시에는 **ServiceId의 중복·미등록·Handler 불일치를 검사한다.**이라는 방향을 구체적인 값과 경계로 확정하고, **소스와 OM Catalog를 자동 대조한다.**을 코드·설정·테스트·운영 증적으로 입증해야 한다. 그때 비로소 QLT-07를 완료로 판정할 수 있다.

## 21. 참조 문서

- [build.gradle](../../../build.gradle)
- [zdocs-1/architecture/50-test-architecture.md](../../../zdocs-1/architecture/50-test-architecture.md)
- [zdocs-1/architecture/49-release-strategy.md](../../../zdocs-1/architecture/49-release-strategy.md)
- [tcf-cicd/README.md](../../../tcf-cicd/README.md)
- [xdoc/agents/quality-agent.md](../../../xdoc/agents/quality-agent.md)
