...
이 문서는 요약본 [QLT-01-Git-Branch-전략.md](./QLT-01-Git-Branch-전략.md)의 내용을 처음 접하는 사람도 QLT-01의 의미, 필요한 이유, 실제로 결정할 항목과 완료 판단 방법을 이해할 수 있도록 풀어서 설명한다.

QLT-01의 핵심 주제는 **Git Branch 전략**이다. 쉽게 말하면 **표준 위반과 결함을 사람의 기억이 아니라 빌드·테스트·배포 Gate에서 반복 차단하는 영역**에서 `main·develop·feature·release`에 관한 공통 기준을 정하는 일이다. 이 기준을 정하지 않으면 팀마다 서로 다른 판단을 구현하고, 통합·보안·운영 단계에서 뒤늦게 충돌할 가능성이 높다.
...
|---|---|
| TASK ID | QLT-01 |
| 의사결정 사항 | Git Branch 전략 |
| 이번에 확정할 범위 | main·develop·feature·release |
| 권고 초안 | main·develop·feature·release 브랜치 책임을 분리한다. |
| 이행 방향 | 운영 브랜치 직접 Push를 금지한다. |
| 주관 | DVO |
| 협의 | AA·QA |
| 승인 | SA |
| 우선순위 | P0 |
...
- 의사결정 주제: `main·develop·feature·release`
...
- QLT-01와 직접 관련 없는 코드 정리나 대규모 이름 변경
...
- 팀별로 `main·develop·feature·release` 해석이 달라져 동일 기능이 여러 방식으로 구현된다.
...
| 순서 | 결정 질문 | 답변에 포함할 내용 |
|---:|---|---|
| 1 | `main·develop·feature·release`의 단일 기준은 무엇인가? | 허용·권고·금지 규칙과 적용 대상 |
| 2 | 기준은 어느 경계에서 실행되는가? | Commit/PR → Build → 정적·보안·구조·계약·통합 시험 → Artifact → 배포 승인 → Smoke/Health 검증 |
| 3 | 누가 작성·구현·검증·승인·운영하는가? | DVO·AA·QA·SA의 역할 분리 |
...
| 9 | 완료로 인정할 최소 증적은 무엇인가? | Branch 정책, 테스트, 자동검증, 운영 확인 |
...
**main·develop·feature·release 브랜치 책임을 분리한다.**
...
**운영 브랜치 직접 Push를 금지한다.**

이행 결과는 `Branch 정책` 하나만 제출하는 것으로 끝나지 않는다. 관련 소스·설정·테스트·자동검증·운영 절차가 같은 결정 ID로 추적되어야 한다.
...
| 역할 | 수행 내용 | 제출 증적 |
|---|---|---|
| 주관 DVO | 현행·문제·대안·권고안 작성, 영향 대상 조정 | TASK 분석서, 권고안, ADR 초안 |
| 협의 AA·QA | 영역 요구와 부작용 검토, 테스트·운영 조건 제시 | 검토 의견, 영향도, 조건부 조치 |
...
consulted: "AA·QA"
approver: "SA"
decisionScope: "main·develop·feature·release"
dueDate: "승인 회의에서 확정"
deliverable: "Branch 정책"
```
...
implementation: "운영 브랜치 직접 Push를 금지한다."
alternatives:
  - 중앙 표준·공통 강제
  - 영역 자율·경계 계약
  - 승인된 제한 예외
...
| 구분 | 기록할 값 |
|---|---|
| 요구사항·TASK·ADR | 요구사항 ID, QLT-01, ADR ID |
| 설계·가이드 | 문서 경로와 승인 버전 |
...
- [ ] 처음 읽는 사람이 `main·develop·feature·release`의 의미와 결정 이유를 설명할 수 있다.
...
- [ ] 주관 DVO, 협의 AA·QA, 승인 SA의 행동이 구분되어 있다.
- [ ] `Branch 정책`의 작성자·검토자·기한이 있다.
...
QLT-01의 목적은 **Git Branch 전략**에 관한 결정을 문서 한 장으로 끝내는 것이 아니라, `main·develop·feature·release` 기준을 프로젝트 전체가 같은 방식으로 이해하고 실행·검증하게 만드는 것이다.

최종 승인 시에는 **main·develop·feature·release 브랜치 책임을 분리한다.**이라는 방향을 구체적인 값과 경계로 확정하고, **운영 브랜치 직접 Push를 금지한다.**을 코드·설정·테스트·운영 증적으로 입증해야 한다. 그때 비로소 QLT-01를 완료로 판정할 수 있다.
