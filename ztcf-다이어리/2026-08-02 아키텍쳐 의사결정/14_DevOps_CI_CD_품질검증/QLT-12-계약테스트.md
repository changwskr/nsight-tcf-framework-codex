...
> 상태: **결정 전 설명서 초안**  
> 영역: DevOps·CI/CD·품질검증 · 우선순위: **P1** · 주관: **QA** · 승인: **SA**

## 1. 도입 전 안내말

이 문서는 QLT-12 아키텍처 의사결정을 검토·승인·이행하기 위한 설명서다. 현재 저장소의 구현과 참조 문서를 근거로 작성했으며, ARB 승인 전까지는 권고 초안으로 취급한다.

## 2. 문서 개요

| 항목 | 내용 |
|---|---|
| 목적 | 계약테스트 기준을 프로젝트 단일 원칙으로 확정한다. |
| 결정 범위 | MCA·API·EAI 전문 호환성 |
| 적용 대상 | NSIGHT TCF 프레임워크, 플랫폼 모듈, 업무 WAR와 관련 운영 절차 |
| 대상 독자 | QA, MCA·EAI·AA, SA 및 구현·시험·운영 담당자 |
| 선행조건 | 현행 코드·설정·운영 기준 확인, 영향 대상 식별, 책임자 지정 |
| 주요 산출물 | Contract Test |
...
| 주관(Responsible) | QA |
| 승인(Accountable) | SA |
| 협의(Consulted) | MCA·EAI·AA |
| 구현·검증 | QA·MCA·EAI |
| 공유(Informed) | 영향받는 업무팀·운영팀·QA |

## 12. 정상 처리 흐름

1. QLT-12를 결정 원장에 등록하고 영향 범위를 식별한다.
...
QLT-12 결정, 관련 코드·설정, Contract Test, 자동검증 결과와 운영 확인 내역이 동일 변경 단위로 추적된다.
...
QLT-12는 문서 승인만으로 완료되지 않는다. Contract Test, 구현·설정 반영, 테스트, 자동검증과 업무팀 적용 확인까지 충족해야 완료로 판정한다.
