...
이 문서는 요약본 [INF-02-Tomcat-업무그룹.md](./INF-02-Tomcat-업무그룹.md)의 내용을 처음 접하는 사람도 INF-02의 의미, 필요한 이유, 실제로 결정할 항목과 완료 판단 방법을 이해할 수 있도록 풀어서 설명한다.

INF-02의 핵심 주제는 **Tomcat 업무그룹**이다. 쉽게 말하면 **WAR·Tomcat·JVM·Thread·Connection·네트워크를 목표 부하와 장애범위에 맞게 배치하는 영역**에서 `A/B 그룹·업무 배치기준`에 관한 공통 기준을 정하는 일이다. 이 기준을 정하지 않으면 팀마다 서로 다른 판단을 구현하고, 통합·보안·운영 단계에서 뒤늦게 충돌할 가능성이 높다.
...
| TASK ID | INF-02 |
| 의사결정 사항 | Tomcat 업무그룹 |
| 이번에 확정할 범위 | A/B 그룹·업무 배치기준 |
| 권고 초안 | 업무 중요도와 자원특성에 따라 A/B 그룹으로 분리한다. |
| 이행 방향 | 고부하·핵심 업무가 동일 JVM에 집중되지 않게 한다. |
| 주관 | TA |
| 협의 | AA·OPS |
| 승인 | SA |
| 우선순위 | P0 |
...
- INF-02와 직접 관련 없는 코드 정리나 대규모 이름 변경
...
- 팀별로 `A/B 그룹·업무 배치기준` 해석이 달라져 동일 기능이 여러 방식으로 구현된다.
...
| 순서 | 결정 질문 | 답변에 포함할 내용 |
|---:|---|---|
| 1 | `A/B 그룹·업무 배치기준`의 단일 기준은 무엇인가? | 허용·권고·금지 규칙과 적용 대상 |
...
| 9 | 완료로 인정할 최소 증적은 무엇인가? | 업무 배치표, 테스트, 자동검증, 운영 확인 |
...
**업무 중요도와 자원특성에 따라 A/B 그룹으로 분리한다.**
...
**고부하·핵심 업무가 동일 JVM에 집중되지 않게 한다.**

이행 결과는 `업무 배치표` 하나만 제출하는 것으로 끝나지 않는다. 관련 소스·설정·테스트·자동검증·운영 절차가 같은 결정 ID로 추적되어야 한다.
...
implementation: "고부하·핵심 업무가 동일 JVM에 집중되지 않게 한다."
alternatives:
  - 중앙 표준·공통 강제
  - 영역 자율·경계 계약
  - 승인된 제한 예외
...
| 구분 | 기록할 값 |
|---|---|
| 요구사항·TASK·ADR | 요구사항 ID, INF-02, ADR ID |
| 설계·가이드 | 문서 경로와 승인 버전 |
...
- [ ] 처음 읽는 사람이 `A/B 그룹·업무 배치기준`의 의미와 결정 이유를 설명할 수 있다.
...
INF-02의 목적은 **Tomcat 업무그룹**에 관한 결정을 문서 한 장으로 끝내는 것이 아니라, `A/B 그룹·업무 배치기준` 기준을 프로젝트 전체가 같은 방식으로 이해하고 실행·검증하게 만드는 것이다.

최종 승인 시에는 **업무 중요도와 자원특성에 따라 A/B 그룹으로 분리한다.**이라는 방향을 구체적인 값과 경계로 확정하고, **고부하·핵심 업무가 동일 JVM에 집중되지 않게 한다.**을 코드·설정·테스트·운영 증적으로 입증해야 한다. 그때 비로소 INF-02를 완료로 판정할 수 있다.
