# INF-07 HikariCP Pool 상세 설명서

> 문서 성격: **이해·검토용 상세 초안**  
> 상태: **ARB 승인 전** · 영역: **기술·인프라·용량** · 우선순위: **P1**

## 1. 이 문서를 먼저 읽는 이유

이 문서는 요약본 [INF-07-HikariCP-Pool.md](./INF-07-HikariCP-Pool.md)의 내용을 처음 접하는 사람도 INF-07의 의미, 필요한 이유, 실제로 결정할 항목과 완료 판단 방법을 이해할 수 있도록 풀어서 설명한다.

INF-07의 핵심 주제는 **HikariCP Pool**이다. 쉽게 말하면 **WAR·Tomcat·JVM·Thread·Connection·네트워크를 목표 부하와 장애범위에 맞게 배치하는 영역**에서 `WAR별 Pool·전체 DB Session`에 관한 공통 기준을 정하는 일이다. 이 기준을 정하지 않으면 팀마다 서로 다른 판단을 구현하고, 통합·보안·운영 단계에서 뒤늦게 충돌할 가능성이 높다.
...
|---|---|
| TASK ID | INF-07 |
| 의사결정 사항 | HikariCP Pool |
| 이번에 확정할 범위 | WAR별 Pool·전체 DB Session |
| 권고 초안 | Hikari Pool은 WAR별 필요량과 DB 전체 한도를 함께 산정한다. |
| 이행 방향 | Pool 합계가 DB Session 허용량을 넘지 않게 한다. |
| 주관 | TA |
| 협의 | DA·DBA·AA |
| 승인 | TA·DA |
| 우선순위 | P1 |
...
기술·인프라·용량는 **WAR·Tomcat·JVM·Thread·Connection·네트워크를 목표 부하와 장애범위에 맞게 배치하는 영역**이다.
...
- 의사결정 주제: `WAR별 Pool·전체 DB Session`
- 책임 경계: L4/Apache/Gateway → Tomcat/JVM/WAR → Thread/DB Pool → DB·외부 시스템과 DR
- 관련 구성요소: ztomcat, tcf-cicd, Context/Port, JVM/GC, Connector Thread, HikariCP, Health, 배포·Rollback, DR
- 설계·개발·시험·배포·운영 중 이 결정의 영향을 받는 산출물
- 기존 구현을 변경할 경우 호환성, 전환, Rollback과 폐기 계획

### 기본적으로 적용하지 않는 범위

- INF-07와 직접 관련 없는 코드 정리나 대규모 이름 변경
...
- 팀별로 `WAR별 Pool·전체 DB Session` 해석이 달라져 동일 기능이 여러 방식으로 구현된다.
...
대표 실패 유형은 **근거 없는 Pool 확대, 단일 JVM 장애 확산, Context 충돌, 환경값 하드코딩, Health와 준비상태 혼동**이다. 단기적으로 기능이 동작해도 운영 안정성, 변경 용이성, 감사 가능성과 장애 격리 능력이 약해질 수 있다.
...
| 순서 | 결정 질문 | 답변에 포함할 내용 |
|---:|---|---|
| 1 | `WAR별 Pool·전체 DB Session`의 단일 기준은 무엇인가? | 허용·권고·금지 규칙과 적용 대상 |
| 2 | 기준은 어느 경계에서 실행되는가? | L4/Apache/Gateway → Tomcat/JVM/WAR → Thread/DB Pool → DB·외부 시스템과 DR |
| 3 | 누가 작성·구현·검증·승인·운영하는가? | TA·DA·DBA·AA·TA·DA의 역할 분리 |
...
| 8 | 무엇으로 준수를 증명하는가? | TPS·p95·Stress·Soak·Failover·DR·배포·Rollback 시험과 Thread/Heap/GC/Pool/DB Session 합산 확인 |
| 9 | 완료로 인정할 최소 증적은 무엇인가? | DB Pool 산정서, 테스트, 자동검증, 운영 확인 |
...
**Hikari Pool은 WAR별 필요량과 DB 전체 한도를 함께 산정한다.**
...
**Pool 합계가 DB Session 허용량을 넘지 않게 한다.**

이행 결과는 `DB Pool 산정서` 하나만 제출하는 것으로 끝나지 않는다. 관련 소스·설정·테스트·자동검증·운영 절차가 같은 결정 ID로 추적되어야 한다.
...
요구사항·현행 문제
  ...
  → 대안과 영향도 비교
  → PoC 또는 대표 시나리오 검증
  → TA 권고안 작성 / DA·DBA·AA 협의
  → TA·DA 승인 및 ADR Baseline
  → 개발표준·공통 구현·샘플 반영
  → 테스트·CI/CD·운영 검증
  → 적용 확인·예외 추적·정기 재검토
```
...
```text
L4/Apache/Gateway → Tomcat/JVM/WAR → Thread/DB Pool → DB·외부 시스템과 DR
```
...
| 협의 DA·DBA·AA | 영역 요구와 부작용 검토, 테스트·운영 조건 제시 | 검토 의견, 영향도, 조건부 조치 |
| 승인 TA·DA | 선택·기각 근거, 잔여 위험과 예외 수용 여부 결정 | 승인 기록, ADR Baseline |
...
WAR별 Hikari Pool 합계가 DB Session 한도를 넘지 않도록 산정하고 장애 시 잔여 인스턴스가 목표 부하를 처리.
...
consulted: "DA·DBA·AA"
approver: "TA·DA"
decisionScope: "WAR별 Pool·전체 DB Session"
dueDate: "승인 회의에서 확정"
deliverable: "DB Pool 산정서"
```
...
implementation: "Pool 합계가 DB Session 허용량을 넘지 않게 한다."
alternatives:
  - 중앙 표준·공통 강제
  - 영역 자율·경계 계약
  - 승인된 제한 예외
...
| 구분 | 기록할 값 |
|---|---|
| 요구사항·TASK·ADR | 요구사항 ID, INF-07, ADR ID |
| 설계·가이드 | 문서 경로와 승인 버전 |
...
- TPS·p95·Stress·Soak·Failover·DR·배포·Rollback 시험과 Thread/Heap/GC/Pool/DB Session 합산 확인
...
- [ ] 처음 읽는 사람이 `WAR별 Pool·전체 DB Session`의 의미와 결정 이유를 설명할 수 있다.
...
- [ ] 주관 TA, 협의 DA·DBA·AA, 승인 TA·DA의 행동이 구분되어 있다.
- [ ] `DB Pool 산정서`의 작성자·검토자·기한이 있다.
...
INF-07의 목적은 **HikariCP Pool**에 관한 결정을 문서 한 장으로 끝내는 것이 아니라, `WAR별 Pool·전체 DB Session` 기준을 프로젝트 전체가 같은 방식으로 이해하고 실행·검증하게 만드는 것이다.

최종 승인 시에는 **Hikari Pool은 WAR별 필요량과 DB 전체 한도를 함께 산정한다.**이라는 방향을 구체적인 값과 경계로 확정하고, **Pool 합계가 DB Session 허용량을 넘지 않게 한다.**을 코드·설정·테스트·운영 증적으로 입증해야 한다. 그때 비로소 INF-07를 완료로 판정할 수 있다.

## 21. 참조 문서

- [zarchitecture/01-전체-시스템-아키텍처.md](../../../zarchitecture/01-전체-시스템-아키텍처.md)
- [zarchitecture/15-배포-환경-CICD-아키텍처.md](../../../zarchitecture/15-배포-환경-CICD-아키텍처.md)
- [ztomcat/README.md](../../../ztomcat/README.md)
- [zdocs-1/architecture/45-disaster-recovery.md](../../../zdocs-1/architecture/45-disaster-recovery.md)
