...
> 문서 성격: **이해·검토용 상세 초안**  
> 상태: **ARB 승인 전** · 영역: **기술·인프라·용량** · 우선순위: **P2**

## 1. 이 문서를 먼저 읽는 이유

이 문서는 요약본 [INF-18-DR.md](./INF-18-DR.md)의 내용을 처음 접하는 사람도 INF-18의 의미, 필요한 이유, 실제로 결정할 항목과 완료 판단 방법을 이해할 수 있도록 풀어서 설명한다.

INF-18의 핵심 주제는 **DR**이다. 쉽게 말하면 **WAR·Tomcat·JVM·Thread·Connection·네트워크를 목표 부하와 장애범위에 맞게 배치하는 영역**에서 `RTO·RPO·전환·복귀·데이터복구`에 관한 공통 기준을 정하는 일이다. 이 기준을 정하지 않으면 팀마다 서로 다른 판단을 구현하고, 통합·보안·운영 단계에서 뒤늦게 충돌할 가능성이 높다.
...
| TASK ID | INF-18 |
| 의사결정 사항 | DR |
| 이번에 확정할 범위 | RTO·RPO·전환·복귀·데이터복구 |
| 권고 초안 | DR은 RTO·RPO뿐 아니라 전환·복귀 절차까지 시험한다. |
| 이행 방향 | 선언된 DR과 실제 처리용량을 대조한다. |
| 주관 | TA |
| 협의 | DA·OPS·SA |
| 승인 | SA |
| 우선순위 | P2 |
...
- INF-18와 직접 관련 없는 코드 정리나 대규모 이름 변경
...
- 팀별로 `RTO·RPO·전환·복귀·데이터복구` 해석이 달라져 동일 기능이 여러 방식으로 구현된다.
...
| 순서 | 결정 질문 | 답변에 포함할 내용 |
|---:|---|---|
| 1 | `RTO·RPO·전환·복귀·데이터복구`의 단일 기준은 무엇인가? | 허용·권고·금지 규칙과 적용 대상 |
| 2 | 기준은 어느 경계에서 실행되는가? | L4/Apache/Gateway → Tomcat/JVM/WAR → Thread/DB Pool → DB·외부 시스템과 DR |
| 3 | 누가 작성·구현·검증·승인·운영하는가? | TA·DA·OPS·SA·SA의 역할 분리 |
...
| 9 | 완료로 인정할 최소 증적은 무엇인가? | DR 설계서, 테스트, 자동검증, 운영 확인 |
...
**DR은 RTO·RPO뿐 아니라 전환·복귀 절차까지 시험한다.**
...
**선언된 DR과 실제 처리용량을 대조한다.**

이행 결과는 `DR 설계서` 하나만 제출하는 것으로 끝나지 않는다. 관련 소스·설정·테스트·자동검증·운영 절차가 같은 결정 ID로 추적되어야 한다.
...
| 협의 DA·OPS·SA | 영역 요구와 부작용 검토, 테스트·운영 조건 제시 | 검토 의견, 영향도, 조건부 조치 |
...
consulted: "DA·OPS·SA"
approver: "SA"
decisionScope: "RTO·RPO·전환·복귀·데이터복구"
dueDate: "승인 회의에서 확정"
deliverable: "DR 설계서"
```
...
implementation: "선언된 DR과 실제 처리용량을 대조한다."
alternatives:
  - 중앙 표준·공통 강제
  - 영역 자율·경계 계약
  - 승인된 제한 예외
...
| 구분 | 기록할 값 |
|---|---|
| 요구사항·TASK·ADR | 요구사항 ID, INF-18, ADR ID |
| 설계·가이드 | 문서 경로와 승인 버전 |
...
- [ ] 처음 읽는 사람이 `RTO·RPO·전환·복귀·데이터복구`의 의미와 결정 이유를 설명할 수 있다.
...
- [ ] 주관 TA, 협의 DA·OPS·SA, 승인 SA의 행동이 구분되어 있다.
- [ ] `DR 설계서`의 작성자·검토자·기한이 있다.
...
INF-18의 목적은 **DR**에 관한 결정을 문서 한 장으로 끝내는 것이 아니라, `RTO·RPO·전환·복귀·데이터복구` 기준을 프로젝트 전체가 같은 방식으로 이해하고 실행·검증하게 만드는 것이다.

최종 승인 시에는 **DR은 RTO·RPO뿐 아니라 전환·복귀 절차까지 시험한다.**이라는 방향을 구체적인 값과 경계로 확정하고, **선언된 DR과 실제 처리용량을 대조한다.**을 코드·설정·테스트·운영 증적으로 입증해야 한다. 그때 비로소 INF-18를 완료로 판정할 수 있다.
