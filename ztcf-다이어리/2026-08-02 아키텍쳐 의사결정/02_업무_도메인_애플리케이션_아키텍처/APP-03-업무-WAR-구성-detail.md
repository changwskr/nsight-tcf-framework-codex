...
이 문서는 요약본 [APP-03-업무-WAR-구성.md](./APP-03-업무-WAR-구성.md)의 내용을 처음 접하는 사람도 APP-03의 의미, 필요한 이유, 실제로 결정할 항목과 완료 판단 방법을 이해할 수 있도록 풀어서 설명한다.

APP-03의 핵심 주제는 **업무 WAR 구성**이다. 쉽게 말하면 **업무 기능을 어느 WAR와 도메인에 배치하고 Handler부터 Mapper까지 책임을 어떻게 나눌지 정하는 영역**에서 `업무코드와 WAR 배포단위 관계`에 관한 공통 기준을 정하는 일이다. 이 기준을 정하지 않으면 팀마다 서로 다른 판단을 구현하고, 통합·보안·운영 단계에서 뒤늦게 충돌할 가능성이 높다.
...
| TASK ID | APP-03 |
| 의사결정 사항 | 업무 WAR 구성 |
| 이번에 확정할 범위 | 업무코드와 WAR 배포단위 관계 |
| 권고 초안 | 업무 WAR는 업무 책임과 장애격리 단위로 구성한다. |
| 이행 방향 | 단순 화면 수가 아니라 배포·부하·장애·조직을 기준으로 나눈다. |
| 주관 | AA |
| 협의 | TA·DVO |
| 승인 | SA |
| 우선순위 | P0 |
...
- APP-03와 직접 관련 없는 코드 정리나 대규모 이름 변경
...
- 팀별로 `업무코드와 WAR 배포단위 관계` 해석이 달라져 동일 기능이 여러 방식으로 구현된다.
...
| 순서 | 결정 질문 | 답변에 포함할 내용 |
|---:|---|---|
| 1 | `업무코드와 WAR 배포단위 관계`의 단일 기준은 무엇인가? | 허용·권고·금지 규칙과 적용 대상 |
| 2 | 기준은 어느 경계에서 실행되는가? | 공통 Controller/TCF → Handler → Facade → Service → Rule → DAO/Mapper와 WAR 간 계약 |
| 3 | 누가 작성·구현·검증·승인·운영하는가? | AA·TA·DVO·SA의 역할 분리 |
...
| 9 | 완료로 인정할 최소 증적은 무엇인가? | WAR 구성표, 테스트, 자동검증, 운영 확인 |
...
**단순 화면 수가 아니라 배포·부하·장애·조직을 기준으로 나눈다.**

이행 결과는 `WAR 구성표` 하나만 제출하는 것으로 끝나지 않는다. 관련 소스·설정·테스트·자동검증·운영 절차가 같은 결정 ID로 추적되어야 한다.
...
consulted: "TA·DVO"
approver: "SA"
decisionScope: "업무코드와 WAR 배포단위 관계"
dueDate: "승인 회의에서 확정"
deliverable: "WAR 구성표"
```
...
implementation: "단순 화면 수가 아니라 배포·부하·장애·조직을 기준으로 나눈다."
alternatives:
  - 중앙 표준·공통 강제
  - 영역 자율·경계 계약
  - 승인된 제한 예외
...
| 구분 | 기록할 값 |
|---|---|
| 요구사항·TASK·ADR | 요구사항 ID, APP-03, ADR ID |
| 설계·가이드 | 문서 경로와 승인 버전 |
...
- [ ] 처음 읽는 사람이 `업무코드와 WAR 배포단위 관계`의 의미와 결정 이유를 설명할 수 있다.
...
- [ ] 주관 AA, 협의 TA·DVO, 승인 SA의 행동이 구분되어 있다.
- [ ] `WAR 구성표`의 작성자·검토자·기한이 있다.
...
APP-03의 목적은 **업무 WAR 구성**에 관한 결정을 문서 한 장으로 끝내는 것이 아니라, `업무코드와 WAR 배포단위 관계` 기준을 프로젝트 전체가 같은 방식으로 이해하고 실행·검증하게 만드는 것이다.

최종 승인 시에는 **업무 WAR는 업무 책임과 장애격리 단위로 구성한다.**이라는 방향을 구체적인 값과 경계로 확정하고, **단순 화면 수가 아니라 배포·부하·장애·조직을 기준으로 나눈다.**을 코드·설정·테스트·운영 증적으로 입증해야 한다. 그때 비로소 APP-03를 완료로 판정할 수 있다.
