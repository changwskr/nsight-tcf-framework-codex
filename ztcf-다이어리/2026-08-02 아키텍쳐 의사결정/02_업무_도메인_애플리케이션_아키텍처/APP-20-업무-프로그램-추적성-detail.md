# APP-20 업무 프로그램 추적성 상세 설명서

> 문서 성격: **이해·검토용 상세 초안**  
> 상태: **ARB 승인 전** · 영역: **업무·도메인·애플리케이션** · 우선순위: **P1**

## 1. 이 문서를 먼저 읽는 이유

이 문서는 요약본 [APP-20-업무-프로그램-추적성.md](./APP-20-업무-프로그램-추적성.md)의 내용을 처음 접하는 사람도 APP-20의 의미, 필요한 이유, 실제로 결정할 항목과 완료 판단 방법을 이해할 수 있도록 풀어서 설명한다.

APP-20의 핵심 주제는 **업무 프로그램 추적성**이다. 쉽게 말하면 **업무 기능을 어느 WAR와 도메인에 배치하고 Handler부터 Mapper까지 책임을 어떻게 나눌지 정하는 영역**에서 `화면–ServiceId–프로그램–SQL 연결`에 관한 공통 기준을 정하는 일이다. 이 기준을 정하지 않으면 팀마다 서로 다른 판단을 구현하고, 통합·보안·운영 단계에서 뒤늦게 충돌할 가능성이 높다.
...
| TASK ID | APP-20 |
| 의사결정 사항 | 업무 프로그램 추적성 |
| 이번에 확정할 범위 | 화면–ServiceId–프로그램–SQL 연결 |
| 권고 초안 | 화면부터 DB까지 양방향 추적성을 관리한다. |
| 이행 방향 | 화면 이벤트–ServiceId–Handler–SQL–Table 매트릭스를 유지한다. |
| 주관 | AA |
| 협의 | UI·DA·QA |
| 승인 | SA |
| 우선순위 | P1 |
...
- 의사결정 주제: `화면–ServiceId–프로그램–SQL 연결`
...
- APP-20와 직접 관련 없는 코드 정리나 대규모 이름 변경
...
- 팀별로 `화면–ServiceId–프로그램–SQL 연결` 해석이 달라져 동일 기능이 여러 방식으로 구현된다.
...
| 순서 | 결정 질문 | 답변에 포함할 내용 |
|---:|---|---|
| 1 | `화면–ServiceId–프로그램–SQL 연결`의 단일 기준은 무엇인가? | 허용·권고·금지 규칙과 적용 대상 |
| 2 | 기준은 어느 경계에서 실행되는가? | 공통 Controller/TCF → Handler → Facade → Service → Rule → DAO/Mapper와 WAR 간 계약 |
| 3 | 누가 작성·구현·검증·승인·운영하는가? | AA·UI·DA·QA·SA의 역할 분리 |
...
| 9 | 완료로 인정할 최소 증적은 무엇인가? | 추적성 매트릭스, 테스트, 자동검증, 운영 확인 |
...
**화면 이벤트–ServiceId–Handler–SQL–Table 매트릭스를 유지한다.**

이행 결과는 `추적성 매트릭스` 하나만 제출하는 것으로 끝나지 않는다. 관련 소스·설정·테스트·자동검증·운영 절차가 같은 결정 ID로 추적되어야 한다.
...
| 협의 UI·DA·QA | 영역 요구와 부작용 검토, 테스트·운영 조건 제시 | 검토 의견, 영향도, 조건부 조치 |
...
consulted: "UI·DA·QA"
approver: "SA"
decisionScope: "화면–ServiceId–프로그램–SQL 연결"
dueDate: "승인 회의에서 확정"
deliverable: "추적성 매트릭스"
```
...
implementation: "화면 이벤트–ServiceId–Handler–SQL–Table 매트릭스를 유지한다."
alternatives:
  - 중앙 표준·공통 강제
  - 영역 자율·경계 계약
  - 승인된 제한 예외
...
| 구분 | 기록할 값 |
|---|---|
| 요구사항·TASK·ADR | 요구사항 ID, APP-20, ADR ID |
| 설계·가이드 | 문서 경로와 승인 버전 |
...
- [ ] 처음 읽는 사람이 `화면–ServiceId–프로그램–SQL 연결`의 의미와 결정 이유를 설명할 수 있다.
...
- [ ] 주관 AA, 협의 UI·DA·QA, 승인 SA의 행동이 구분되어 있다.
- [ ] `추적성 매트릭스`의 작성자·검토자·기한이 있다.
...
APP-20의 목적은 **업무 프로그램 추적성**에 관한 결정을 문서 한 장으로 끝내는 것이 아니라, `화면–ServiceId–프로그램–SQL 연결` 기준을 프로젝트 전체가 같은 방식으로 이해하고 실행·검증하게 만드는 것이다.

최종 승인 시에는 **화면부터 DB까지 양방향 추적성을 관리한다.**이라는 방향을 구체적인 값과 경계로 확정하고, **화면 이벤트–ServiceId–Handler–SQL–Table 매트릭스를 유지한다.**을 코드·설정·테스트·운영 증적으로 입증해야 한다. 그때 비로소 APP-20를 완료로 판정할 수 있다.
