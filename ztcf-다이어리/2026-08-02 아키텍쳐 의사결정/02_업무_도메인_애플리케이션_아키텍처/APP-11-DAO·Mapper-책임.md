# APP-11 DAO·Mapper 책임

> 상태: **결정 전 설명서 초안**  
> 영역: 업무·도메인·애플리케이션 · 우선순위: **P0** · 주관: **DA** · 승인: **SA**

## 1. 도입 전 안내말

이 문서는 APP-11 아키텍처 의사결정을 검토·승인·이행하기 위한 설명서다. 현재 저장소의 구현과 참조 문서를 근거로 작성했으며, ARB 승인 전까지는 권고 초안으로 취급한다.

## 2. 문서 개요

| 항목 | 내용 |
|---|---|
| 목적 | DAO·Mapper 책임 기준을 프로젝트 단일 원칙으로 확정한다. |
| 결정 범위 | SQL 실행과 업무로직 분리 |
| 적용 대상 | NSIGHT TCF 프레임워크, 플랫폼 모듈, 업무 WAR와 관련 운영 절차 |
| 대상 독자 | DA, DBA·AA, SA 및 구현·시험·운영 담당자 |
| 선행조건 | 현행 코드·설정·운영 기준 확인, 영향 대상 식별, 책임자 지정 |
| 주요 산출물 | DAO·Mapper 기준 |
...
1. APP-11를 결정 원장에 등록하고 영향 범위를 식별한다.
...
APP-11 결정, 관련 코드·설정, DAO·Mapper 기준, 자동검증 결과와 운영 확인 내역이 동일 변경 단위로 추적된다.
...
APP-11는 문서 승인만으로 완료되지 않는다. DAO·Mapper 기준, 구현·설정 반영, 테스트, 자동검증과 업무팀 적용 확인까지 충족해야 완료로 판정한다.
