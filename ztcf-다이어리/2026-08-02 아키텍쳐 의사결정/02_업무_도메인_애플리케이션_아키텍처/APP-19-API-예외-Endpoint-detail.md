# APP-19 API 예외 Endpoint 상세 설명서

> 문서 성격: **이해·검토용 상세 초안**  
> 상태: **ARB 승인 전** · 영역: **업무·도메인·애플리케이션** · 우선순위: **P0**

## 1. 이 문서를 먼저 읽는 이유

이 문서는 요약본 [APP-19-API-예외-Endpoint.md](./APP-19-API-예외-Endpoint.md)의 내용을 처음 접하는 사람도 APP-19의 의미, 필요한 이유, 실제로 결정할 항목과 완료 판단 방법을 이해할 수 있도록 풀어서 설명한다.

APP-19의 핵심 주제는 **API 예외 Endpoint**이다. 쉽게 말하면 **업무 기능을 어느 WAR와 도메인에 배치하고 Handler부터 Mapper까지 책임을 어떻게 나눌지 정하는 영역**에서 `Health·파일·인증 등 TCF 비경유 대상`에 관한 공통 기준을 정하는 일이다. 이 기준을 정하지 않으면 팀마다 서로 다른 판단을 구현하고, 통합·보안·운영 단계에서 뒤늦게 충돌할 가능성이 높다.
...
|---|---|
| TASK ID | APP-19 |
| 의사결정 사항 | API 예외 Endpoint |
| 이번에 확정할 범위 | Health·파일·인증 등 TCF 비경유 대상 |
| 권고 초안 | TCF 비경유 Endpoint는 White List로 관리한다. |
| 이행 방향 | Health, JWKS, 로그인, 제한된 파일 API만 예외로 허용한다. |
| 주관 | AA |
| 협의 | FW·SEC |
| 승인 | SA |
| 우선순위 | P0 |
| 필수 산출물 | 예외 Endpoint 목록 |

## 3. 용어와 배경지식

### 3.1 영역의 의미

업무·도메인·애플리케이션는 **업무 기능을 어느 WAR와 도메인에 배치하고 Handler부터 Mapper까지 책임을 어떻게 나눌지 정하는 영역**이다.
...
- 의사결정 주제: `Health·파일·인증 등 TCF 비경유 대상`
- 책임 경계: 공통 Controller/TCF → Handler → Facade → Service → Rule → DAO/Mapper와 WAR 간 계약
- 관련 구성요소: tcf-core, tcf-web, 업무 WAR, TransactionHandler, Facade, Service, Rule, DAO, Mapper, DTO
- 설계·개발·시험·배포·운영 중 이 결정의 영향을 받는 산출물
- 기존 구현을 변경할 경우 호환성, 전환, Rollback과 폐기 계획

### 기본적으로 적용하지 않는 범위

- APP-19와 직접 관련 없는 코드 정리나 대규모 이름 변경
...
- 팀별로 `Health·파일·인증 등 TCF 비경유 대상` 해석이 달라져 동일 기능이 여러 방식으로 구현된다.
...
대표 실패 유형은 **TCF 우회, Handler의 업무로직·SQL 포함, Facade 밖 트랜잭션 혼재, 다른 WAR 코드·DB 직접참조, 순환 의존**이다. 단기적으로 기능이 동작해도 운영 안정성, 변경 용이성, 감사 가능성과 장애 격리 능력이 약해질 수 있다.
...
| 순서 | 결정 질문 | 답변에 포함할 내용 |
|---:|---|---|
| 1 | `Health·파일·인증 등 TCF 비경유 대상`의 단일 기준은 무엇인가? | 허용·권고·금지 규칙과 적용 대상 |
| 2 | 기준은 어느 경계에서 실행되는가? | 공통 Controller/TCF → Handler → Facade → Service → Rule → DAO/Mapper와 WAR 간 계약 |
| 3 | 누가 작성·구현·검증·승인·운영하는가? | AA·FW·SEC·SA의 역할 분리 |
...
| 8 | 무엇으로 준수를 증명하는가? | ArchUnit 계층검사, Handler serviceIds 중복검사, 업무 WAR 빌드, 정상·업무오류·Rollback 통합시험 |
| 9 | 완료로 인정할 최소 증적은 무엇인가? | 예외 Endpoint 목록, 테스트, 자동검증, 운영 확인 |
...
**TCF 비경유 Endpoint는 White List로 관리한다.**
...
**Health, JWKS, 로그인, 제한된 파일 API만 예외로 허용한다.**

이행 결과는 `예외 Endpoint 목록` 하나만 제출하는 것으로 끝나지 않는다. 관련 소스·설정·테스트·자동검증·운영 절차가 같은 결정 ID로 추적되어야 한다.
...
```text
공통 Controller/TCF → Handler → Facade → Service → Rule → DAO/Mapper와 WAR 간 계약
```
...
| 협의 FW·SEC | 영역 요구와 부작용 검토, 테스트·운영 조건 제시 | 검토 의견, 영향도, 조건부 조치 |
...
SV.Customer.selectSummary 요청이 공통 /sv/online에서 TCF를 거쳐 CustomerHandler가 Facade를 호출하고 Mapper가 Bind SQL만 실행.
...
```yaml
taskId: APP-19
title: "API 예외 Endpoint"
status: 분석-중
priority: P0
owner: AA
consulted: "FW·SEC"
approver: "SA"
decisionScope: "Health·파일·인증 등 TCF 비경유 대상"
dueDate: "승인 회의에서 확정"
deliverable: "예외 Endpoint 목록"
```
...
implementation: "Health, JWKS, 로그인, 제한된 파일 API만 예외로 허용한다."
alternatives:
  - 중앙 표준·공통 강제
  - 영역 자율·경계 계약
  - 승인된 제한 예외
...
| 구분 | 기록할 값 |
|---|---|
| 요구사항·TASK·ADR | 요구사항 ID, APP-19, ADR ID |
| 설계·가이드 | 문서 경로와 승인 버전 |
...
- ArchUnit 계층검사, Handler serviceIds 중복검사, 업무 WAR 빌드, 정상·업무오류·Rollback 통합시험
...
- [ ] 처음 읽는 사람이 `Health·파일·인증 등 TCF 비경유 대상`의 의미와 결정 이유를 설명할 수 있다.
...
- [ ] 주관 AA, 협의 FW·SEC, 승인 SA의 행동이 구분되어 있다.
- [ ] `예외 Endpoint 목록`의 작성자·검토자·기한이 있다.
...
APP-19의 목적은 **API 예외 Endpoint**에 관한 결정을 문서 한 장으로 끝내는 것이 아니라, `Health·파일·인증 등 TCF 비경유 대상` 기준을 프로젝트 전체가 같은 방식으로 이해하고 실행·검증하게 만드는 것이다.

최종 승인 시에는 **TCF 비경유 Endpoint는 White List로 관리한다.**이라는 방향을 구체적인 값과 경계로 확정하고, **Health, JWKS, 로그인, 제한된 파일 API만 예외로 허용한다.**을 코드·설정·테스트·운영 증적으로 입증해야 한다. 그때 비로소 APP-19를 완료로 판정할 수 있다.
...
- [zdocs-1/architecture/01-application-layer.md](../../../zdocs-1/architecture/01-application-layer.md)
