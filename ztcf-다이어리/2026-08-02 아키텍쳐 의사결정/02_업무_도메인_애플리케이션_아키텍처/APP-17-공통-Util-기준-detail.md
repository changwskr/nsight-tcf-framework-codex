...
이 문서는 요약본 [APP-17-공통-Util-기준.md](./APP-17-공통-Util-기준.md)의 내용을 처음 접하는 사람도 APP-17의 의미, 필요한 이유, 실제로 결정할 항목과 완료 판단 방법을 이해할 수 있도록 풀어서 설명한다.

APP-17의 핵심 주제는 **공통 Util 기준**이다. 쉽게 말하면 **업무 기능을 어느 WAR와 도메인에 배치하고 Handler부터 Mapper까지 책임을 어떻게 나눌지 정하는 영역**에서 `공통화 조건·업무로직 유입 방지`에 관한 공통 기준을 정하는 일이다. 이 기준을 정하지 않으면 팀마다 서로 다른 판단을 구현하고, 통합·보안·운영 단계에서 뒤늦게 충돌할 가능성이 높다.
...
| TASK ID | APP-17 |
| 의사결정 사항 | 공통 Util 기준 |
| 이번에 확정할 범위 | 공통화 조건·업무로직 유입 방지 |
| 권고 초안 | 공통 Util은 기술 공통 기능만 수용한다. |
| 이행 방향 | 업무규칙이나 특정 도메인 DTO의 공통 모듈 유입을 금지한다. |
| 주관 | FW |
| 협의 | AA·DEV |
| 승인 | AA |
| 우선순위 | P1 |
...
- APP-17와 직접 관련 없는 코드 정리나 대규모 이름 변경
...
- 팀별로 `공통화 조건·업무로직 유입 방지` 해석이 달라져 동일 기능이 여러 방식으로 구현된다.
...
| 순서 | 결정 질문 | 답변에 포함할 내용 |
|---:|---|---|
| 1 | `공통화 조건·업무로직 유입 방지`의 단일 기준은 무엇인가? | 허용·권고·금지 규칙과 적용 대상 |
...
| 9 | 완료로 인정할 최소 증적은 무엇인가? | 공통화 심의기준, 테스트, 자동검증, 운영 확인 |
...
**업무규칙이나 특정 도메인 DTO의 공통 모듈 유입을 금지한다.**

이행 결과는 `공통화 심의기준` 하나만 제출하는 것으로 끝나지 않는다. 관련 소스·설정·테스트·자동검증·운영 절차가 같은 결정 ID로 추적되어야 한다.
...
consulted: "AA·DEV"
approver: "AA"
decisionScope: "공통화 조건·업무로직 유입 방지"
dueDate: "승인 회의에서 확정"
deliverable: "공통화 심의기준"
```
...
implementation: "업무규칙이나 특정 도메인 DTO의 공통 모듈 유입을 금지한다."
alternatives:
  - 중앙 표준·공통 강제
  - 영역 자율·경계 계약
  - 승인된 제한 예외
...
| 구분 | 기록할 값 |
|---|---|
| 요구사항·TASK·ADR | 요구사항 ID, APP-17, ADR ID |
| 설계·가이드 | 문서 경로와 승인 버전 |
...
- [ ] 처음 읽는 사람이 `공통화 조건·업무로직 유입 방지`의 의미와 결정 이유를 설명할 수 있다.
...
APP-17의 목적은 **공통 Util 기준**에 관한 결정을 문서 한 장으로 끝내는 것이 아니라, `공통화 조건·업무로직 유입 방지` 기준을 프로젝트 전체가 같은 방식으로 이해하고 실행·검증하게 만드는 것이다.

최종 승인 시에는 **공통 Util은 기술 공통 기능만 수용한다.**이라는 방향을 구체적인 값과 경계로 확정하고, **업무규칙이나 특정 도메인 DTO의 공통 모듈 유입을 금지한다.**을 코드·설정·테스트·운영 증적으로 입증해야 한다. 그때 비로소 APP-17를 완료로 판정할 수 있다.
