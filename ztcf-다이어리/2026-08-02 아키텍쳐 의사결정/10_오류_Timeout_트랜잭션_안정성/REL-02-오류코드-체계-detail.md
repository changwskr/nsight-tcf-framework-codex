...
이 문서는 요약본 [REL-02-오류코드-체계.md](./REL-02-오류코드-체계.md)의 내용을 처음 접하는 사람도 REL-02의 의미, 필요한 이유, 실제로 결정할 항목과 완료 판단 방법을 이해할 수 있도록 풀어서 설명한다.

REL-02의 핵심 주제는 **오류코드 체계**이다. 쉽게 말하면 **실패가 발생해도 응답·데이터·외부 처리 상태가 예측 가능하고 복구 가능하도록 정하는 영역**에서 `공통·업무·외부·DB 오류영역`에 관한 공통 기준을 정하는 일이다. 이 기준을 정하지 않으면 팀마다 서로 다른 판단을 구현하고, 통합·보안·운영 단계에서 뒤늦게 충돌할 가능성이 높다.
...
| TASK ID | REL-02 |
| 의사결정 사항 | 오류코드 체계 |
| 이번에 확정할 범위 | 공통·업무·외부·DB 오류영역 |
| 권고 초안 | 오류코드에 영역·업무·유형·순번을 포함한다. |
| 이행 방향 | 중복과 임의 코드를 중앙 관리한다. |
| 주관 | FW |
| 협의 | AA·BA |
| 승인 | AA |
| 우선순위 | P0 |
...
- REL-02와 직접 관련 없는 코드 정리나 대규모 이름 변경
...
- 팀별로 `공통·업무·외부·DB 오류영역` 해석이 달라져 동일 기능이 여러 방식으로 구현된다.
...
| 9 | 완료로 인정할 최소 증적은 무엇인가? | 오류코드 규칙, 테스트, 자동검증, 운영 확인 |
...
이행 결과는 `오류코드 규칙` 하나만 제출하는 것으로 끝나지 않는다. 관련 소스·설정·테스트·자동검증·운영 절차가 같은 결정 ID로 추적되어야 한다.
...
| 협의 AA·BA | 영역 요구와 부작용 검토, 테스트·운영 조건 제시 | 검토 의견, 영향도, 조건부 조치 |
...
consulted: "AA·BA"
approver: "AA"
decisionScope: "공통·업무·외부·DB 오류영역"
dueDate: "승인 회의에서 확정"
deliverable: "오류코드 규칙"
```
...
implementation: "중복과 임의 코드를 중앙 관리한다."
alternatives:
  - 중앙 표준·공통 강제
  - 영역 자율·경계 계약
  - 승인된 제한 예외
...
| 구분 | 기록할 값 |
|---|---|
| 요구사항·TASK·ADR | 요구사항 ID, REL-02, ADR ID |
| 설계·가이드 | 문서 경로와 승인 버전 |
...
- [ ] 처음 읽는 사람이 `공통·업무·외부·DB 오류영역`의 의미와 결정 이유를 설명할 수 있다.
...
- [ ] 주관 FW, 협의 AA·BA, 승인 AA의 행동이 구분되어 있다.
- [ ] `오류코드 규칙`의 작성자·검토자·기한이 있다.
...
REL-02의 목적은 **오류코드 체계**에 관한 결정을 문서 한 장으로 끝내는 것이 아니라, `공통·업무·외부·DB 오류영역` 기준을 프로젝트 전체가 같은 방식으로 이해하고 실행·검증하게 만드는 것이다.

최종 승인 시에는 **오류코드에 영역·업무·유형·순번을 포함한다.**이라는 방향을 구체적인 값과 경계로 확정하고, **중복과 임의 코드를 중앙 관리한다.**을 코드·설정·테스트·운영 증적으로 입증해야 한다. 그때 비로소 REL-02를 완료로 판정할 수 있다.
