# REL-07 DB Timeout 상세 설명서

> 문서 성격: **이해·검토용 상세 초안**  
> 상태: **ARB 승인 전** · 영역: **오류·Timeout·트랜잭션·안정성** · 우선순위: **P0**

## 1. 이 문서를 먼저 읽는 이유

이 문서는 요약본 [REL-07-DB-Timeout.md](./REL-07-DB-Timeout.md)의 내용을 처음 접하는 사람도 REL-07의 의미, 필요한 이유, 실제로 결정할 항목과 완료 판단 방법을 이해할 수 있도록 풀어서 설명한다.

REL-07의 핵심 주제는 **DB Timeout**이다. 쉽게 말하면 **실패가 발생해도 응답·데이터·외부 처리 상태가 예측 가능하고 복구 가능하도록 정하는 영역**에서 `Query·Lock·Connection Timeout`에 관한 공통 기준을 정하는 일이다. 이 기준을 정하지 않으면 팀마다 서로 다른 판단을 구현하고, 통합·보안·운영 단계에서 뒤늦게 충돌할 가능성이 높다.
...
| TASK ID | REL-07 |
| 의사결정 사항 | DB Timeout |
| 이번에 확정할 범위 | Query·Lock·Connection Timeout |
| 권고 초안 | Query·Lock·Connection Timeout을 분리한다. |
| 이행 방향 | JDBC Driver와 Hikari 설정의 관계를 검증한다. |
| 주관 | DBA |
| 협의 | DA·AA·TA |
| 승인 | DA |
| 우선순위 | P0 |
| 필수 산출물 | DB Timeout 기준 |

## 3. 용어와 배경지식

### 3.1 영역의 의미

오류·Timeout·트랜잭션·안정성는 **실패가 발생해도 응답·데이터·외부 처리 상태가 예측 가능하고 복구 가능하도록 정하는 영역**이다.
...
- 의사결정 주제: `Query·Lock·Connection Timeout`
- 책임 경계: UI/MCA Timeout → 온라인 Timeout → Transaction Timeout → DB Query/외부호출 Timeout과 ETF 오류 응답
- 관련 구성요소: ErrorCode, BusinessException, ETF, Timeout Policy, @Transactional, 멱등성, 재시도, 보상·재처리
- 설계·개발·시험·배포·운영 중 이 결정의 영향을 받는 산출물
- 기존 구현을 변경할 경우 호환성, 전환, Rollback과 폐기 계획

### 기본적으로 적용하지 않는 범위

- REL-07와 직접 관련 없는 코드 정리나 대규모 이름 변경
...
- 팀별로 `Query·Lock·Connection Timeout` 해석이 달라져 동일 기능이 여러 방식으로 구현된다.
...
대표 실패 유형은 **상위 Timeout 후 하위 처리 지속, Checked Exception 미Rollback, 비멱등 변경 재시도, 내부 예외 노출**이다. 단기적으로 기능이 동작해도 운영 안정성, 변경 용이성, 감사 가능성과 장애 격리 능력이 약해질 수 있다.
...
| 순서 | 결정 질문 | 답변에 포함할 내용 |
|---:|---|---|
| 1 | `Query·Lock·Connection Timeout`의 단일 기준은 무엇인가? | 허용·권고·금지 규칙과 적용 대상 |
| 2 | 기준은 어느 경계에서 실행되는가? | UI/MCA Timeout → 온라인 Timeout → Transaction Timeout → DB Query/외부호출 Timeout과 ETF 오류 응답 |
| 3 | 누가 작성·구현·검증·승인·운영하는가? | DBA·DA·AA·TA·DA의 역할 분리 |
...
| 8 | 무엇으로 준수를 증명하는가? | 정상·업무오류·시스템오류·Timeout·Rollback·중복·부분성공·재처리 시나리오와 최종 상태 대조 |
| 9 | 완료로 인정할 최소 증적은 무엇인가? | DB Timeout 기준, 테스트, 자동검증, 운영 확인 |
...
**Query·Lock·Connection Timeout을 분리한다.**
...
**JDBC Driver와 Hikari 설정의 관계를 검증한다.**

이행 결과는 `DB Timeout 기준` 하나만 제출하는 것으로 끝나지 않는다. 관련 소스·설정·테스트·자동검증·운영 절차가 같은 결정 ID로 추적되어야 한다.
...
요구사항·현행 문제
  ...
  → 대안과 영향도 비교
  → PoC 또는 대표 시나리오 검증
  → DBA 권고안 작성 / DA·AA·TA 협의
  → DA 승인 및 ADR Baseline
  → 개발표준·공통 구현·샘플 반영
  → 테스트·CI/CD·운영 검증
  → 적용 확인·예외 추적·정기 재검토
```
...
```text
UI/MCA Timeout → 온라인 Timeout → Transaction Timeout → DB Query/외부호출 Timeout과 ETF 오류 응답
```
...
| 역할 | 수행 내용 | 제출 증적 |
|---|---|---|
| 주관 DBA | 현행·문제·대안·권고안 작성, 영향 대상 조정 | TASK 분석서, 권고안, ADR 초안 |
| 협의 DA·AA·TA | 영역 요구와 부작용 검토, 테스트·운영 조건 제시 | 검토 의견, 영향도, 조건부 조치 |
| 승인 DA | 선택·기각 근거, 잔여 위험과 예외 수용 여부 결정 | 승인 기록, ADR Baseline |
...
온라인 Timeout보다 DB Query Timeout을 짧게 두고 실패 시 ETF가 표준 오류를 만들며 변경 데이터는 Rollback.
...
consulted: "DA·AA·TA"
approver: "DA"
decisionScope: "Query·Lock·Connection Timeout"
dueDate: "승인 회의에서 확정"
deliverable: "DB Timeout 기준"
```
...
implementation: "JDBC Driver와 Hikari 설정의 관계를 검증한다."
alternatives:
  - 중앙 표준·공통 강제
  - 영역 자율·경계 계약
  - 승인된 제한 예외
...
| 구분 | 기록할 값 |
|---|---|
| 요구사항·TASK·ADR | 요구사항 ID, REL-07, ADR ID |
| 설계·가이드 | 문서 경로와 승인 버전 |
...
- 정상·업무오류·시스템오류·Timeout·Rollback·중복·부분성공·재처리 시나리오와 최종 상태 대조
...
- [ ] 처음 읽는 사람이 `Query·Lock·Connection Timeout`의 의미와 결정 이유를 설명할 수 있다.
...
- [ ] 주관 DBA, 협의 DA·AA·TA, 승인 DA의 행동이 구분되어 있다.
- [ ] `DB Timeout 기준`의 작성자·검토자·기한이 있다.
...
REL-07의 목적은 **DB Timeout**에 관한 결정을 문서 한 장으로 끝내는 것이 아니라, `Query·Lock·Connection Timeout` 기준을 프로젝트 전체가 같은 방식으로 이해하고 실행·검증하게 만드는 것이다.

최종 승인 시에는 **Query·Lock·Connection Timeout을 분리한다.**이라는 방향을 구체적인 값과 경계로 확정하고, **JDBC Driver와 Hikari 설정의 관계를 검증한다.**을 코드·설정·테스트·운영 증적으로 입증해야 한다. 그때 비로소 REL-07를 완료로 판정할 수 있다.

## 21. 참조 문서

- [zarchitecture/10-거래통제-Timeout-로깅-아키텍처.md](../../../zarchitecture/10-거래통제-Timeout-로깅-아키텍처.md)
- [zdocs-1/architecture/03-transaction.md](../../../zdocs-1/architecture/03-transaction.md)
- [zdocs-1/architecture/05-exception.md](../../../zdocs-1/architecture/05-exception.md)
- [zdocs-1/architecture/41-service-timeout-policy.md](../../../zdocs-1/architecture/41-service-timeout-policy.md)
