...
이 문서는 요약본 [REL-15-사용자-재처리.md](./REL-15-사용자-재처리.md)의 내용을 처음 접하는 사람도 REL-15의 의미, 필요한 이유, 실제로 결정할 항목과 완료 판단 방법을 이해할 수 있도록 풀어서 설명한다.

REL-15의 핵심 주제는 **사용자 재처리**이다. 쉽게 말하면 **실패가 발생해도 응답·데이터·외부 처리 상태가 예측 가능하고 복구 가능하도록 정하는 영역**에서 `재조회·재요청·상태확인 UX`에 관한 공통 기준을 정하는 일이다. 이 기준을 정하지 않으면 팀마다 서로 다른 판단을 구현하고, 통합·보안·운영 단계에서 뒤늦게 충돌할 가능성이 높다.
...
| TASK ID | REL-15 |
| 의사결정 사항 | 사용자 재처리 |
| 이번에 확정할 범위 | 재조회·재요청·상태확인 UX |
| 권고 초안 | 사용자에게 상태조회와 안전한 재처리 경로를 제공한다. |
| 이행 방향 | Timeout 후 무조건 재호출하도록 안내하지 않는다. |
| 주관 | BA |
| 협의 | UI·AA·OPS |
| 승인 | AA |
| 우선순위 | P1 |
...
- REL-15와 직접 관련 없는 코드 정리나 대규모 이름 변경
...
- 팀별로 `재조회·재요청·상태확인 UX` 해석이 달라져 동일 기능이 여러 방식으로 구현된다.
...
| 9 | 완료로 인정할 최소 증적은 무엇인가? | 사용자 대응기준, 테스트, 자동검증, 운영 확인 |
...
**Timeout 후 무조건 재호출하도록 안내하지 않는다.**

이행 결과는 `사용자 대응기준` 하나만 제출하는 것으로 끝나지 않는다. 관련 소스·설정·테스트·자동검증·운영 절차가 같은 결정 ID로 추적되어야 한다.
...
| 역할 | 수행 내용 | 제출 증적 |
|---|---|---|
| 주관 BA | 현행·문제·대안·권고안 작성, 영향 대상 조정 | TASK 분석서, 권고안, ADR 초안 |
| 협의 UI·AA·OPS | 영역 요구와 부작용 검토, 테스트·운영 조건 제시 | 검토 의견, 영향도, 조건부 조치 |
...
consulted: "UI·AA·OPS"
approver: "AA"
decisionScope: "재조회·재요청·상태확인 UX"
dueDate: "승인 회의에서 확정"
deliverable: "사용자 대응기준"
```
...
implementation: "Timeout 후 무조건 재호출하도록 안내하지 않는다."
alternatives:
  - 중앙 표준·공통 강제
  - 영역 자율·경계 계약
  - 승인된 제한 예외
...
| 구분 | 기록할 값 |
|---|---|
| 요구사항·TASK·ADR | 요구사항 ID, REL-15, ADR ID |
| 설계·가이드 | 문서 경로와 승인 버전 |
...
- [ ] 처음 읽는 사람이 `재조회·재요청·상태확인 UX`의 의미와 결정 이유를 설명할 수 있다.
...
- [ ] 주관 BA, 협의 UI·AA·OPS, 승인 AA의 행동이 구분되어 있다.
- [ ] `사용자 대응기준`의 작성자·검토자·기한이 있다.
...
REL-15의 목적은 **사용자 재처리**에 관한 결정을 문서 한 장으로 끝내는 것이 아니라, `재조회·재요청·상태확인 UX` 기준을 프로젝트 전체가 같은 방식으로 이해하고 실행·검증하게 만드는 것이다.

최종 승인 시에는 **사용자에게 상태조회와 안전한 재처리 경로를 제공한다.**이라는 방향을 구체적인 값과 경계로 확정하고, **Timeout 후 무조건 재호출하도록 안내하지 않는다.**을 코드·설정·테스트·운영 증적으로 입증해야 한다. 그때 비로소 REL-15를 완료로 판정할 수 있다.
