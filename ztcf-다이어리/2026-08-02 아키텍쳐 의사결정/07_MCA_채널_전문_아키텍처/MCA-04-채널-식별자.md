...
이 문서는 MCA-04 아키텍처 의사결정을 검토·승인·이행하기 위한 설명서다. 현재 저장소의 구현과 참조 문서를 근거로 작성했으며, ARB 승인 전까지는 권고 초안으로 취급한다.

## 2. 문서 개요

| 항목 | 내용 |
|---|---|
| 목적 | 채널 식별자 기준을 프로젝트 단일 원칙으로 확정한다. |
| 결정 범위 | channelId·terminalId·deviceId |
| 적용 대상 | NSIGHT TCF 프레임워크, 플랫폼 모듈, 업무 WAR와 관련 운영 절차 |
| 대상 독자 | MCA, SEC·UI, AA 및 구현·시험·운영 담당자 |
...
- 요구사항: channelId·terminalId·deviceId
...
1. MCA-04를 결정 원장에 등록하고 영향 범위를 식별한다.
...
MCA-04 결정, 관련 코드·설정, 채널 식별기준, 자동검증 결과와 운영 확인 내역이 동일 변경 단위로 추적된다.
...
MCA-04는 문서 승인만으로 완료되지 않는다. 채널 식별기준, 구현·설정 반영, 테스트, 자동검증과 업무팀 적용 확인까지 충족해야 완료로 판정한다.
