...
이 문서는 요약본 [MCA-07-채널-라우팅.md](./MCA-07-채널-라우팅.md)의 내용을 처음 접하는 사람도 MCA-07의 의미, 필요한 이유, 실제로 결정할 항목과 완료 판단 방법을 이해할 수 있도록 풀어서 설명한다.

MCA-07의 핵심 주제는 **채널 라우팅**이다. 쉽게 말하면 **서로 다른 단말과 서버가 같은 요청·응답 의미를 공유하도록 전문 구조·필드·버전·라우팅을 정하는 영역**에서 `업무코드·Context·ServiceId 기준`에 관한 공통 기준을 정하는 일이다. 이 기준을 정하지 않으면 팀마다 서로 다른 판단을 구현하고, 통합·보안·운영 단계에서 뒤늦게 충돌할 가능성이 높다.
...
| TASK ID | MCA-07 |
| 의사결정 사항 | 채널 라우팅 |
| 이번에 확정할 범위 | 업무코드·Context·ServiceId 기준 |
| 권고 초안 | URL과 업무코드 기준 라우팅을 표준화한다. |
| 이행 방향 | `/sv`, `/ic` 등 Context와 업무코드를 일치시킨다. |
| 주관 | MCA |
| 협의 | TA·AA |
| 승인 | SA |
| 우선순위 | P0 |
...
- 의사결정 주제: `업무코드·Context·ServiceId 기준`
...
- MCA-07와 직접 관련 없는 코드 정리나 대규모 이름 변경
...
- 팀별로 `업무코드·Context·ServiceId 기준` 해석이 달라져 동일 기능이 여러 방식으로 구현된다.
...
| 순서 | 결정 질문 | 답변에 포함할 내용 |
|---:|---|---|
| 1 | `업무코드·Context·ServiceId 기준`의 단일 기준은 무엇인가? | 허용·권고·금지 규칙과 적용 대상 |
| 2 | 기준은 어느 경계에서 실행되는가? | 단말 모델 → MCA 표준 Header/Body → Gateway/TCF StandardRequest → StandardResponse |
| 3 | 누가 작성·구현·검증·승인·운영하는가? | MCA·TA·AA·SA의 역할 분리 |
...
| 9 | 완료로 인정할 최소 증적은 무엇인가? | 라우팅 규칙, 테스트, 자동검증, 운영 확인 |
...
**`/sv`, `/ic` 등 Context와 업무코드를 일치시킨다.**

이행 결과는 `라우팅 규칙` 하나만 제출하는 것으로 끝나지 않는다. 관련 소스·설정·테스트·자동검증·운영 절차가 같은 결정 ID로 추적되어야 한다.
...
consulted: "TA·AA"
approver: "SA"
decisionScope: "업무코드·Context·ServiceId 기준"
dueDate: "승인 회의에서 확정"
deliverable: "라우팅 규칙"
```
...
implementation: "`/sv`, `/ic` 등 Context와 업무코드를 일치시킨다."
alternatives:
  - 중앙 표준·공통 강제
  - 영역 자율·경계 계약
  - 승인된 제한 예외
...
| 구분 | 기록할 값 |
|---|---|
| 요구사항·TASK·ADR | 요구사항 ID, MCA-07, ADR ID |
| 설계·가이드 | 문서 경로와 승인 버전 |
...
- [ ] 처음 읽는 사람이 `업무코드·Context·ServiceId 기준`의 의미와 결정 이유를 설명할 수 있다.
...
- [ ] 주관 MCA, 협의 TA·AA, 승인 SA의 행동이 구분되어 있다.
- [ ] `라우팅 규칙`의 작성자·검토자·기한이 있다.
...
MCA-07의 목적은 **채널 라우팅**에 관한 결정을 문서 한 장으로 끝내는 것이 아니라, `업무코드·Context·ServiceId 기준` 기준을 프로젝트 전체가 같은 방식으로 이해하고 실행·검증하게 만드는 것이다.

최종 승인 시에는 **URL과 업무코드 기준 라우팅을 표준화한다.**이라는 방향을 구체적인 값과 경계로 확정하고, **`/sv`, `/ic` 등 Context와 업무코드를 일치시킨다.**을 코드·설정·테스트·운영 증적으로 입증해야 한다. 그때 비로소 MCA-07를 완료로 판정할 수 있다.
