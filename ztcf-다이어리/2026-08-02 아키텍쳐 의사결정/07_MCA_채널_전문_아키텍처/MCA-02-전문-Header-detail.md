# MCA-02 전문 Header 상세 설명서

> 문서 성격: **이해·검토용 상세 초안**  
> 상태: **ARB 승인 전** · 영역: **MCA·채널·전문** · 우선순위: **P0**

## 1. 이 문서를 먼저 읽는 이유

이 문서는 요약본 [MCA-02-전문-Header.md](./MCA-02-전문-Header.md)의 내용을 처음 접하는 사람도 MCA-02의 의미, 필요한 이유, 실제로 결정할 항목과 완료 판단 방법을 이해할 수 있도록 풀어서 설명한다.

MCA-02의 핵심 주제는 **전문 Header**이다. 쉽게 말하면 **서로 다른 단말과 서버가 같은 요청·응답 의미를 공유하도록 전문 구조·필드·버전·라우팅을 정하는 영역**에서 `ServiceId·사용자·채널·지점·Trace`에 관한 공통 기준을 정하는 일이다. 이 기준을 정하지 않으면 팀마다 서로 다른 판단을 구현하고, 통합·보안·운영 단계에서 뒤늦게 충돌할 가능성이 높다.
...
| TASK ID | MCA-02 |
| 의사결정 사항 | 전문 Header |
| 이번에 확정할 범위 | ServiceId·사용자·채널·지점·Trace |
| 권고 초안 | Header에 거래 식별·인증·추적 필드를 정의한다. |
| 이행 방향 | ServiceId, 거래코드, 사용자, 지점, 채널, TraceId를 포함한다. |
| 주관 | MCA |
| 협의 | FW·SEC |
| 승인 | AA |
| 우선순위 | P0 |
...
MCA·채널·전문는 **서로 다른 단말과 서버가 같은 요청·응답 의미를 공유하도록 전문 구조·필드·버전·라우팅을 정하는 영역**이다.
...
- 의사결정 주제: `ServiceId·사용자·채널·지점·Trace`
- 책임 경계: 단말 모델 → MCA 표준 Header/Body → Gateway/TCF StandardRequest → StandardResponse
- 관련 구성요소: 표준 전문 Schema, StandardHeader, Result, 필드사전, 버전정책, 채널 라우팅·Timeout·오류 매핑
- 설계·개발·시험·배포·운영 중 이 결정의 영향을 받는 산출물
- 기존 구현을 변경할 경우 호환성, 전환, Rollback과 폐기 계획

### 기본적으로 적용하지 않는 범위

- MCA-02와 직접 관련 없는 코드 정리나 대규모 이름 변경
...
- 팀별로 `ServiceId·사용자·채널·지점·Trace` 해석이 달라져 동일 기능이 여러 방식으로 구현된다.
...
대표 실패 유형은 **채널별 필드 의미 불일치, 길이 증가 미반영, Header 위변조, 무제한 전문, 하위호환 없는 변경**이다. 단기적으로 기능이 동작해도 운영 안정성, 변경 용이성, 감사 가능성과 장애 격리 능력이 약해질 수 있다.
...
| 순서 | 결정 질문 | 답변에 포함할 내용 |
|---:|---|---|
| 1 | `ServiceId·사용자·채널·지점·Trace`의 단일 기준은 무엇인가? | 허용·권고·금지 규칙과 적용 대상 |
| 2 | 기준은 어느 경계에서 실행되는가? | 단말 모델 → MCA 표준 Header/Body → Gateway/TCF StandardRequest → StandardResponse |
| 3 | 누가 작성·구현·검증·승인·운영하는가? | MCA·FW·SEC·AA의 역할 분리 |
...
| 8 | 무엇으로 준수를 증명하는가? | Schema/Contract Test, 필수·길이·코드·민감도 검증, 구·신 버전 병행, 오류·Timeout 변환 시험 |
| 9 | 완료로 인정할 최소 증적은 무엇인가? | Header 필드표, 테스트, 자동검증, 운영 확인 |
...
**ServiceId, 거래코드, 사용자, 지점, 채널, TraceId를 포함한다.**

이행 결과는 `Header 필드표` 하나만 제출하는 것으로 끝나지 않는다. 관련 소스·설정·테스트·자동검증·운영 절차가 같은 결정 ID로 추적되어야 한다.
...
```text
단말 모델 → MCA 표준 Header/Body → Gateway/TCF StandardRequest → StandardResponse
```
...
| 역할 | 수행 내용 | 제출 증적 |
|---|---|---|
| 주관 MCA | 현행·문제·대안·권고안 작성, 영향 대상 조정 | TASK 분석서, 권고안, ADR 초안 |
...
channelId와 serviceId가 검증된 표준 Header로 전달되고 업무 Body는 계약 버전별 Schema와 최대 크기를 준수.
...
consulted: "FW·SEC"
approver: "AA"
decisionScope: "ServiceId·사용자·채널·지점·Trace"
dueDate: "승인 회의에서 확정"
deliverable: "Header 필드표"
```
...
implementation: "ServiceId, 거래코드, 사용자, 지점, 채널, TraceId를 포함한다."
alternatives:
  - 중앙 표준·공통 강제
  - 영역 자율·경계 계약
  - 승인된 제한 예외
...
| 구분 | 기록할 값 |
|---|---|
| 요구사항·TASK·ADR | 요구사항 ID, MCA-02, ADR ID |
| 설계·가이드 | 문서 경로와 승인 버전 |
...
- Schema/Contract Test, 필수·길이·코드·민감도 검증, 구·신 버전 병행, 오류·Timeout 변환 시험
...
- [ ] 처음 읽는 사람이 `ServiceId·사용자·채널·지점·Trace`의 의미와 결정 이유를 설명할 수 있다.
...
- [ ] 주관 MCA, 협의 FW·SEC, 승인 AA의 행동이 구분되어 있다.
- [ ] `Header 필드표`의 작성자·검토자·기한이 있다.
...
MCA-02의 목적은 **전문 Header**에 관한 결정을 문서 한 장으로 끝내는 것이 아니라, `ServiceId·사용자·채널·지점·Trace` 기준을 프로젝트 전체가 같은 방식으로 이해하고 실행·검증하게 만드는 것이다.

최종 승인 시에는 **Header에 거래 식별·인증·추적 필드를 정의한다.**이라는 방향을 구체적인 값과 경계로 확정하고, **ServiceId, 거래코드, 사용자, 지점, 채널, TraceId를 포함한다.**을 코드·설정·테스트·운영 증적으로 입증해야 한다. 그때 비로소 MCA-02를 완료로 판정할 수 있다.

## 21. 참조 문서

- [zdocs-1/architecture/02-junmun.md](../../../zdocs-1/architecture/02-junmun.md)
- [zarchitecture/13-UI-채널-아키텍처.md](../../../zarchitecture/13-UI-채널-아키텍처.md)
- [zdocs-1/architecture/46-service-integration-contract.md](../../../zdocs-1/architecture/46-service-integration-contract.md)
