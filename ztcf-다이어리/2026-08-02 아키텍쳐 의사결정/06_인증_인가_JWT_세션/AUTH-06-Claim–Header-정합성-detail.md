# AUTH-06 Claim–Header 정합성 상세 설명서
...
이 문서는 요약본 [AUTH-06-Claim–Header-정합성.md](./AUTH-06-Claim–Header-정합성.md)의 내용을 처음 접하는 사람도 AUTH-06의 의미, 필요한 이유, 실제로 결정할 항목과 완료 판단 방법을 이해할 수 있도록 풀어서 설명한다.

AUTH-06의 핵심 주제는 **Claim–Header 정합성**이다. 쉽게 말하면 **사용자가 누구인지 확인하고 어떤 기능·데이터를 사용할 수 있는지 서버가 검증하는 영역**에서 `보정·비교·불일치 거부 기준`에 관한 공통 기준을 정하는 일이다. 이 기준을 정하지 않으면 팀마다 서로 다른 판단을 구현하고, 통합·보안·운영 단계에서 뒤늦게 충돌할 가능성이 높다.
...
|---|---|
| TASK ID | AUTH-06 |
| 의사결정 사항 | Claim–Header 정합성 |
| 이번에 확정할 범위 | 보정·비교·불일치 거부 기준 |
| 권고 초안 | JWT Claim을 신뢰원으로 삼아 Header를 보정한다. |
| 이행 방향 | Claim과 클라이언트 Header 불일치 시 요청을 거부한다. |
| 주관 | FW |
| 협의 | SEC·MCA·AA |
| 승인 | SEC |
| 우선순위 | P0 |
...
- AUTH-06와 직접 관련 없는 코드 정리나 대규모 이름 변경
...
- 팀별로 `보정·비교·불일치 거부 기준` 해석이 달라져 동일 기능이 여러 방식으로 구현된다.
...
| 9 | 완료로 인정할 최소 증적은 무엇인가? | Header 검증기준, 테스트, 자동검증, 운영 확인 |
...
**JWT Claim을 신뢰원으로 삼아 Header를 보정한다.**
...
**Claim과 클라이언트 Header 불일치 시 요청을 거부한다.**

이행 결과는 `Header 검증기준` 하나만 제출하는 것으로 끝나지 않는다. 관련 소스·설정·테스트·자동검증·운영 절차가 같은 결정 ID로 추적되어야 한다.
...
요구사항·현행 문제
  ...
  → 대안과 영향도 비교
  → PoC 또는 대표 시나리오 검증
  → FW 권고안 작성 / SEC·MCA·AA 협의
  → SEC 승인 및 ADR Baseline
  → 개발표준·공통 구현·샘플 반영
  → 테스트·CI/CD·운영 검증
  → 적용 확인·예외 추적·정기 재검토
```
...
| 협의 SEC·MCA·AA | 영역 요구와 부작용 검토, 테스트·운영 조건 제시 | 검토 의견, 영향도, 조건부 조치 |
| 승인 SEC | 선택·기각 근거, 잔여 위험과 예외 수용 여부 결정 | 승인 기록, ADR Baseline |
...
```yaml
taskId: AUTH-06
title: "Claim–Header 정합성"
status: 분석-중
priority: P0
owner: FW
consulted: "SEC·MCA·AA"
approver: "SEC"
decisionScope: "보정·비교·불일치 거부 기준"
dueDate: "승인 회의에서 확정"
deliverable: "Header 검증기준"
```
...
implementation: "Claim과 클라이언트 Header 불일치 시 요청을 거부한다."
alternatives:
  - 중앙 표준·공통 강제
  - 영역 자율·경계 계약
  - 승인된 제한 예외
...
| 구분 | 기록할 값 |
|---|---|
| 요구사항·TASK·ADR | 요구사항 ID, AUTH-06, ADR ID |
| 설계·가이드 | 문서 경로와 승인 버전 |
...
- [ ] 처음 읽는 사람이 `보정·비교·불일치 거부 기준`의 의미와 결정 이유를 설명할 수 있다.
...
- [ ] 주관 FW, 협의 SEC·MCA·AA, 승인 SEC의 행동이 구분되어 있다.
- [ ] `Header 검증기준`의 작성자·검토자·기한이 있다.
...
AUTH-06의 목적은 **Claim–Header 정합성**에 관한 결정을 문서 한 장으로 끝내는 것이 아니라, `보정·비교·불일치 거부 기준` 기준을 프로젝트 전체가 같은 방식으로 이해하고 실행·검증하게 만드는 것이다.

최종 승인 시에는 **JWT Claim을 신뢰원으로 삼아 Header를 보정한다.**이라는 방향을 구체적인 값과 경계로 확정하고, **Claim과 클라이언트 Header 불일치 시 요청을 거부한다.**을 코드·설정·테스트·운영 증적으로 입증해야 한다. 그때 비로소 AUTH-06를 완료로 판정할 수 있다.
