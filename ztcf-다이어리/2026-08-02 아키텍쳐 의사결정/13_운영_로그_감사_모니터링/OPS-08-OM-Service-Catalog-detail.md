# OPS-08 OM Service Catalog 상세 설명서

> 문서 성격: **이해·검토용 상세 초안**  
> 상태: **ARB 승인 전** · 영역: **운영·로그·감사·모니터링** · 우선순위: **P0**

## 1. 이 문서를 먼저 읽는 이유

이 문서는 요약본 [OPS-08-OM-Service-Catalog.md](./OPS-08-OM-Service-Catalog.md)의 내용을 처음 접하는 사람도 OPS-08의 의미, 필요한 이유, 실제로 결정할 항목과 완료 판단 방법을 이해할 수 있도록 풀어서 설명한다.

OPS-08의 핵심 주제는 **OM Service Catalog**이다. 쉽게 말하면 **거래와 장애를 빠르게 찾고 중요 행위의 책임과 변경 이력을 증명할 수 있게 운영 정보를 정하는 영역**에서 `ServiceId·Handler·Timeout·상태`에 관한 공통 기준을 정하는 일이다. 이 기준을 정하지 않으면 팀마다 서로 다른 판단을 구현하고, 통합·보안·운영 단계에서 뒤늦게 충돌할 가능성이 높다.
...
|---|---|
| TASK ID | OPS-08 |
| 의사결정 사항 | OM Service Catalog |
| 이번에 확정할 범위 | ServiceId·Handler·Timeout·상태 |
| 권고 초안 | OM Catalog를 ServiceId 공식 원장으로 운영한다. |
| 이행 방향 | Handler·Timeout·상태·업무책임 정보를 관리한다. |
| 주관 | FW |
| 협의 | AA·OPS·DEV |
| 승인 | AA |
| 우선순위 | P0 |
...
운영·로그·감사·모니터링는 **거래와 장애를 빠르게 찾고 중요 행위의 책임과 변경 이력을 증명할 수 있게 운영 정보를 정하는 영역**이다.
...
- 의사결정 주제: `ServiceId·Handler·Timeout·상태`
- 책임 경계: Gateway/TCF/업무/EAI/DB 로그·메트릭 → OM/수집·대시보드 → 알림·Runbook·감사
- 관련 구성요소: TransactionLogService, TraceId, 구조화 로그, 감사 이벤트, tcf-om, tcf-batch, Service Catalog, Runbook
- 설계·개발·시험·배포·운영 중 이 결정의 영향을 받는 산출물
- 기존 구현을 변경할 경우 호환성, 전환, Rollback과 폐기 계획

### 기본적으로 적용하지 않는 범위

- OPS-08와 직접 관련 없는 코드 정리나 대규모 이름 변경
...
- 팀별로 `ServiceId·Handler·Timeout·상태` 해석이 달라져 동일 기능이 여러 방식으로 구현된다.
...
대표 실패 유형은 **개인정보·Token 로그, Trace 단절, 성공 로그만 기록, 증상 중심 과다 알림, 운영 기준정보 Drift**이다. 단기적으로 기능이 동작해도 운영 안정성, 변경 용이성, 감사 가능성과 장애 격리 능력이 약해질 수 있다.
...
| 순서 | 결정 질문 | 답변에 포함할 내용 |
|---:|---|---|
| 1 | `ServiceId·Handler·Timeout·상태`의 단일 기준은 무엇인가? | 허용·권고·금지 규칙과 적용 대상 |
| 2 | 기준은 어느 경계에서 실행되는가? | Gateway/TCF/업무/EAI/DB 로그·메트릭 → OM/수집·대시보드 → 알림·Runbook·감사 |
| 3 | 누가 작성·구현·검증·승인·운영하는가? | FW·AA·OPS·DEV·AA의 역할 분리 |
...
| 8 | 무엇으로 준수를 증명하는가? | GUID·TraceId 종단 추적, 로그 마스킹, 거래 시작·종료, 감사 완전성, 알림 조치가능성, Runbook 복구훈련 |
| 9 | 완료로 인정할 최소 증적은 무엇인가? | Catalog 설계, 테스트, 자동검증, 운영 확인 |
...
**OM Catalog를 ServiceId 공식 원장으로 운영한다.**
...
**Handler·Timeout·상태·업무책임 정보를 관리한다.**

이행 결과는 `Catalog 설계` 하나만 제출하는 것으로 끝나지 않는다. 관련 소스·설정·테스트·자동검증·운영 절차가 같은 결정 ID로 추적되어야 한다.
...
요구사항·현행 문제
  ...
  → 대안과 영향도 비교
  → PoC 또는 대표 시나리오 검증
  → FW 권고안 작성 / AA·OPS·DEV 협의
  → AA 승인 및 ADR Baseline
  → 개발표준·공통 구현·샘플 반영
  → 테스트·CI/CD·운영 검증
  → 적용 확인·예외 추적·정기 재검토
```
...
```text
Gateway/TCF/업무/EAI/DB 로그·메트릭 → OM/수집·대시보드 → 알림·Runbook·감사
```
...
| 역할 | 수행 내용 | 제출 증적 |
|---|---|---|
| 주관 FW | 현행·문제·대안·권고안 작성, 영향 대상 조정 | TASK 분석서, 권고안, ADR 초안 |
| 협의 AA·OPS·DEV | 영역 요구와 부작용 검토, 테스트·운영 조건 제시 | 검토 의견, 영향도, 조건부 조치 |
| 승인 AA | 선택·기각 근거, 잔여 위험과 예외 수용 여부 결정 | 승인 기록, ADR Baseline |
...
TraceId 하나로 Gateway, TCF 거래로그, ServiceId, Mapper SQL과 외부연계 결과를 찾고 담당 Runbook으로 연결.
...
```yaml
taskId: OPS-08
title: "OM Service Catalog"
status: 분석-중
priority: P0
owner: FW
consulted: "AA·OPS·DEV"
approver: "AA"
decisionScope: "ServiceId·Handler·Timeout·상태"
dueDate: "승인 회의에서 확정"
deliverable: "Catalog 설계"
```
...
implementation: "Handler·Timeout·상태·업무책임 정보를 관리한다."
alternatives:
  - 중앙 표준·공통 강제
  - 영역 자율·경계 계약
  - 승인된 제한 예외
...
| 구분 | 기록할 값 |
|---|---|
| 요구사항·TASK·ADR | 요구사항 ID, OPS-08, ADR ID |
| 설계·가이드 | 문서 경로와 승인 버전 |
...
- GUID·TraceId 종단 추적, 로그 마스킹, 거래 시작·종료, 감사 완전성, 알림 조치가능성, Runbook 복구훈련
...
- [ ] 처음 읽는 사람이 `ServiceId·Handler·Timeout·상태`의 의미와 결정 이유를 설명할 수 있다.
...
- [ ] 주관 FW, 협의 AA·OPS·DEV, 승인 AA의 행동이 구분되어 있다.
- [ ] `Catalog 설계`의 작성자·검토자·기한이 있다.
...
OPS-08의 목적은 **OM Service Catalog**에 관한 결정을 문서 한 장으로 끝내는 것이 아니라, `ServiceId·Handler·Timeout·상태` 기준을 프로젝트 전체가 같은 방식으로 이해하고 실행·검증하게 만드는 것이다.

최종 승인 시에는 **OM Catalog를 ServiceId 공식 원장으로 운영한다.**이라는 방향을 구체적인 값과 경계로 확정하고, **Handler·Timeout·상태·업무책임 정보를 관리한다.**을 코드·설정·테스트·운영 증적으로 입증해야 한다. 그때 비로소 OPS-08를 완료로 판정할 수 있다.

## 21. 참조 문서

- [zdocs-1/architecture/37-transaction-log.md](../../../zdocs-1/architecture/37-transaction-log.md)
- [zdocs-1/architecture/44-observability.md](../../../zdocs-1/architecture/44-observability.md)
- [zdocs-1/architecture/52-om-operations.md](../../../zdocs-1/architecture/52-om-operations.md)
