# STD-10 Mapper·SQL ID 상세 설명서

> 문서 성격: **이해·검토용 상세 초안**  
> 상태: **ARB 승인 전** · 영역: **명명·식별자·추적성** · 우선순위: **P0**

## 1. 이 문서를 먼저 읽는 이유

이 문서는 요약본 [STD-10-Mapper·SQL-ID.md](./STD-10-Mapper·SQL-ID.md)의 내용을 처음 접하는 사람도 STD-10의 의미, 필요한 이유, 실제로 결정할 항목과 완료 판단 방법을 이해할 수 있도록 풀어서 설명한다.

STD-10의 핵심 주제는 **Mapper·SQL ID**이다. 쉽게 말하면 **화면에서 SQL과 운영로그까지 같은 거래를 일관된 식별자로 찾아갈 수 있게 이름과 관계를 정하는 영역**에서 `Mapper Namespace·Statement ID`에 관한 공통 기준을 정하는 일이다. 이 기준을 정하지 않으면 팀마다 서로 다른 판단을 구현하고, 통합·보안·운영 단계에서 뒤늦게 충돌할 가능성이 높다.
...
|---|---|
| TASK ID | STD-10 |
| 의사결정 사항 | Mapper·SQL ID |
| 이번에 확정할 범위 | Mapper Namespace·Statement ID |
| 권고 초안 | Mapper ID는 행위와 업무대상을 표현한다. |
| 이행 방향 | Namespace 중복과 Statement ID 중복을 빌드 시 검사한다. |
| 주관 | DA |
| 협의 | DBA·AA |
| 승인 | DA |
| 우선순위 | P0 |
...
명명·식별자·추적성는 **화면에서 SQL과 운영로그까지 같은 거래를 일관된 식별자로 찾아갈 수 있게 이름과 관계를 정하는 영역**이다.
...
- 의사결정 주제: `Mapper Namespace·Statement ID`
- 책임 경계: 화면 ID·이벤트 ID·ServiceId·거래코드·Java 이름·Mapper ID·DB 객체·설정 Key·TraceId
- 관련 구성요소: 명명사전, 관리대장, Service Catalog, 소스 색인, Checkstyle, ArchUnit, 정규식 검사
- 설계·개발·시험·배포·운영 중 이 결정의 영향을 받는 산출물
- 기존 구현을 변경할 경우 호환성, 전환, Rollback과 폐기 계획

### 기본적으로 적용하지 않는 범위

- STD-10와 직접 관련 없는 코드 정리나 대규모 이름 변경
...
- 팀별로 `Mapper Namespace·Statement ID` 해석이 달라져 동일 기능이 여러 방식으로 구현된다.
...
대표 실패 유형은 **동일 대상을 서로 다른 이름으로 표현, 임시 ID 고착, 중복 ServiceId, 소스와 OM 불일치, 추적 단절**이다. 단기적으로 기능이 동작해도 운영 안정성, 변경 용이성, 감사 가능성과 장애 격리 능력이 약해질 수 있다.
...
| 순서 | 결정 질문 | 답변에 포함할 내용 |
|---:|---|---|
| 1 | `Mapper Namespace·Statement ID`의 단일 기준은 무엇인가? | 허용·권고·금지 규칙과 적용 대상 |
| 2 | 기준은 어느 경계에서 실행되는가? | 화면 ID·이벤트 ID·ServiceId·거래코드·Java 이름·Mapper ID·DB 객체·설정 Key·TraceId |
| 3 | 누가 작성·구현·검증·승인·운영하는가? | DA·DBA·AA·DA의 역할 분리 |
...
| 8 | 무엇으로 준수를 증명하는가? | 정규식·중복검사, 화면–ServiceId–Handler–Mapper–Table–로그 추적성 매트릭스 대조 |
| 9 | 완료로 인정할 최소 증적은 무엇인가? | SQL ID 규칙, 테스트, 자동검증, 운영 확인 |
...
**Namespace 중복과 Statement ID 중복을 빌드 시 검사한다.**

이행 결과는 `SQL ID 규칙` 하나만 제출하는 것으로 끝나지 않는다. 관련 소스·설정·테스트·자동검증·운영 절차가 같은 결정 ID로 추적되어야 한다.
...
```text
화면 ID·이벤트 ID·ServiceId·거래코드·Java 이름·Mapper ID·DB 객체·설정 Key·TraceId
```
...
| 역할 | 수행 내용 | 제출 증적 |
|---|---|---|
| 주관 DA | 현행·문제·대안·권고안 작성, 영향 대상 조정 | TASK 분석서, 권고안, ADR 초안 |
| 협의 DBA·AA | 영역 요구와 부작용 검토, 테스트·운영 조건 제시 | 검토 의견, 영향도, 조건부 조치 |
...
화면 이벤트가 SV.Customer.selectSummary와 Handler 상수, OM Catalog, 거래로그 serviceId 및 Mapper SQL ID로 연결.
...
consulted: "DBA·AA"
approver: "DA"
decisionScope: "Mapper Namespace·Statement ID"
dueDate: "승인 회의에서 확정"
deliverable: "SQL ID 규칙"
```
...
implementation: "Namespace 중복과 Statement ID 중복을 빌드 시 검사한다."
alternatives:
  - 중앙 표준·공통 강제
  - 영역 자율·경계 계약
  - 승인된 제한 예외
...
| 구분 | 기록할 값 |
|---|---|
| 요구사항·TASK·ADR | 요구사항 ID, STD-10, ADR ID |
| 설계·가이드 | 문서 경로와 승인 버전 |
...
- 정규식·중복검사, 화면–ServiceId–Handler–Mapper–Table–로그 추적성 매트릭스 대조
...
- [ ] 처음 읽는 사람이 `Mapper Namespace·Statement ID`의 의미와 결정 이유를 설명할 수 있다.
...
- [ ] 주관 DA, 협의 DBA·AA, 승인 DA의 행동이 구분되어 있다.
- [ ] `SQL ID 규칙`의 작성자·검토자·기한이 있다.
...
STD-10의 목적은 **Mapper·SQL ID**에 관한 결정을 문서 한 장으로 끝내는 것이 아니라, `Mapper Namespace·Statement ID` 기준을 프로젝트 전체가 같은 방식으로 이해하고 실행·검증하게 만드는 것이다.

최종 승인 시에는 **Mapper ID는 행위와 업무대상을 표현한다.**이라는 방향을 구체적인 값과 경계로 확정하고, **Namespace 중복과 Statement ID 중복을 빌드 시 검사한다.**을 코드·설정·테스트·운영 증적으로 입증해야 한다. 그때 비로소 STD-10를 완료로 판정할 수 있다.

## 21. 참조 문서

- [zdocs-1/architecture/53-naming-conventions.md](../../../zdocs-1/architecture/53-naming-conventions.md)
- [zdocs-1/architecture/06-naming.md](../../../zdocs-1/architecture/06-naming.md)
- [zdocs-1/SOURCE_INDEX.md](../../../zdocs-1/SOURCE_INDEX.md)
