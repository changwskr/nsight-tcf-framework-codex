# UI-08 iframe 연계 상세 설명서

> 문서 성격: **이해·검토용 상세 초안**  
> 상태: **ARB 승인 전** · 영역: **단말·UI** · 우선순위: **P0**

## 1. 이 문서를 먼저 읽는 이유

이 문서는 요약본 [UI-08-iframe-연계.md](./UI-08-iframe-연계.md)의 내용을 처음 접하는 사람도 UI-08의 의미, 필요한 이유, 실제로 결정할 항목과 완료 판단 방법을 이해할 수 있도록 풀어서 설명한다.

UI-08의 핵심 주제는 **iframe 연계**이다. 쉽게 말하면 **사용자 화면이 입력·오류·권한·파일·상태를 안전하고 일관되게 처리하도록 정하는 영역**에서 `postMessage·origin·토큰 전달`에 관한 공통 기준을 정하는 일이다. 이 기준을 정하지 않으면 팀마다 서로 다른 판단을 구현하고, 통합·보안·운영 단계에서 뒤늦게 충돌할 가능성이 높다.
...
| TASK ID | UI-08 |
| 의사결정 사항 | iframe 연계 |
| 이번에 확정할 범위 | postMessage·origin·토큰 전달 |
| 권고 초안 | iframe 간 전달은 제한된 postMessage를 사용한다. |
| 이행 방향 | targetOrigin·event.origin·메시지 타입을 검증한다. |
| 주관 | UI |
| 협의 | SEC·MCA |
| 승인 | SEC |
| 우선순위 | P0 |
...
단말·UI는 **사용자 화면이 입력·오류·권한·파일·상태를 안전하고 일관되게 처리하도록 정하는 영역**이다.
...
- 의사결정 주제: `postMessage·origin·토큰 전달`
- 책임 경계: WEBTOPSUITE·React·일반 Web → Gateway/MCA → 공통 온라인 Endpoint
- 관련 구성요소: tcf-ui, tcf-uj, 공통 UI 컴포넌트, Validation, 오류표시, Token 저장, 파일·엑셀 처리
- 설계·개발·시험·배포·운영 중 이 결정의 영향을 받는 산출물
- 기존 구현을 변경할 경우 호환성, 전환, Rollback과 폐기 계획

### 기본적으로 적용하지 않는 범위

- UI-08와 직접 관련 없는 코드 정리나 대규모 이름 변경
...
- 팀별로 `postMessage·origin·토큰 전달` 해석이 달라져 동일 기능이 여러 방식으로 구현된다.
...
대표 실패 유형은 **UI 검증만 신뢰, 원문 개인정보 수신 후 숨김, Token 장기저장, origin 미검증, 전체 데이터 브라우저 적재**이다. 단기적으로 기능이 동작해도 운영 안정성, 변경 용이성, 감사 가능성과 장애 격리 능력이 약해질 수 있다.
...
| 순서 | 결정 질문 | 답변에 포함할 내용 |
|---:|---|---|
| 1 | `postMessage·origin·토큰 전달`의 단일 기준은 무엇인가? | 허용·권고·금지 규칙과 적용 대상 |
| 2 | 기준은 어느 경계에서 실행되는가? | WEBTOPSUITE·React·일반 Web → Gateway/MCA → 공통 온라인 Endpoint |
| 3 | 누가 작성·구현·검증·승인·운영하는가? | UI·SEC·MCA·SEC의 역할 분리 |
...
| 8 | 무엇으로 준수를 증명하는가? | 브라우저 호환·접근성·보안·대용량 시험과 서버 우회 요청을 포함한 통합시험 |
| 9 | 완료로 인정할 최소 증적은 무엇인가? | iframe 보안설계, 테스트, 자동검증, 운영 확인 |
...
**iframe 간 전달은 제한된 postMessage를 사용한다.**
...
**targetOrigin·event.origin·메시지 타입을 검증한다.**

이행 결과는 `iframe 보안설계` 하나만 제출하는 것으로 끝나지 않는다. 관련 소스·설정·테스트·자동검증·운영 절차가 같은 결정 ID로 추적되어야 한다.
...
```text
WEBTOPSUITE·React·일반 Web → Gateway/MCA → 공통 온라인 Endpoint
```
...
| 협의 SEC·MCA | 영역 요구와 부작용 검토, 테스트·운영 조건 제시 | 검토 의견, 영향도, 조건부 조치 |
...
조회 버튼은 중복 클릭을 막지만 서버도 멱등키를 검증하고, 서버가 권한에 맞게 마스킹한 응답만 화면에 전달.
...
consulted: "SEC·MCA"
approver: "SEC"
decisionScope: "postMessage·origin·토큰 전달"
dueDate: "승인 회의에서 확정"
deliverable: "iframe 보안설계"
```
...
implementation: "targetOrigin·event.origin·메시지 타입을 검증한다."
alternatives:
  - 중앙 표준·공통 강제
  - 영역 자율·경계 계약
  - 승인된 제한 예외
...
| 구분 | 기록할 값 |
|---|---|
| 요구사항·TASK·ADR | 요구사항 ID, UI-08, ADR ID |
| 설계·가이드 | 문서 경로와 승인 버전 |
...
- 브라우저 호환·접근성·보안·대용량 시험과 서버 우회 요청을 포함한 통합시험
...
- [ ] 처음 읽는 사람이 `postMessage·origin·토큰 전달`의 의미와 결정 이유를 설명할 수 있다.
...
- [ ] 주관 UI, 협의 SEC·MCA, 승인 SEC의 행동이 구분되어 있다.
- [ ] `iframe 보안설계`의 작성자·검토자·기한이 있다.
...
UI-08의 목적은 **iframe 연계**에 관한 결정을 문서 한 장으로 끝내는 것이 아니라, `postMessage·origin·토큰 전달` 기준을 프로젝트 전체가 같은 방식으로 이해하고 실행·검증하게 만드는 것이다.

최종 승인 시에는 **iframe 간 전달은 제한된 postMessage를 사용한다.**이라는 방향을 구체적인 값과 경계로 확정하고, **targetOrigin·event.origin·메시지 타입을 검증한다.**을 코드·설정·테스트·운영 증적으로 입증해야 한다. 그때 비로소 UI-08를 완료로 판정할 수 있다.
