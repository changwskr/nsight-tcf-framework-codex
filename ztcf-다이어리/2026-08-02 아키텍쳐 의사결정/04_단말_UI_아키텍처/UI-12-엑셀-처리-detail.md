...
이 문서는 요약본 [UI-12-엑셀-처리.md](./UI-12-엑셀-처리.md)의 내용을 처음 접하는 사람도 UI-12의 의미, 필요한 이유, 실제로 결정할 항목과 완료 판단 방법을 이해할 수 있도록 풀어서 설명한다.

UI-12의 핵심 주제는 **엑셀 처리**이다. 쉽게 말하면 **사용자 화면이 입력·오류·권한·파일·상태를 안전하고 일관되게 처리하도록 정하는 영역**에서 `건수 제한·마스킹·비동기 생성`에 관한 공통 기준을 정하는 일이다. 이 기준을 정하지 않으면 팀마다 서로 다른 판단을 구현하고, 통합·보안·운영 단계에서 뒤늦게 충돌할 가능성이 높다.
...
| TASK ID | UI-12 |
| 의사결정 사항 | 엑셀 처리 |
| 이번에 확정할 범위 | 건수 제한·마스킹·비동기 생성 |
| 권고 초안 | 대량 엑셀은 비동기 생성과 건수 제한을 적용한다. |
| 이행 방향 | 완료 알림과 다운로드 만료시간을 관리한다. |
| 주관 | UI |
| 협의 | AA·DA·OPS |
| 승인 | AA |
| 우선순위 | P1 |
...
- UI-12와 직접 관련 없는 코드 정리나 대규모 이름 변경
...
- 팀별로 `건수 제한·마스킹·비동기 생성` 해석이 달라져 동일 기능이 여러 방식으로 구현된다.
...
| 순서 | 결정 질문 | 답변에 포함할 내용 |
|---:|---|---|
| 1 | `건수 제한·마스킹·비동기 생성`의 단일 기준은 무엇인가? | 허용·권고·금지 규칙과 적용 대상 |
| 2 | 기준은 어느 경계에서 실행되는가? | WEBTOPSUITE·React·일반 Web → Gateway/MCA → 공통 온라인 Endpoint |
| 3 | 누가 작성·구현·검증·승인·운영하는가? | UI·AA·DA·OPS·AA의 역할 분리 |
...
| 9 | 완료로 인정할 최소 증적은 무엇인가? | 엑셀 처리기준, 테스트, 자동검증, 운영 확인 |
...
**완료 알림과 다운로드 만료시간을 관리한다.**

이행 결과는 `엑셀 처리기준` 하나만 제출하는 것으로 끝나지 않는다. 관련 소스·설정·테스트·자동검증·운영 절차가 같은 결정 ID로 추적되어야 한다.
...
| 협의 AA·DA·OPS | 영역 요구와 부작용 검토, 테스트·운영 조건 제시 | 검토 의견, 영향도, 조건부 조치 |
...
consulted: "AA·DA·OPS"
approver: "AA"
decisionScope: "건수 제한·마스킹·비동기 생성"
dueDate: "승인 회의에서 확정"
deliverable: "엑셀 처리기준"
```
...
taskId: UI-12
status: proposed
decision: "대량 엑셀은 비동기 생성과 건수 제한을 적용한다."
implementation: "완료 알림과 다운로드 만료시간을 관리한다."
alternatives:
  - 중앙 표준·공통 강제
  - 영역 자율·경계 계약
  - 승인된 제한 예외
...
| 구분 | 기록할 값 |
|---|---|
| 요구사항·TASK·ADR | 요구사항 ID, UI-12, ADR ID |
| 설계·가이드 | 문서 경로와 승인 버전 |
...
- [ ] 처음 읽는 사람이 `건수 제한·마스킹·비동기 생성`의 의미와 결정 이유를 설명할 수 있다.
...
- [ ] 주관 UI, 협의 AA·DA·OPS, 승인 AA의 행동이 구분되어 있다.
- [ ] `엑셀 처리기준`의 작성자·검토자·기한이 있다.
...
UI-12의 목적은 **엑셀 처리**에 관한 결정을 문서 한 장으로 끝내는 것이 아니라, `건수 제한·마스킹·비동기 생성` 기준을 프로젝트 전체가 같은 방식으로 이해하고 실행·검증하게 만드는 것이다.

최종 승인 시에는 **대량 엑셀은 비동기 생성과 건수 제한을 적용한다.**이라는 방향을 구체적인 값과 경계로 확정하고, **완료 알림과 다운로드 만료시간을 관리한다.**을 코드·설정·테스트·운영 증적으로 입증해야 한다. 그때 비로소 UI-12를 완료로 판정할 수 있다.
