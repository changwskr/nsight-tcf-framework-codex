...
public class SvCustomerService {
    ...
    public CustomerSummaryResponse selectCustomerSummary(CustomerSummaryRequest request, TransactionContext context) {
        var criteria = rule.buildCustomerSummaryCriteria(request);
        CustomerSummaryRow result = dao.selectCustomerSummary(criteria);
        rule.validateCustomerSummaryResult(result);
        return CustomerSummaryResponse.of(context, result);
    }
}
