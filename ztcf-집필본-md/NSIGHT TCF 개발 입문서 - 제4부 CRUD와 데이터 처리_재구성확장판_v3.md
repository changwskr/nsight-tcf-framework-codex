<!-- source: ztcf-집필본/NSIGHT TCF 개발 입문서 - 제4부 CRUD와 데이터 처리_재구성확장판_v3.docx -->
<!-- tables: mammoth-html + placeholder table-to-md -->

나는 제4부. CRUD와 데이터 처리 제대로 구현하기의 개념을 코드·설정·로그·산출물 중 어디에서 확인할 수 있는가? 문제가 발생했을 때 어느 경계부터 조사해야 하는가?
...
1.  단건 조회와 목록 조회를 구분한다.
2.  조회 결과 없음의 처리기준을 정의한다.
3.  검색조건과 페이징 DTO를 작성한다.
4.  등록 거래의 중복방지와 식별자 생성 방식을 이해한다.
5.  변경 전 현재 상태와 버전을 검증한다.
6.  물리 삭제와 논리 삭제를 구분한다.
7.  Facade에 적절한 트랜잭션 경계를 설정한다.
8.  일부 실패 시 Rollback 여부를 판단한다.
9.  Mapper SQL의 성능과 보안 기준을 적용한다.
10.  변경 이력·감사로그·운영로그를 연결한다.
11.  CRUD별 정상·오류·동시성 테스트를 작성한다.
12.  CI/CD 품질 Gate에 SQL과 데이터 변경 검증을 포함한다.
...
학습 목표 | 21장. CRUD 거래를 설계하는 기준의 핵심 개념을 설명하고, 프로젝트에서 적용 위치와 검증 방법을 스스로 판단한다.
...
SV 업무 Service
→ CM\_CAMPAIGN\_MASTER 직접 UPDATE
...
학습 목표 | 21장 요약의 핵심 개념을 설명하고, 프로젝트에서 적용 위치와 검증 방법을 스스로 판단한다.
...
학습 목표 | 22장. 단건 조회 구현하기의 핵심 개념을 설명하고, 프로젝트에서 적용 위치와 검증 방법을 스스로 판단한다.
...
학습 목표 | 22장 요약의 핵심 개념을 설명하고, 프로젝트에서 적용 위치와 검증 방법을 스스로 판단한다.
...
학습 목표 | 23장. 목록 조회와 페이징 구현하기의 핵심 개념을 설명하고, 프로젝트에서 적용 위치와 검증 방법을 스스로 판단한다.
...
case "campaignName" -> "CAMPAIGN\_NAME";
case "startDate" -> "START\_DATE";
case "createdAt" -> "CREATE\_DTM";
default -> "CAMPAIGN\_ID";
};
}
...
and campaignName != ''">
AND CAMPAIGN\_NAME LIKE '%' || #{campaignName} || '%'
</if>

<if test="campaignStatus != null
and campaignStatus != ''">
AND CAMPAIGN\_STATUS = #{campaignStatus}
</if>

<if test="startDateFrom != null
and startDateFrom != ''">
AND START\_DATE &gt;= #{startDateFrom}
</if>

<if test="startDateTo != null
and startDateTo != ''">
AND START\_DATE &lt;= #{startDateTo}
</if>
...
학습 목표 | 23장 요약의 핵심 개념을 설명하고, 프로젝트에서 적용 위치와 검증 방법을 스스로 판단한다.
...
학습 목표 | 24장. 등록 거래 구현하기의 핵심 개념을 설명하고, 프로젝트에서 적용 위치와 검증 방법을 스스로 판단한다.
...
INSERT INTO CM\_CAMPAIGN\_MASTER (
CAMPAIGN\_ID,
CAMPAIGN\_NAME,
CAMPAIGN\_TYPE,
CAMPAIGN\_STATUS,
...
학습 목표 | 24장 요약의 핵심 개념을 설명하고, 프로젝트에서 적용 위치와 검증 방법을 스스로 판단한다.
...
학습 목표 | 25장. 변경 거래와 동시성 제어의 핵심 개념을 설명하고, 프로젝트에서 적용 위치와 검증 방법을 스스로 판단한다.
...
학습 목표 | 25장 요약의 핵심 개념을 설명하고, 프로젝트에서 적용 위치와 검증 방법을 스스로 판단한다.
...
학습 목표 | 26장. 삭제와 사용중지 설계의 핵심 개념을 설명하고, 프로젝트에서 적용 위치와 검증 방법을 스스로 판단한다.
...
| 논리 삭제 | DELETE\_YN='Y' | 복구·이력 가능 | 모든 조회에 조건 필요 |
...
학습 목표 | 26장 요약의 핵심 개념을 설명하고, 프로젝트에서 적용 위치와 검증 방법을 스스로 판단한다.
...
학습 목표 | 27장. 트랜잭션과 Rollback 이해하기의 핵심 개념을 설명하고, 프로젝트에서 적용 위치와 검증 방법을 스스로 판단한다.
...
학습 목표 | 27장 요약의 핵심 개념을 설명하고, 프로젝트에서 적용 위치와 검증 방법을 스스로 판단한다.
...
학습 목표 | 28장. SQL 품질과 성능 기준의 핵심 개념을 설명하고, 프로젝트에서 적용 위치와 검증 방법을 스스로 판단한다.
...
WHERE SUBSTR(CREATE\_DTM, 1, 8) = :baseDate

가능하면 범위조건을 사용합니다.

WHERE CREATE\_DTM >= :startDtm
AND CREATE\_DTM < :endDtm

실제 실행계획은 DBA와 확인합니다.

## **28.5 Leading Wildcard**

WHERE CAMPAIGN\_NAME LIKE '%' || :name || '%'
...
학습 목표 | 28장 요약의 핵심 개념을 설명하고, 프로젝트에서 적용 위치와 검증 방법을 스스로 판단한다.
...
DELETE FROM CM\_CAMPAIGN\_MASTER
...
| **컬럼** | **설명** |
| --- | --- |
| CREATE\_USER\_ID | 생성자 |
| CREATE\_DTM | 생성시각 |
| UPDATE\_USER\_ID | 최종 변경자 |
| UPDATE\_DTM | 최종 변경시각 |
| VERSION\_NO | 낙관적 잠금 |
| DELETE\_YN | 논리 삭제 |
| CAMPAIGN\_STATUS | 업무 상태 |
