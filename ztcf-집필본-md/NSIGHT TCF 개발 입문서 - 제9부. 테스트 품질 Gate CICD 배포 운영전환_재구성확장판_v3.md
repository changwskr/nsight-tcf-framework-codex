<!-- source: ztcf-집필본/NSIGHT TCF 개발 입문서 - 제9부. 테스트 품질 Gate CICD 배포 운영전환_재구성확장판_v3.docx -->
<!-- tables: mammoth-html + placeholder table-to-md -->

나는 제9부. 테스트·품질 Gate·CI/CD·배포·운영전환의 개념을 코드·설정·로그·산출물 중 어디에서 확인할 수 있는가? 문제가 발생했을 때 어느 경계부터 조사해야 하는가?
...
1.  단위·통합·거래·계약 테스트를 구분한다.
2.  계층별로 적절한 테스트 범위를 결정한다.
3.  TCF 표준 요청을 이용한 거래 테스트를 작성한다.
4.  정상·업무 오류·시스템 오류·Timeout을 검증한다.
5.  성능·보안·장애 테스트의 목적을 설명한다.
6.  정적분석과 Architecture Test를 품질 Gate로 적용한다.
7.  Gradle 멀티모듈 프로젝트를 빌드한다.
8.  실행 가능한 WAR와 배포 대상 WAR를 구분한다.
9.  동일 Artifact를 개발·검증·운영 환경에 승격한다.
10.  CI와 CD의 차이를 설명한다.
11.  배포 전·중·후 점검항목을 정의한다.
12.  Rolling·Blue-Green·일괄 배포를 비교한다.
13.  Health Check와 Smoke Test를 구분한다.
14.  Rollback 기준과 절차를 설계한다.
15.  운영전환 체크리스트와 인수 증적을 작성한다.
...
학습 목표 | 64장. 테스트 전략 세우기의 핵심 개념을 설명하고, 프로젝트에서 적용 위치와 검증 방법을 스스로 판단한다.
...
학습 목표 | 64장 요약의 핵심 개념을 설명하고, 프로젝트에서 적용 위치와 검증 방법을 스스로 판단한다.
...
학습 목표 | 65장. 단위 테스트 작성하기의 핵심 개념을 설명하고, 프로젝트에서 적용 위치와 검증 방법을 스스로 판단한다.
...
학습 목표 | 65장 요약의 핵심 개념을 설명하고, 프로젝트에서 적용 위치와 검증 방법을 스스로 판단한다.
...
학습 목표 | 66장. Mapper·통합·거래 테스트의 핵심 개념을 설명하고, 프로젝트에서 적용 위치와 검증 방법을 스스로 판단한다.
...
학습 목표 | 66장 요약의 핵심 개념을 설명하고, 프로젝트에서 적용 위치와 검증 방법을 스스로 판단한다.
...
학습 목표 | 67장. 성능·장애·보안 테스트의 핵심 개념을 설명하고, 프로젝트에서 적용 위치와 검증 방법을 스스로 판단한다.
...
"customerNo": "' OR '1'='1",
"pageSize": 999999999,
"sortField": "CAMPAIGN\_ID DESC; DROP TABLE..."
}
...
학습 목표 | 67장 요약의 핵심 개념을 설명하고, 프로젝트에서 적용 위치와 검증 방법을 스스로 판단한다.
...
학습 목표 | 68장. 품질 Gate와 자동검증의 핵심 개념을 설명하고, 프로젝트에서 적용 위치와 검증 방법을 스스로 판단한다.
...
학습 목표 | 68장 요약의 핵심 개념을 설명하고, 프로젝트에서 적용 위치와 검증 방법을 스스로 판단한다.
...
학습 목표 | 69장. Gradle 빌드와 Artifact 관리의 핵심 개념을 설명하고, 프로젝트에서 적용 위치와 검증 방법을 스스로 판단한다.
...
학습 목표 | 69장 요약의 핵심 개념을 설명하고, 프로젝트에서 적용 위치와 검증 방법을 스스로 판단한다.
...
학습 목표 | 70장. CI Pipeline 구성하기의 핵심 개념을 설명하고, 프로젝트에서 적용 위치와 검증 방법을 스스로 판단한다.
...
학습 목표 | 70장 요약의 핵심 개념을 설명하고, 프로젝트에서 적용 위치와 검증 방법을 스스로 판단한다.
...
학습 목표 | 71장. CD와 안전한 배포의 핵심 개념을 설명하고, 프로젝트에서 적용 위치와 검증 방법을 스스로 판단한다.
...
학습 목표 | 71장 요약의 핵심 개념을 설명하고, 프로젝트에서 적용 위치와 검증 방법을 스스로 판단한다.
...
학습 목표 | 72장. DB 변경과 무중단 호환성의 핵심 개념을 설명하고, 프로젝트에서 적용 위치와 검증 방법을 스스로 판단한다.
...
ALTER TABLE CM\_CAMPAIGN\_MASTER
ADD NEW\_COLUMN VARCHAR2(20) NOT NULL;
...
V20260717\_01\_\_add\_campaign\_category.sql
...
학습 목표 | 72장 요약의 핵심 개념을 설명하고, 프로젝트에서 적용 위치와 검증 방법을 스스로 판단한다.
...
학습 목표 | 73장. Rollback과 장애 복구의 핵심 개념을 설명하고, 프로젝트에서 적용 위치와 검증 방법을 스스로 판단한다.
...
학습 목표 | 73장 요약의 핵심 개념을 설명하고, 프로젝트에서 적용 위치와 검증 방법을 스스로 판단한다.
...
학습 목표 | 74장. 운영전환과 Release 관리의 핵심 개념을 설명하고, 프로젝트에서 적용 위치와 검증 방법을 스스로 판단한다.
...
학습 목표 | 74장 요약의 핵심 개념을 설명하고, 프로젝트에서 적용 위치와 검증 방법을 스스로 판단한다.
