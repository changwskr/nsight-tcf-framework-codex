<!-- source: ztcf-집필본/NSIGHT TCF 개발 입문서 - 제19부. 요구사항 화면 테이블 정보로 신규 CRUD 설계·구현하기_재구성확장판_v3.docx -->
<!-- tables: mammoth-html + placeholder table-to-md -->

나는 제19부. 요구사항·화면·테이블 정보로 신규 CRUD 설계·구현하기의 개념을 코드·설정·로그·산출물 중 어디에서 확인할 수 있는가? 문제가 발생했을 때 어느 경계부터 조사해야 하는가?
...
1.  업무팀이 제공한 요구사항을 구조화한다.
2.  화면 항목과 테이블 컬럼을 업무 개념으로 연결한다.
3.  CRUD 기능을 독립된 ServiceId로 분해한다.
4.  화면 이벤트와 ServiceId를 매핑한다.
5.  거래코드·권한·Timeout·감사정책을 설계한다.
6.  Handler·Facade·Service·Rule·DAO·Mapper를 설계한다.
7.  Request·Response·Command·Query·Data DTO를 구분한다.
8.  등록·수정·취소의 트랜잭션과 이력을 설계한다.
9.  Version 기반 낙관적 동시성 제어를 구현한다.
10.  사용자·지점 기반 데이터권한을 구현한다.
11.  목록조회 Paging과 최대조회 범위를 적용한다.
12.  MyBatis Mapper XML과 SQL을 작성한다.
13.  정상·오류·Timeout·중복 시나리오를 테스트한다.
14.  OM Service Catalog와 운영 Metric을 등록한다.
15.  요구사항부터 DB와 테스트까지 추적성을 완성한다.
16.  AI 코딩도구가 생성한 CRUD 코드를 검증한다.
...
학습 목표 | 192장. 화면·테이블·요구사항 입력자료 정리의 핵심 개념을 설명하고, 프로젝트에서 적용 위치와 검증 방법을 스스로 판단한다.
...
| **컬럼** | **의미** | **유형** |
| --- | --- | --- |
| RESERVATION\_ID | 예약 ID | PK |
| CUSTOMER\_NO | 고객번호 | 업무참조 |
| RESERVATION\_DTM | 상담예약일시 | 업무데이터 |
| PURPOSE\_CD | 상담목적코드 | 코드 |
| MEMO\_CONTENT | 예약 메모 | 업무데이터 |
| STATUS\_CD | 예약상태 | 상태 |
| BRANCH\_ID | 등록 지점 | 데이터권한 |
| OWNER\_USER\_ID | 담당 사용자 | 데이터권한 |
| VERSION\_NO | Version | 동시성 |
| CREATED\_BY | 생성자 | 감사 |
| CREATED\_DTM | 생성일시 | 감사 |
| UPDATED\_BY | 변경자 | 감사 |
| UPDATED\_DTM | 변경일시 | 감사 |
...
| BEFORE\_STATUS\_CD | 이전 상태 |
| AFTER\_STATUS\_CD | 이후 상태 |
...
학습 목표 | 192장 요약의 핵심 개념을 설명하고, 프로젝트에서 적용 위치와 검증 방법을 스스로 판단한다.
...
학습 목표 | 193장. 업무 개념·상태·규칙 설계의 핵심 개념을 설명하고, 프로젝트에서 적용 위치와 검증 방법을 스스로 판단한다.
...
필요에 따라 IN\_PROGRESS, NO\_SHOW 등을 추가할 수 있지만, 이번 실습에서는 세 가지 상태만 사용합니다.
...
BRANCH\_ID = 인증 Context의 지점

OWNER\_USER\_ID = 인증 Context의 사용자
...
학습 목표 | 193장 요약의 핵심 개념을 설명하고, 프로젝트에서 적용 위치와 검증 방법을 스스로 판단한다.
...
학습 목표 | 194장. 화면 이벤트·ServiceId·거래코드 설계의 핵심 개념을 설명하고, 프로젝트에서 적용 위치와 검증 방법을 스스로 판단한다.
...
학습 목표 | 194장 요약의 핵심 개념을 설명하고, 프로젝트에서 적용 위치와 검증 방법을 스스로 판단한다.
...
학습 목표 | 195장. 패키지·프로그램·DTO 구조 설계의 핵심 개념을 설명하고, 프로젝트에서 적용 위치와 검증 방법을 스스로 판단한다.
...
학습 목표 | 195장 요약의 핵심 개념을 설명하고, 프로젝트에서 적용 위치와 검증 방법을 스스로 판단한다.
...
학습 목표 | 196장. Handler·Facade 구현의 핵심 개념을 설명하고, 프로젝트에서 적용 위치와 검증 방법을 스스로 판단한다.
...
return Set.of(
CtReservationServiceIds.SELECT\_LIST,
CtReservationServiceIds.SELECT\_DETAIL,
...
case CtReservationServiceIds.SELECT\_LIST ->
facade.selectList(
converter.convert(
body,
...
case CtReservationServiceIds.SELECT\_DETAIL ->
facade.selectDetail(
converter.convert(
body,
...
학습 목표 | 196장 요약의 핵심 개념을 설명하고, 프로젝트에서 적용 위치와 검증 방법을 스스로 판단한다.
...
학습 목표 | 197장. Service·Rule 구현의 핵심 개념을 설명하고, 프로젝트에서 적용 위치와 검증 방법을 스스로 판단한다.
...
request,
context.getBranchId(),
context.hasRole("CT\_RESERVATION\_ADMIN")
);
...
학습 목표 | 197장 요약의 핵심 개념을 설명하고, 프로젝트에서 적용 위치와 검증 방법을 스스로 판단한다.
...
학습 목표 | 198장. DAO·Mapper·SQL 구현의 핵심 개념을 설명하고, 프로젝트에서 적용 위치와 검증 방법을 스스로 판단한다.
...
AND RESERVATION\_DTM >= #{fromDateTime}
AND RESERVATION\_DTM < #{toDateTimeExclusive}
<if test="statusCode != null
and statusCode != ''">
AND STATUS\_CD = #{statusCode}
</if>
<if test="admin == false">
...
SET RESERVATION\_DTM = #{reservationDateTime}
, PURPOSE\_CD = #{purposeCode}
, MEMO\_CONTENT = #{memoContent}
, VERSION\_NO = VERSION\_NO + 1
, UPDATED\_BY = #{updatedBy}
, UPDATED\_DTM = #{updatedDateTime}
WHERE RESERVATION\_ID = #{reservationId}
...
INSERT INTO CT\_CONTACT\_RESERVATION\_HISTORY
(
HISTORY\_ID,
RESERVATION\_ID,
CHANGE\_TYPE\_CD,
...
조회 Index
CUSTOMER\_NO, RESERVATION\_DTM

지점 조회
BRANCH\_ID, RESERVATION\_DTM

중복 검사용
CUSTOMER\_NO, RESERVATION\_DTM, STATUS\_CD

실제 데이터 분포와 실행계획을 기준으로 결정합니다.

## **제198장 요약**

학습 목표 | 198장 요약의 핵심 개념을 설명하고, 프로젝트에서 적용 위치와 검증 방법을 스스로 판단한다.
...
학습 목표 | 199장. 보안·동시성·Idempotency·감사 구현의 핵심 개념을 설명하고, 프로젝트에서 적용 위치와 검증 방법을 스스로 판단한다.
...
학습 목표 | 199장 요약의 핵심 개념을 설명하고, 프로젝트에서 적용 위치와 검증 방법을 스스로 판단한다.
...
학습 목표 | 200장. 정상·오류·Timeout 처리 흐름의 핵심 개념을 설명하고, 프로젝트에서 적용 위치와 검증 방법을 스스로 판단한다.
...
학습 목표 | 200장 요약의 핵심 개념을 설명하고, 프로젝트에서 적용 위치와 검증 방법을 스스로 판단한다.
...
학습 목표 | 201장. 단위·Mapper·통합·E2E 테스트의 핵심 개념을 설명하고, 프로젝트에서 적용 위치와 검증 방법을 스스로 판단한다.
...
학습 목표 | 201장 요약의 핵심 개념을 설명하고, 프로젝트에서 적용 위치와 검증 방법을 스스로 판단한다.
...
학습 목표 | 202장. OM·모니터링·배포 준비의 핵심 개념을 설명하고, 프로젝트에서 적용 위치와 검증 방법을 스스로 판단한다.
...
학습 목표 | 202장 요약의 핵심 개념을 설명하고, 프로젝트에서 적용 위치와 검증 방법을 스스로 판단한다.
...
학습 목표 | 203장. 통합 추적성·산출물 완성의 핵심 개념을 설명하고, 프로젝트에서 적용 위치와 검증 방법을 스스로 판단한다.
...
학습 목표 | 203장 요약의 핵심 개념을 설명하고, 프로젝트에서 적용 위치와 검증 방법을 스스로 판단한다.
...
학습 목표 | 204장. AI 코딩도구를 이용한 CRUD 구현의 핵심 개념을 설명하고, 프로젝트에서 적용 위치와 검증 방법을 스스로 판단한다.
...
학습 목표 | 204장 요약의 핵심 개념을 설명하고, 프로젝트에서 적용 위치와 검증 방법을 스스로 판단한다.
...
VERSION\_NO 기반 낙관적 동시성 제어를 적용한다.
