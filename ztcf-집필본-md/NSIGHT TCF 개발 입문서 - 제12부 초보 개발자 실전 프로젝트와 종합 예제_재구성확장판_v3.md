<!-- source: ztcf-집필본/NSIGHT TCF 개발 입문서 - 제12부 초보 개발자 실전 프로젝트와 종합 예제_재구성확장판_v3.docx -->
<!-- tables: mammoth-html + placeholder table-to-md -->

나는 제12부. 초보 개발자 실전 프로젝트와 종합 예제의 개념을 코드·설정·로그·산출물 중 어디에서 확인할 수 있는가? 문제가 발생했을 때 어느 경계부터 조사해야 하는가?
...
1.  화면과 테이블 정보를 분석하여 업무 범위를 정의한다.
2.  화면 이벤트를 독립적인 ServiceId로 분리한다.
3.  화면 ID·이벤트 ID·거래코드를 발급한다.
4.  요구사항과 추적성 매트릭스를 작성한다.
5.  패키지와 프로그램 골격을 구성한다.
6.  Request·Response·Command·Query·Data DTO를 구분한다.
7.  Validation과 업무 Rule을 구현한다.
8.  Handler·Facade·Service 책임을 분리한다.
9.  DAO·Mapper·SQL을 구현한다.
10.  트랜잭션·Rollback·동시성 제어를 적용한다.
11.  인증·권한·데이터권한을 적용한다.
12.  오류·Timeout·중복요청을 처리한다.
13.  단위·Mapper·거래 테스트를 작성한다.
14.  OM 기준정보와 운영항목을 등록한다.
15.  빌드·배포·Smoke Test·운영인수를 수행한다.
16.  AI 개발도구를 사용할 때 입력해야 할 설계정보를 정리한다.
...
| 대상 고객 | 고객번호 기준 |
| 주요 테이블 | CT\_CONTACT\_MASTER |
| 이력 테이블 | CT\_CONTACT\_HISTORY |
...
학습 목표 | 97장. 화면과 테이블 정보 분석하기의 핵심 개념을 설명하고, 프로젝트에서 적용 위치와 검증 방법을 스스로 판단한다.
...
| **컬럼** | **유형** | **설명** |
| --- | --- | --- |
| CONTACT\_ID | VARCHAR2 | 상담이력 ID |
| CUSTOMER\_NO | VARCHAR2 | 고객번호 |
| CONTACT\_TYPE\_CD | VARCHAR2 | 상담구분 |
| CONTACT\_TITLE | VARCHAR2 | 상담제목 |
| CONTACT\_CONTENT | CLOB | 상담내용 |
| PROCESS\_RESULT | VARCHAR2 | 처리결과 |
| OPEN\_YN | CHAR | 공개 여부 |
| STATUS\_CD | VARCHAR2 | 상태 |
| CONTACT\_DTM | TIMESTAMP | 상담일시 |
| BRANCH\_ID | VARCHAR2 | 등록지점 |
| VERSION\_NO | NUMBER | Version |
| CREATE\_USER\_ID | VARCHAR2 | 등록자 |
| CREATE\_DTM | TIMESTAMP | 등록일시 |
| UPDATE\_USER\_ID | VARCHAR2 | 수정자 |
| UPDATE\_DTM | TIMESTAMP | 수정일시 |

### **CT\_CONTACT\_HISTORY**

| **컬럼** | **설명** |
| --- | --- |
| HISTORY\_ID | 이력 식별자 |
| CONTACT\_ID | 상담이력 ID |
| CHANGE\_TYPE\_CD | CREATE·UPDATE·DEACTIVATE |
| BEFORE\_DATA | 변경 전 데이터 |
| AFTER\_DATA | 변경 후 데이터 |
| CHANGE\_USER\_ID | 변경 사용자 |
| CHANGE\_DTM | 변경일시 |
| TRACE\_ID | 거래 추적 ID |
...
CT가 IC\_CUSTOMER\_MASTER를 직접 조회하지 않습니다.

## **제97장 요약**

학습 목표 | 97장 요약의 핵심 개념을 설명하고, 프로젝트에서 적용 위치와 검증 방법을 스스로 판단한다.
...
학습 목표 | 98장. 요구사항과 추적성 식별자 정의하기의 핵심 개념을 설명하고, 프로젝트에서 적용 위치와 검증 방법을 스스로 판단한다.
...
| **기능** | **권한코드** |
| --- | --- |
| 목록·상세조회 | CT\_CONTACT\_VIEW |
| 등록 | CT\_CONTACT\_CREATE |
| 변경 | CT\_CONTACT\_UPDATE |
| 사용중지 | CT\_CONTACT\_DEACTIVATE |
...
| --- | --- | --- | --- | --- |
| REQ-CT-0001 | E01 | CT.Contact.selectList | CT-INQ-0001 | CT\_CONTACT\_VIEW |
| REQ-CT-0002 | E02 | CT.Contact.selectDetail | CT-INQ-0002 | CT\_CONTACT\_VIEW |
| REQ-CT-0003 | E03 | CT.Contact.create | CT-REG-0001 | CT\_CONTACT\_CREATE |
| REQ-CT-0004 | E04 | CT.Contact.update | CT-UPD-0001 | CT\_CONTACT\_UPDATE |
| REQ-CT-0005 | E05 | CT.Contact.deactivate | CT-UPD-0002 | CT\_CONTACT\_DEACTIVATE |

## **제98장 요약**

학습 목표 | 98장 요약의 핵심 개념을 설명하고, 프로젝트에서 적용 위치와 검증 방법을 스스로 판단한다.
...
학습 목표 | 99장. 설계서와 프로그램 목록 작성하기의 핵심 개념을 설명하고, 프로젝트에서 적용 위치와 검증 방법을 스스로 판단한다.
...
학습 목표 | 99장 요약의 핵심 개념을 설명하고, 프로젝트에서 적용 위치와 검증 방법을 스스로 판단한다.
...
학습 목표 | 100장. 프로젝트와 패키지 골격 만들기의 핵심 개념을 설명하고, 프로젝트에서 적용 위치와 검증 방법을 스스로 판단한다.
...
학습 목표 | 100장 요약의 핵심 개념을 설명하고, 프로젝트에서 적용 위치와 검증 방법을 스스로 판단한다.
...
학습 목표 | 101장. 요청·응답 DTO 설계하기의 핵심 개념을 설명하고, 프로젝트에서 적용 위치와 검증 방법을 스스로 판단한다.
...
학습 목표 | 101장 요약의 핵심 개념을 설명하고, 프로젝트에서 적용 위치와 검증 방법을 스스로 판단한다.
...
학습 목표 | 102장. Validation과 업무 Rule 구현하기의 핵심 개념을 설명하고, 프로젝트에서 적용 위치와 검증 방법을 스스로 판단한다.
...
학습 목표 | 102장 요약의 핵심 개념을 설명하고, 프로젝트에서 적용 위치와 검증 방법을 스스로 판단한다.
...
학습 목표 | 103장. Handler 구현하기의 핵심 개념을 설명하고, 프로젝트에서 적용 위치와 검증 방법을 스스로 판단한다.
...
return Set.of(
CtContactServiceIds.SELECT\_LIST,
CtContactServiceIds.SELECT\_DETAIL,
...
case CtContactServiceIds.SELECT\_DETAIL ->
...
학습 목표 | 103장 요약의 핵심 개념을 설명하고, 프로젝트에서 적용 위치와 검증 방법을 스스로 판단한다.
...
학습 목표 | 104장. Facade와 Service 구현하기의 핵심 개념을 설명하고, 프로젝트에서 적용 위치와 검증 방법을 스스로 판단한다.
...
학습 목표 | 104장 요약의 핵심 개념을 설명하고, 프로젝트에서 적용 위치와 검증 방법을 스스로 판단한다.
...
학습 목표 | 105장. DAO·Mapper·SQL 구현하기의 핵심 개념을 설명하고, 프로젝트에서 적용 위치와 검증 방법을 스스로 판단한다.
...
AND CONTACT\_DTM >= #{fromDtm}
AND CONTACT\_DTM < #{toDtmExclusive}
AND STATUS\_CD = 'ACTIVE'
AND (
OPEN\_YN = 'Y'
...
and contactTypeCode != ''">
AND CONTACT\_TYPE\_CD =
#{contactTypeCode}
</if>

ORDER BY CONTACT\_DTM DESC,
CONTACT\_ID DESC
...
INSERT INTO CT\_CONTACT\_MASTER
(
CONTACT\_ID,
CUSTOMER\_NO,
CONTACT\_TYPE\_CD,
...
UPDATE CT\_CONTACT\_MASTER
SET CONTACT\_TYPE\_CD = #{contactTypeCode},
CONTACT\_TITLE = #{contactTitle},
CONTACT\_CONTENT = #{contactContent},
PROCESS\_RESULT = #{processResult},
OPEN\_YN = #{openYn},
VERSION\_NO = VERSION\_NO + 1,
UPDATE\_USER\_ID = #{updateUserId},
UPDATE\_DTM = CURRENT\_TIMESTAMP
WHERE CONTACT\_ID = #{contactId}
...
INSERT INTO CT\_CONTACT\_HISTORY
(
HISTORY\_ID,
CONTACT\_ID,
CHANGE\_TYPE\_CD,
...
IX\_CT\_CONTACT\_MASTER\_01
(CUSTOMER\_NO, STATUS\_CD, CONTACT\_DTM DESC)
...
학습 목표 | 105장 요약의 핵심 개념을 설명하고, 프로젝트에서 적용 위치와 검증 방법을 스스로 판단한다.
...
학습 목표 | 106장. 인증·권한·데이터권한 적용하기의 핵심 개념을 설명하고, 프로젝트에서 적용 위치와 검증 방법을 스스로 판단한다.
...
CT\_CONTACT\_UPDATE\_ALL\_BRANCH
...
학습 목표 | 106장 요약의 핵심 개념을 설명하고, 프로젝트에서 적용 위치와 검증 방법을 스스로 판단한다.
...
학습 목표 | 107장. 오류·Timeout·중복·동시성 처리의 핵심 개념을 설명하고, 프로젝트에서 적용 위치와 검증 방법을 스스로 판단한다.
...
학습 목표 | 107장 요약의 핵심 개념을 설명하고, 프로젝트에서 적용 위치와 검증 방법을 스스로 판단한다.
...
학습 목표 | 108장. 표준 요청과 응답 예제의 핵심 개념을 설명하고, 프로젝트에서 적용 위치와 검증 방법을 스스로 판단한다.
...
학습 목표 | 109장. 테스트 작성하기의 핵심 개념을 설명하고, 프로젝트에서 적용 위치와 검증 방법을 스스로 판단한다.
...
학습 목표 | 109장 요약의 핵심 개념을 설명하고, 프로젝트에서 적용 위치와 검증 방법을 스스로 판단한다.
...
학습 목표 | 110장. OM 등록과 운영 준비의 핵심 개념을 설명하고, 프로젝트에서 적용 위치와 검증 방법을 스스로 판단한다.
...
| --- | --- | --- | --- |
| CT.Contact.selectList | ACTIVE | 3,000ms | CT\_CONTACT\_VIEW |
| CT.Contact.selectDetail | ACTIVE | 3,000ms | CT\_CONTACT\_VIEW |
| CT.Contact.create | ACTIVE | 5,000ms | CT\_CONTACT\_CREATE |
| CT.Contact.update | ACTIVE | 5,000ms | CT\_CONTACT\_UPDATE |
| CT.Contact.deactivate | ACTIVE | 5,000ms | CT\_CONTACT\_DEACTIVATE |
...
학습 목표 | 110장 요약의 핵심 개념을 설명하고, 프로젝트에서 적용 위치와 검증 방법을 스스로 판단한다.
...
학습 목표 | 111장. 빌드·배포·Smoke Test의 핵심 개념을 설명하고, 프로젝트에서 적용 위치와 검증 방법을 스스로 판단한다.
...
학습 목표 | 111장 요약의 핵심 개념을 설명하고, 프로젝트에서 적용 위치와 검증 방법을 스스로 판단한다.
...
학습 목표 | 112장. 화면과 테이블 정보만 있을 때의 표준 개발절차의 핵심 개념을 설명하고, 프로젝트에서 적용 위치와 검증 방법을 스스로 판단한다.
...
학습 목표 | 112장 요약의 핵심 개념을 설명하고, 프로젝트에서 적용 위치와 검증 방법을 스스로 판단한다.
...
학습 목표 | 113장. AI 개발도구를 활용하는 방법의 핵심 개념을 설명하고, 프로젝트에서 적용 위치와 검증 방법을 스스로 판단한다.
...
\- CT\_CONTACT\_MASTER와 CT\_CONTACT\_HISTORY를 같은 트랜잭션으로 저장한다.
...
학습 목표 | 113장 요약의 핵심 개념을 설명하고, 프로젝트에서 적용 위치와 검증 방법을 스스로 판단한다.
...
| 목록 거래코드 | CT-INQ-0001 |
| 목록 권한 | CT\_CONTACT\_VIEW |
| 등록 ServiceId | CT.Contact.create |
| 등록 거래코드 | CT-REG-0001 |
| 등록 권한 | CT\_CONTACT\_CREATE |
...
| **유형** | **이름** |
| --- | --- |
| Master | CT\_CONTACT\_MASTER |
| History | CT\_CONTACT\_HISTORY |
...
DELETE FROM CT\_CONTACT\_MASTER
