<!-- source: ztcf-집필본/NSIGHT TCF 개발 입문서 - 제5부 오류처리와 안정적인 거래 제어_재구성확장판_v3.docx -->
<!-- tables: mammoth-html + placeholder table-to-md -->

나는 제5부. 오류처리와 안정적인 거래 제어의 개념을 코드·설정·로그·산출물 중 어디에서 확인할 수 있는가? 문제가 발생했을 때 어느 경계부터 조사해야 하는가?
...
1.  오류를 입력·인증·권한·통제·업무·시스템·Timeout으로 구분한다.
2.  업무 예외와 시스템 예외를 구분하여 발생시킨다.
3.  표준 오류코드를 설계하고 오류 응답을 작성한다.
4.  계층별 예외 책임을 이해한다.
5.  ETF를 통해 거래 종료 상태를 일관되게 기록한다.
6.  ServiceId별 Timeout 정책을 적용한다.
7.  SQL과 외부 연계 Timeout을 함께 설계한다.
8.  OM 거래통제로 Handler 실행 전 거래를 차단한다.
9.  Idempotency Key를 이용하여 중복 변경을 방지한다.
10.  재시도 가능한 오류와 재시도하면 안 되는 오류를 구분한다.
11.  GUID·TraceId·ServiceId로 전체 실행 흐름을 추적한다.
12.  장애 발생 시 로그·Thread·DB Pool·SQL 순서로 원인을 찾는다.
...
학습 목표 | 29장. 오류를 제대로 구분하기의 핵심 개념을 설명하고, 프로젝트에서 적용 위치와 검증 방법을 스스로 판단한다.
...
학습 목표 | 29장 요약의 핵심 개념을 설명하고, 프로젝트에서 적용 위치와 검증 방법을 스스로 판단한다.
...
학습 목표 | 30장. 표준 예외와 오류코드 설계의 핵심 개념을 설명하고, 프로젝트에서 적용 위치와 검증 방법을 스스로 판단한다.
...
throw new BusinessException(
CmCampaignError.CAMPAIGN\_NOT\_FOUND
);
...
학습 목표 | 30장 요약의 핵심 개념을 설명하고, 프로젝트에서 적용 위치와 검증 방법을 스스로 판단한다.
...
학습 목표 | 31장. 계층별 예외처리와 ETF의 핵심 개념을 설명하고, 프로젝트에서 적용 위치와 검증 방법을 스스로 판단한다.
...
| 업무 오류 | BUSINESS\_FAIL | FAIL | 업무 오류 건수 | 업무 오류 |
| 시스템 오류 | SYSTEM\_ERROR | FAIL | 시스템 오류 건수 | 시스템 오류 |
...
학습 목표 | 31장 요약의 핵심 개념을 설명하고, 프로젝트에서 적용 위치와 검증 방법을 스스로 판단한다.
...
학습 목표 | 32장. Timeout 설계와 구현의 핵심 개념을 설명하고, 프로젝트에서 적용 위치와 검증 방법을 스스로 판단한다.
...
학습 목표 | 32장 요약의 핵심 개념을 설명하고, 프로젝트에서 적용 위치와 검증 방법을 스스로 판단한다.
...
학습 목표 | 33장. 거래통제와 실행 차단의 핵심 개념을 설명하고, 프로젝트에서 적용 위치와 검증 방법을 스스로 판단한다.
...
"errorType": "TRANSACTION\_CONTROL",
"retryable": true
}
}
...
학습 목표 | 33장 요약의 핵심 개념을 설명하고, 프로젝트에서 적용 위치와 검증 방법을 스스로 판단한다.
...
학습 목표 | 34장. Idempotency·재시도·재처리의 핵심 개념을 설명하고, 프로젝트에서 적용 위치와 검증 방법을 스스로 판단한다.
...
학습 목표 | 34장 요약의 핵심 개념을 설명하고, 프로젝트에서 적용 위치와 검증 방법을 스스로 판단한다.
...
학습 목표 | 35장. 거래로그·감사로그·추적 ID의 핵심 개념을 설명하고, 프로젝트에서 적용 위치와 검증 방법을 스스로 판단한다.
...
TxLog.end
status = SUCCESS | BUSINESS\_FAIL |
SYSTEM\_ERROR | TIMEOUT | REJECTED
endDtm
elapsedMs
resultCode
...
학습 목표 | 35장 요약의 핵심 개념을 설명하고, 프로젝트에서 적용 위치와 검증 방법을 스스로 판단한다.
...
학습 목표 | 36장. 장애 흐름과 운영 대응의 핵심 개념을 설명하고, 프로젝트에서 적용 위치와 검증 방법을 스스로 판단한다.
...
학습 목표 | 36장 요약의 핵심 개념을 설명하고, 프로젝트에서 적용 위치와 검증 방법을 스스로 판단한다.
