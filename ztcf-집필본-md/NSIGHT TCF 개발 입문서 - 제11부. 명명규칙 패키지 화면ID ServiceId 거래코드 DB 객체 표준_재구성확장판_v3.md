<!-- source: ztcf-집필본/NSIGHT TCF 개발 입문서 - 제11부. 명명규칙 패키지 화면ID ServiceId 거래코드 DB 객체 표준_재구성확장판_v3.docx -->
<!-- tables: mammoth-html + placeholder table-to-md -->

나는 제11부. 명명규칙·패키지·화면 ID·ServiceId·거래코드·DB 객체 표준의 개념을 코드·설정·로그·산출물 중 어디에서 확인할 수 있는가? 문제가 발생했을 때 어느 경계부터 조사해야 하는가?
...
1.  업무코드와 도메인 경계를 정의한다.
2.  업무세구분코드를 설계한다.
3.  모듈·WAR·Context Path를 일관되게 구성한다.
4.  Java Package를 표준 계층으로 구성한다.
5.  화면 ID와 화면 이벤트 ID를 정의한다.
6.  메뉴 ID와 화면 ID를 구분한다.
7.  ServiceId를 업무·도메인·행위 기준으로 정의한다.
8.  거래코드를 처리유형에 따라 부여한다.
9.  Handler·Facade·Service·Rule·DAO·Mapper 이름을 정의한다.
10.  Request·Response·Command·Data DTO를 구분한다.
11.  Mapper 메서드와 SQL ID를 일치시킨다.
12.  테이블·컬럼·인덱스·제약조건 이름을 정의한다.
13.  파일·Batch·Cache·환경설정 이름을 정의한다.
14.  화면부터 DB까지 추적성 매트릭스를 작성한다.
15.  명명규칙 위반을 CI 품질 Gate로 자동검증한다.
...
학습 목표 | 86장. 명명규칙이 필요한 이유의 핵심 개념을 설명하고, 프로젝트에서 적용 위치와 검증 방법을 스스로 판단한다.
...
학습 목표 | 86장 요약의 핵심 개념을 설명하고, 프로젝트에서 적용 위치와 검증 방법을 스스로 판단한다.
...
학습 목표 | 87장. 업무코드·도메인·업무세구분코드의 핵심 개념을 설명하고, 프로젝트에서 적용 위치와 검증 방법을 스스로 판단한다.
...
학습 목표 | 87장 요약의 핵심 개념을 설명하고, 프로젝트에서 적용 위치와 검증 방법을 스스로 판단한다.
...
학습 목표 | 88장. 모듈·WAR·Context Path·패키지 표준의 핵심 개념을 설명하고, 프로젝트에서 적용 위치와 검증 방법을 스스로 판단한다.
...
학습 목표 | 88장 요약의 핵심 개념을 설명하고, 프로젝트에서 적용 위치와 검증 방법을 스스로 판단한다.
...
학습 목표 | 89장. 화면 ID·화면그룹·이벤트 ID·메뉴 ID의 핵심 개념을 설명하고, 프로젝트에서 적용 위치와 검증 방법을 스스로 판단한다.
...
학습 목표 | 89장 요약의 핵심 개념을 설명하고, 프로젝트에서 적용 위치와 검증 방법을 스스로 판단한다.
...
학습 목표 | 90장. ServiceId 설계 표준의 핵심 개념을 설명하고, 프로젝트에서 적용 위치와 검증 방법을 스스로 판단한다.
...
학습 목표 | 90장 요약의 핵심 개념을 설명하고, 프로젝트에서 적용 위치와 검증 방법을 스스로 판단한다.
...
학습 목표 | 91장. 거래코드 설계 표준의 핵심 개념을 설명하고, 프로젝트에서 적용 위치와 검증 방법을 스스로 판단한다.
...
학습 목표 | 91장 요약의 핵심 개념을 설명하고, 프로젝트에서 적용 위치와 검증 방법을 스스로 판단한다.
...
학습 목표 | 92장. Java 클래스·인터페이스·메서드·DTO 명명의 핵심 개념을 설명하고, 프로젝트에서 적용 위치와 검증 방법을 스스로 판단한다.
...
학습 목표 | 92장 요약의 핵심 개념을 설명하고, 프로젝트에서 적용 위치와 검증 방법을 스스로 판단한다.
...
학습 목표 | 93장. DAO·Mapper·SQL ID 명명의 핵심 개념을 설명하고, 프로젝트에서 적용 위치와 검증 방법을 스스로 판단한다.
...
학습 목표 | 93장 요약의 핵심 개념을 설명하고, 프로젝트에서 적용 위치와 검증 방법을 스스로 판단한다.
...
학습 목표 | 94장. DB 테이블·컬럼·인덱스·제약조건 표준의 핵심 개념을 설명하고, 프로젝트에서 적용 위치와 검증 방법을 스스로 판단한다.
...
CM\_CAMPAIGN\_MASTER
CM\_CAMPAIGN\_HISTORY
CM\_CAMPAIGN\_TARGET
SV\_CUSTOMER\_SUMMARY
OM\_SERVICE\_CATALOG
OM\_TRANSACTION\_LOG
...
| 금액 | \_AMT | LIMIT\_AMT |
| 수량 | \_QTY | TARGET\_QTY |
| 건수 | \_CNT | CUSTOMER\_CNT |
| 여부 | \_YN | USE\_YN |
| 순번 | \_SEQ | DISPLAY\_SEQ |
| 비율 | \_RATE | RESPONSE\_RATE |
| 버전 | \_VER 또는 \_VERSION\_NO | 프로젝트 표준 선택 |
...
동일 테이블에서 USE\_YN, DELETE\_YN, STATUS\_CD를 중복 사용하면 상태 의미가 충돌할 수 있습니다.
...
PK\_CM\_CAMPAIGN\_MASTER

DB 길이 제한이 있다면 공식 축약규칙을 적용합니다.

## **94.12 Foreign Key 이름**

FK\_{자식테이블약어}\_{부모테이블약어}\_{순번}

예:

FK\_CM\_CAM\_TGT\_CM\_CAM\_01
...
학습 목표 | 94장 요약의 핵심 개념을 설명하고, 프로젝트에서 적용 위치와 검증 방법을 스스로 판단한다.
...
학습 목표 | 95장. Batch·파일·Cache·환경설정·로그 명명의 핵심 개념을 설명하고, 프로젝트에서 적용 위치와 검증 방법을 스스로 판단한다.
...
SV\_CUSTOMER\_EXPORT\_20260717\_103000.csv
...
PRD:OM:SERVICE\_POLICY:SV.Customer.selectSummary
...
TCF\_TRANSACTION\_DEFAULT\_TIMEOUT\_MS
...
tcf\_transaction\_requests\_total

tcf\_transaction\_duration\_seconds

tcf\_db\_pool\_pending

tcf\_batch\_job\_failures\_total
...
학습 목표 | 95장 요약의 핵심 개념을 설명하고, 프로젝트에서 적용 위치와 검증 방법을 스스로 판단한다.
...
학습 목표 | 96장. 통합 추적성 매트릭스와 자동검증의 핵심 개념을 설명하고, 프로젝트에서 적용 위치와 검증 방법을 스스로 판단한다.
...
| 테이블 | SV\_CUSTOMER\_SUMMARY |
| 권한 | SV\_CUSTOMER\_VIEW |
| Timeout | 3000ms |
| 담당자 | SV 업무팀 |
...
학습 목표 | 96장 요약의 핵심 개념을 설명하고, 프로젝트에서 적용 위치와 검증 방법을 스스로 판단한다.
...
| 권한코드 | {업무}\_{도메인}\_{행위} | SV\_CUSTOMER\_VIEW |
| SQL ID | {업무}-{도메인}-{유형}-{번호} | SV-CUST-SEL-001 |
| WAR | {업무소문자}-service | sv-service |
| Context | /{업무소문자} | /sv |
| 테이블 | {업무}\_{객체}\_{유형} | SV\_CUSTOMER\_SUMMARY |
