...
:gradle_ok
if not exist "!GRADLE!" (
    echo [oc-build] gradle not found.
    exit /b 1
)
...
echo [oc-build] Stop Gradle daemons...
call gradle --stop >nul 2>&1

cd /d "!PROJECT_HOME!"
echo [oc-build] !GRADLE! !TASKS!
call "!GRADLE!" !TASKS!
if errorlevel 1 exit /b %errorlevel%

if /i "%~1"=="run" exit /b 0

set "WAR_FILE=!PROJECT_HOME!\!MODULE!\build\libs\oc.war"
echo.
echo [oc-build] Build output:
if exist "!WAR_FILE!" (
    echo   [OK] oc.war
    dir "!WAR_FILE!"
) else (
    echo   [MISSING] oc.war - not found in !MODULE!\build\libs
    exit /b 1
)
exit /b 0
...
echo   build.bat        Build tcf-util/core/web + oc.war
