...
goto :webapps_ok
:webapps_missing
echo [oc-deploy] webapps directory not found: !WEBAPPS!
exit /b 1
:webapps_ok
...
goto :gradle_found
:gradle_missing
echo [oc-deploy] gradle not found.
exit /b 1
:gradle_found
...
if not exist "!SRC!" (
    echo [oc-deploy] WAR not found: !SRC!
    exit /b 1
)
...
:verify_failed
echo [oc-deploy] Verification failed: !DEST_WAR! not found in !WEBAPPS!
exit /b 1

:usage
echo Usage: deploy.bat
echo   Build oc.war and deploy to Tomcat webapps ^(/oc^)
echo.
echo Target webapps:
echo   !WEBAPPS!
