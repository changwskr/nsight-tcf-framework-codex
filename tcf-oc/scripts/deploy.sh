...
resolve_gradle() {
  ...
  echo "[oc-deploy] gradle not found." >&2
  exit 1
}
...
if [[ "${1:-}" == "help" || "${1:-}" == "-h" || "${1:-}" == "--help" ]]; then
  cat <<EOF
Usage: deploy.sh
  Build oc.war and deploy to Tomcat webapps (/oc)
...
[[ -d "${WEBAPPS}" ]] || { echo "[oc-deploy] webapps not found: ${WEBAPPS}" >&2; exit 1; }

resolve_gradle

echo "[oc-deploy] Building ${SRC_WAR} ..."
(
  cd "${PROJECT_HOME}"
...
[[ -f "${src}" ]] || { echo "[oc-deploy] WAR not found: ${src}" >&2; exit 1; }

rm -rf "${WEBAPPS}/${CTX}"
echo "[oc-deploy] Copying ${DEST_WAR} to ${WEBAPPS} ..."
cp -f "${src}" "${WEBAPPS}/${DEST_WAR}"
echo "  deployed ${DEST_WAR}"

echo
echo "[oc-deploy] Verifying deployed WAR ..."
if [[ -f "${WEBAPPS}/${DEST_WAR}" ]]; then
  echo "  [OK] ${DEST_WAR}"
  ls -lh "${WEBAPPS}/${DEST_WAR}"
  echo
  echo "[oc-deploy] Done. Tomcat running: /oc redeploys automatically (~15s)."
else
  echo "[oc-deploy] Verification failed: ${DEST_WAR} not found in ${WEBAPPS}" >&2
  exit 1
fi
