...
resolve_gradle() {
  ...
  echo "[oc-build] gradle not found." >&2
  exit 1
}
...
Usage: build.sh [clean|run]
  build.sh        Build tcf-util/core/web + oc.war
...
war_file="${PROJECT_HOME}/${MODULE}/build/libs/oc.war"
echo
...
if [[ -f "${war_file}" ]]; then
  ...
  echo "  [MISSING] oc.war - not found in ${MODULE}/build/libs" >&2
  exit 1
fi
