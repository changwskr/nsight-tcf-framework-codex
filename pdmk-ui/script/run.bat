...
rem ============================================================
rem  pdmk-ui run script (port 8090)
rem
rem  Usage:
rem    run.bat                                   gradle bootRun
rem    run.bat jar                               run build\libs\pdmk-ui.jar
rem    run.bat jar --pdmk.ui.target-base-url=http://localhost:8081
rem
...
echo  TARGET  : pdmk-service http://localhost:8080
echo ------------------------------------------------------------
echo.
...
:run_jar
shift
if not exist "build\libs\pdmk-ui.jar" goto :no_jar
...
java -Dfile.encoding=UTF-8 -jar "build\libs\pdmk-ui.jar" %*
set "RUN_RESULT=%ERRORLEVEL%"
...
:no_jar
echo [ERROR] build\libs\pdmk-ui.jar not found - run script\build.bat first
popd
endlocal & exit /b 1
