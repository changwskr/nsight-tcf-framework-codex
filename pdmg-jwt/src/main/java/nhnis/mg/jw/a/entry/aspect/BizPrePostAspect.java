package nhnis.mg.jw.a.entry.aspect;
...
/**
 * JWT(jw.a) 업무 공통 선/후처리 Aspect.
 *
 * <p>패키지·클래스명을 운영 로그와 동일하게 {@code nhnis.mg.jw.a.entry.aspect.BizPrePostAspect} 로 둔다.
 * {@code log.info} 는 이 클래스에서 직접 호출해 {@code %C.%M} 위치가 맞도록 한다.
 * pdmg-service {@code nhnis.mg.co.a.entry.aspect.BizPrePostAspect} 와 동일한 패턴이다.
 */
@Aspect
@Component
@Order(100)
...
public class BizPrePostAspect {
     ...
     */
    @Pointcut("execution(public * nhnis.mg.jw.a.application.service..*(..))")
    public void jwBizBoundary() {
        // pointcut
    }

    @Before("jwBizBoundary()")
    ...
    @AfterReturning(pointcut = "jwBizBoundary()", returning = "result")
