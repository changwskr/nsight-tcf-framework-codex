...
- OM의 운영관리 책임을 설명한다.
- Service Catalog와 거래통제 기준을 관리한다.
- GUID·TraceId·ServiceId로 거래를 추적한다.
- 거래로그에서 성공·업무 오류·시스템 오류·Timeout을 구분한다.
- Tomcat Thread 상태를 해석한다.
- JVM CPU·Heap·GC 상태를 해석한다.
- HikariCP Active·Idle·Pending 값을 해석한다.
- Slow SQL과 Mapper ID를 ServiceId에 연결한다.
- 특정 WAR의 자원 독점 여부를 판단한다.
- Metric·Log·Trace의 차이를 설명한다.
- 경보 임계값과 지속시간을 설계한다.
- 장애 시 우선 확인 순서를 적용한다.
- 거래통제·Rate Limit·Rollback으로 장애를 완화한다.
- 운영 조치를 감사로그로 남긴다.
- 장애 이후 재발방지 과제를 도출한다.
...
| 권한 | 기능 |
| --- | --- |
| OM_SERVICE_VIEW | 서비스 조회 |
| OM_SERVICE_CONTROL | 거래중지 |
| OM_TIMEOUT_UPDATE | Timeout 변경 |
| OM_BATCH_CONTROL | Batch 실행·중지 |
| OM_LOG_VIEW | 거래로그 조회 |
| OM_SECURITY_AUDIT | 보안감사 조회 |
...
| OM-002 | 업무 오류 | BUSINESS_FAIL |
| OM-003 | 시스템 오류 | SYSTEM_ERROR |
