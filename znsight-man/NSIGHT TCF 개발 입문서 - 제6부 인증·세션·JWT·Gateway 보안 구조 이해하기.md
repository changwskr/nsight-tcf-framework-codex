...
- 인증과 인가의 차이를 설명한다.
- 세션과 JWT의 차이를 설명한다.
- SSO 로그인부터 JWT 발급까지의 흐름을 이해한다.
- Access Token과 Refresh Token의 역할을 구분한다.
- JWT Claim의 의미와 검증항목을 이해한다.
- RS256의 Private Key와 Public Key 역할을 구분한다.
- Gateway에서 JWT를 검증하는 흐름을 이해한다.
- Gateway가 없을 때 업무 WAR의 검증책임을 설명한다.
- Tomcat 다중 WAR 환경의 Public Key 관리방식을 이해한다.
- 요청 Header와 AuthenticationContext의 정합성을 검사한다.
- 기능권한과 데이터권한을 구분한다.
- 인증·권한 오류와 보안감사로그를 설계한다.
...
권한 강제 변경 시 사용자 TOKEN_VERSION을 5로 증가시킵니다.
...
| 권한코드 | 설명 |
| --- | --- |
| SV_CUSTOMER_VIEW | 고객조회 |
| CM_CAMPAIGN_CREATE | 캠페인 등록 |
| CM_CAMPAIGN_UPDATE | 캠페인 변경 |
| CM_CAMPAIGN_APPROVE | 캠페인 승인 |
| OM_SERVICE_CONTROL | 거래통제 변경 |
...
```
event=AUTHORIZATION_FAIL
userId=user01
serviceId=CM.Campaign.approve
requiredAuthority=CM_CAMPAIGN_APPROVE
traceId=T...
```
...
| TOKEN_EXPIRED | 만료 정책·사용행태 |
| INVALID_SIGNATURE | 위조·키 불일치 |
| UNKNOWN_KID | 키 배포·Rotation 문제 |
| 권한 실패율 | 권한설정·비정상 접근 |
