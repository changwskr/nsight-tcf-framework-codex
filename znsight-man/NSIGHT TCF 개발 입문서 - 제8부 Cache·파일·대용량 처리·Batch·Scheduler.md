...
- Cache 적용 대상과 비적용 대상을 구분한다.
- Cache Key, TTL, 무효화 정책을 설계한다.
- 로컬 Cache와 분산 Cache의 차이를 설명한다.
- 파일 업로드와 다운로드의 보안 기준을 적용한다.
- 대용량 파일을 Streaming 방식으로 처리한다.
- 장시간 작업을 비동기 Job으로 전환한다.
- Job 상태와 진행률을 설계한다.
- Batch의 Reader·Processor·Writer 구조를 이해한다.
- Chunk 크기와 Commit 단위를 결정한다.
- Scheduler와 Batch 업무 로직을 분리한다.
- Batch 중복실행과 서버 다중화 문제를 통제한다.
- 실패 Job의 재시도·재시작·재처리를 구분한다.
- 대량 작업의 성능·용량·보안·감사를 검증한다.
- OM에서 Batch와 파일 작업을 운영할 수 있도록 설계한다.
...
```
공통코드 그룹
COMMON_CODE:CHANNEL_TYPE

고객등급
CUSTOMER_GRADE:CUST000001

ServiceId 정책
SERVICE_POLICY:SV.Customer.selectSummary
```
...
```
{환경}:{업무}:{데이터유형}:{식별자}
PRD:SV:SERVICE_POLICY:SV.Customer.selectSummary
```
...
```
CUSTOMER_LIST:{USER_SCOPE_HASH}:{SEARCH_HASH}:PAGE1
```
...
```
{
  "body": {
    ...
    "fileName": "customer_export_20260717.csv",
    "fileSize": 524288000,
