...
- 같은 도메인 내부 호출과 다른 업무 호출을 구분한다.
- 같은 WAR 내부 호출과 다른 WAR 원격 호출을 구분한다.
- 다른 업무의 DAO와 Mapper를 직접 호출하면 안 되는 이유를 설명한다.
- ServiceId 기반 업무 연계 계약을 설계한다.
- 공통 Client와 업무별 Client의 역할을 구분한다.
- GUID·TraceId·사용자 문맥을 하위 호출에 전달한다.
- 동기·비동기 연계 중 적절한 방식을 선택한다.
- 외부 시스템 오류를 내부 표준 오류로 변환한다.
- Timeout·재시도·Circuit Breaker를 계층적으로 적용한다.
- 분산 트랜잭션의 한계를 이해한다.
- 보상처리·Outbox·상태기반 재처리를 설계한다.
- 연계 계약 테스트와 장애 테스트를 작성한다.
- 운영자가 호출자와 피호출자를 추적할 수 있도록 로그를 설계한다.
...
public class IcCustomerClient {
    ...
    private static final String TRANSACTION_CODE =
        "IC-INQ-0001";
...
Consumer는 EVENT_ID를 기준으로 중복처리를 막아야 합니다.
