...
NSIGHT TCF 소스를 분석할 때는 파일 수보다 공식 식별자와 실행경로가 중요합니다.
...
- 저장소와 멀티모듈 구조를 빠르게 파악한다.
- 공통모듈과 업무모듈을 구분한다.
- 화면 ID와 ServiceId를 기준으로 프로그램을 찾는다.
- Handler에서 Mapper까지 정방향으로 추적한다.
- SQL·테이블·오류코드에서 화면까지 역방향으로 추적한다.
- Spring Bean과 Runtime 구현체를 확인한다.
- Transaction·Timeout·인증·권한 적용위치를 찾는다.
- 업무 간 연계와 직접 DB 접근을 구분한다.
- 설정값과 환경별 동작 차이를 분석한다.
- Source·Mapper·DB·테스트·OM의 연결관계를 작성한다.
- 변경 대상별 영향범위를 산정한다.
- 변경 위험을 등급화한다.
- 미사용 코드와 중복 구현을 식별한다.
- 정적분석과 동적분석을 결합한다.
- AI 코딩도구를 이용하되 분석결과를 검증한다.
- 분석보고서와 영향도 매트릭스를 작성한다.
...
Source를 그대로 복사하기보다 업무 동작으로 요약합니다.
...
| --- | --- | --- | --- |
| CT-CNT-SEL-001 | CT_CONTACT_MASTER | CUSTOMER_NO | 조건 |
| CT-CNT-SEL-001 | CT_CONTACT_MASTER | CONTACT_TITLE | 출력 |
| CT-CNT-SEL-001 | CT_CONTACT_MASTER | OPEN_YN | 권한 |
...
| Table | CT_CONTACT_MASTER | 상담 Master |
...
| 변경대상 | 직접 영향 | 간접 영향 |
| --- | --- | --- |
| CONTACT_TITLE 길이 | Mapper·DTO | 화면·파일·연계 |
| CT_CONTACT_MASTER | SQL 7개 | ServiceId 5개 |
...
| 7 | Table | SV_CUSTOMER_SUMMARY | Data |
...
공통통제
JWT·CT_CONTACT_UPDATE·5초 Timeout·Version
