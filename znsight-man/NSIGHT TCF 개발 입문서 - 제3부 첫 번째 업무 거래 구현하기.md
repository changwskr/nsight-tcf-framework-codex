...
- 업무 WAR와 개발 위치를 찾는다.
- ServiceId에 맞는 패키지를 생성한다.
- 요청·응답 DTO를 작성한다.
- 입력값 형식 검증과 업무 검증을 구분한다.
- 도메인 단위 Handler에 ServiceId를 등록한다.
- Facade에 트랜잭션 경계를 설정한다.
- Service에서 업무 처리 순서를 구현한다.
- Rule에서 업무 조건을 검증한다.
- DAO와 Mapper를 통해 SQL을 실행한다.
- 단위·통합·거래 테스트를 작성한다.
- OM 기준정보를 등록한다.
- TraceId로 전체 실행 로그를 확인한다.
...
예를 들어 SV_CUSTOMER_VIEW 기능권한은 STF에서 확인하고, 특정 지점 고객만 조회하는 데이터 범위는 Service·Rule과 SQL에서 함께 적용할 수 있습니다.
