...
resolve_gradle() {
  ...
  echo "[ic-build] gradle not found." >&2
  exit 1
}
...
Usage: build.sh [clean|run]
  build.sh        Build tcf-util/core/web + ic.war
  build.sh clean  clean + build
  build.sh run    bootRun (port 8082)
EOF
    exit 0
    ;;
...
war_file="${PROJECT_HOME}/${MODULE}/build/libs/ic.war"
echo
...
if [[ -f "${war_file}" ]]; then
  ...
  echo "  [MISSING] ic.war - not found in ${MODULE}/build/libs" >&2
  exit 1
fi
