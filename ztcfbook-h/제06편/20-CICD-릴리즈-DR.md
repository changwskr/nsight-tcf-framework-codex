...
| **기반 원본** | [ztcfbook/제06편/20-CICD-릴리즈-DR.md](../ztcfbook/제06편/20-CICD-릴리즈-DR.md) |
...
tcf-cicd manifest.yaml·cicd-build.sh·cicd-deploy.sh는 WAR 빌드·profile별 yml sync·Tomcat·Apache 배포 절차의 SoT입니다. MR→Build→Test→WAR→Deploy→Smoke 실패 시 Rollback WAR pin은 znsight-man 67~68·부록 J와 연결됩니다.
...
```yaml
# NSIGHT TCF — 환경 설정 manifest (tcf-cicd SoT)
# sync: tcf-cicd/scripts/sync-to-framework.ps1 | pull-from-framework.ps1
...
# Spring Boot 모듈 (application-{profile}.yml)
modules:
  platform:
    - tcf-web
    - tcf-cache
...
# ztomcat / 인프라 (profile 디렉터리 기준 상대 경로)
ztomcat:
  local:
    - ztomcat/setenv.local.sh
    - ztomcat/setenv.local.bat
  dev:
    - ztomcat/conf/setenv.sh
```

원본: [`tcf-cicd/manifest.yaml`](../tcf-cicd/manifest.yaml)

---

## Master Deep Dive — CI/CD · 릴리즈 · DR
...
- [docs/architecture/49-release-strategy.md](../docs/architecture/49-release-strategy.md)
- [docs/architecture/45-disaster-recovery.md](../docs/architecture/45-disaster-recovery.md)
- [znsight-man/65-CICD-파이프라인-기준.md](../znsight-man/65-CICD-파이프라인-기준.md)
...
| --- | --- |
| ztcfbook-h | 본 파일 |
| ztcfbook | `../ztcfbook/제06편/20-CICD-릴리즈-DR.md` |
