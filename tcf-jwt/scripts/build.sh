...
resolve_gradle() {
  ...
  echo "[jwt-build] gradle not found." >&2
  exit 1
}
...
Usage: build.sh [clean|run]
  build.sh        Build tcf-util/core/web + jwt.war
  build.sh clean  clean + build
  build.sh run    bootRun (port 8100)
EOF
    exit 0
    ;;
...
war_file="${PROJECT_HOME}/${MODULE}/build/libs/jwt.war"
echo
...
if [[ -f "${war_file}" ]]; then
  ...
  echo "  [MISSING] jwt.war - not found in ${MODULE}/build/libs" >&2
  exit 1
fi
