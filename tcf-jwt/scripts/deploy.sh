...
resolve_gradle() {
  ...
  echo "[jwt-deploy] gradle not found." >&2
  exit 1
}
...
if [[ "${1:-}" == "help" || "${1:-}" == "-h" || "${1:-}" == "--help" ]]; then
  cat <<EOF
Usage: deploy.sh
  Build jwt.war and deploy to Tomcat webapps (/jwt)
...
[[ -d "${WEBAPPS}" ]] || { echo "[jwt-deploy] webapps not found: ${WEBAPPS}" >&2; exit 1; }
...
[[ -f "${src}" ]] || { echo "[jwt-deploy] WAR not found: ${src}" >&2; exit 1; }
...
if [[ -f "${WEBAPPS}/${DEST_WAR}" ]]; then
  ...
  echo "[jwt-deploy] Verification failed: ${DEST_WAR} not found in ${WEBAPPS}" >&2
  exit 1
fi
