# PDMP Framework (`pdmp-fw`)

`pdmp-service`에서 공통/프레임워크 계층을 분리한 **라이브러리 프로젝트**입니다.
...
업무 모듈(`nhnis.mp.*`)과 Boot 애플리케이션은 **포함하지 않습니다.**
...
산출물: `build/libs/pdmp-fw-0.0.1-SNAPSHOT.jar`

## 소비 측 연결 예시 (`pdmp-service`)

```gradle
dependencies {
    implementation project(':pdmp-fw')
    // 또는
    // implementation files('../pdmp-fw/build/libs/pdmp-fw-0.0.1-SNAPSHOT.jar')
}
```

루트 `settings.gradle`에 `include 'pdmp-fw'`를 추가하거나, `pdmp-service`처럼 독립 Gradle 루트로 사용합니다.
...
`pdmp-service`의 `nhnis.fw`, `com.ims.superspring` 및 `exceptionCode.yml`을 기준으로 분리했습니다.
