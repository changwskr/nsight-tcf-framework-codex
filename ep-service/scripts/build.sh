...
resolve_gradle() {
  ...
  echo "[ep-build] gradle not found." >&2
  exit 1
}
...
Usage: build.sh [clean|run]
  build.sh        Build tcf-util/core/web + ep.war
  build.sh clean  clean + build
  build.sh run    bootRun (port 8090)
EOF
    exit 0
    ;;
...
war_file="${PROJECT_HOME}/${MODULE}/build/libs/ep.war"
echo
...
if [[ -f "${war_file}" ]]; then
  ...
  echo "  [MISSING] ep.war - not found in ${MODULE}/build/libs" >&2
  exit 1
fi
