...
**Timeout 계층** — DB → Pool → Tx → Proxy → Web → L4.
...
## 16. 모니터링 / 운영 임계치 설정 가이드타임아웃 값은 설정만으로 끝나지 않는다. 실제 운영에서는 Timeout 발생률, p95/p99 응답시간, DB Pool Wait, SQL Timeout, Circuit Breaker Open, Kafka Lag, Batch Duration을 지속적으로 감시해야 한다.

설정 항목권장값실제 설정 위치설정 예시설명p95 응답시간3초 이하APM/모니터링p95 <= 3s사용자 SLAp99 응답시간5초 이하 목표APM/모니터링p99 <= 5s꼬리 지연 관리Timeout 오류율0.1~0.5% 이상 경고APM/로그timeout rate장애 조기감지Hikari Active80% 이상 경고Hikari Metricconnections.active/maxPool 고갈 감시Hikari Pending0 유지 목표Hikari Metricconnections.pendingConnection 대기 감시SQL p95500ms 이하 목표APM/DBsql p95RDW 조회 안정성GC Pause200ms 목표JVM Metricjvm.gc.pause응답시간 영향 감시Circuit Breaker Open즉시 알림Resilience4j Metricstate=open연계 장애 감지Kafka Lag업무별 임계치Kafka Monitorconsumer lag이벤트 지연 감시Batch Duration계획 대비 120% 경고Scheduler/ETLduration배치 지연 감시설정 예시management:  endpoints:    web:      exposure:        include: health,metrics,prometheus  metrics:    tags:      system: nsight-marketing      area: online-ap작업 가이드모든 Timeout 로그는 GUID, serviceId, interfaceId, sqlId, errorCode, elapsedTime, timeoutType을 포함해야 한다. 그래야 단말·Proxy·AP·DB·연계 중 어느 구간에서 끊겼는지 판단할 수 있다.
...
업무 유형사용자 응답 목표DB/SQL TimeoutTransaction TimeoutClient/Proxy Timeout처리 원칙일반 온라인 조회3초 이내2~3초4~5초10~15초동기 처리 가능Single View 조회3초 이내3초5초15초RDW 튜닝 필수CruzAPIM 동기 연계3~5초필요 시5초 내15초Circuit Breaker 필수다운로드60초 이내 또는 비동기조회부 분리업무별60초대량 다운로드 비동기 권장장시간 분석 조회동기 금지ADW 전용비동기결과조회RDW 수행 금지Kafka 이벤트비동기해당 없음해당 없음해당 없음Lag·DLQ 기준 관리ETL/배치배치 윈도우 기준업무별Job/Step 기준해당 없음분~시간 단위 관리18. 32 Core / 256GB 서버 적용 시 유의사항32 Core / 256GB 서버는 많은 Thread와 큰 Heap을 설정할 수 있지만, 온라인 업무 Timeout을 무조건 길게 가져가는 근거가 되지는 않는다. 서버가 커질수록 한 서버 장애 시 영향 범위도 커지고, DB Pool과 외부연계 동시 호출이 과도하게 증가할 수 있다.

항목32 Core 적용 기준주의사항Tomcat Thread1,500~1,800까지 검토DB Pool과 외부연계 Bulkhead 없이 Thread만 키우면 대기열만 증가Hikari Pool일반 120~150, SingleView 150~180AP 대수 × Pool 수로 DB Session 총량 검증 필요Heap64~96GB부터 검증, 대형 Heap은 단계적 검토Heap을 크게 잡으면 GC Pause와 분석 난이도 증가Transaction Timeout4~5초 원칙 유지Core가 커져도 온라인 동기거래 Timeout을 늘리지 않음SQL Timeout2~3초 원칙 유지RDW 보호가 우선외부연계 TimeoutConnect 3초, Read 5초 원칙 유지외부 시스템 지연이 AP 전체로 번지지 않게 격리운영 검증p95, p99, CPU, GC, Pool Wait, SQL Time 동시 검증단순 TPS 성공만으로 승인하지 않음
...
본 절(**Timeout 계층 구조**)은 Timeout 계층 영역에서 **계층 Timeout** 관점의 NSIGHT 1차 표준을 정의합니다. 피크 **720 TPS**·p95 **3초 이하**·8 vCPU / 32GB 기준으로 권고값을 제시하며, 최종값은 선도개발·성능시험(360/720/1080) 실측으로 확정합니다.
...
관련 원문: `znsight-capacity-word` · `환경셋팅(최종).docx` · 설정 위치: application.yml·Apache·L4

### 16.2 DB Query Timeout

본 절(**DB Query Timeout**)은 Timeout 계층 영역에서 **계층 Timeout** 관점의 NSIGHT 1차 표준을 정의합니다. 피크 **720 TPS**·p95 **3초 이하**·8 vCPU / 32GB 기준으로 권고값을 제시하며, 최종값은 선도개발·성능시험(360/720/1080) 실측으로 확정합니다.
...
### 16.3 Connection 획득 Timeout

본 절(**Connection 획득 Timeout**)은 Timeout 계층 영역에서 **계층 Timeout** 관점의 NSIGHT 1차 표준을 정의합니다. 피크 **720 TPS**·p95 **3초 이하**·8 vCPU / 32GB 기준으로 권고값을 제시하며, 최종값은 선도개발·성능시험(360/720/1080) 실측으로 확정합니다.
...
### 16.4 Spring Transaction Timeout

본 절(**Spring Transaction Timeout**)은 Timeout 계층 영역에서 **계층 Timeout** 관점의 NSIGHT 1차 표준을 정의합니다. 피크 **720 TPS**·p95 **3초 이하**·8 vCPU / 32GB 기준으로 권고값을 제시하며, 최종값은 선도개발·성능시험(360/720/1080) 실측으로 확정합니다.
...
### 16.5 TCF ServiceId Timeout

본 절(**TCF ServiceId Timeout**)은 Timeout 계층 영역에서 **계층 Timeout** 관점의 NSIGHT 1차 표준을 정의합니다. 피크 **720 TPS**·p95 **3초 이하**·8 vCPU / 32GB 기준으로 권고값을 제시하며, 최종값은 선도개발·성능시험(360/720/1080) 실측으로 확정합니다.
...
TCF Slow Transaction p95>3s → APM 알림.
...
### 16.6 외부 API Connect Timeout

본 절(**외부 API Connect Timeout**)은 Timeout 계층 영역에서 **계층 Timeout** 관점의 NSIGHT 1차 표준을 정의합니다. 피크 **720 TPS**·p95 **3초 이하**·8 vCPU / 32GB 기준으로 권고값을 제시하며, 최종값은 선도개발·성능시험(360/720/1080) 실측으로 확정합니다.
...
### 16.7 외부 API Read Timeout

본 절(**외부 API Read Timeout**)은 Timeout 계층 영역에서 **계층 Timeout** 관점의 NSIGHT 1차 표준을 정의합니다. 피크 **720 TPS**·p95 **3초 이하**·8 vCPU / 32GB 기준으로 권고값을 제시하며, 최종값은 선도개발·성능시험(360/720/1080) 실측으로 확정합니다.
...
### 16.8 Gateway Timeout

본 절(**Gateway Timeout**)은 Timeout 계층 영역에서 **계층 Timeout** 관점의 NSIGHT 1차 표준을 정의합니다. 피크 **720 TPS**·p95 **3초 이하**·8 vCPU / 32GB 기준으로 권고값을 제시하며, 최종값은 선도개발·성능시험(360/720/1080) 실측으로 확정합니다.
...
### 16.9 WEB Proxy Timeout

본 절(**WEB Proxy Timeout**)은 Timeout 계층 영역에서 **계층 Timeout** 관점의 NSIGHT 1차 표준을 정의합니다. 피크 **720 TPS**·p95 **3초 이하**·8 vCPU / 32GB 기준으로 권고값을 제시하며, 최종값은 선도개발·성능시험(360/720/1080) 실측으로 확정합니다.
...
### 16.10 L4 Idle Timeout

본 절(**L4 Idle Timeout**)은 Timeout 계층 영역에서 **계층 Timeout** 관점의 NSIGHT 1차 표준을 정의합니다. 피크 **720 TPS**·p95 **3초 이하**·8 vCPU / 32GB 기준으로 권고값을 제시하며, 최종값은 선도개발·성능시험(360/720/1080) 실측으로 확정합니다.
...
본 절(**장시간 거래 예외기준**)은 Timeout 계층 영역에서 **계층 Timeout** 관점의 NSIGHT 1차 표준을 정의합니다. 피크 **720 TPS**·p95 **3초 이하**·8 vCPU / 32GB 기준으로 권고값을 제시하며, 최종값은 선도개발·성능시험(360/720/1080) 실측으로 확정합니다.
...
| 기준 VM | 8 vCPU / 32GB |
| TPS | 360/720/1080 |
| 설정 파일 | application.yml·Apache·L4 |
| 핵심 | DB3·Tx5·Proxy10 |
...
### 16.12 Timeout 설정 정합성 검사

본 절(**Timeout 설정 정합성 검사**)은 Timeout 계층 영역에서 **계층 Timeout** 관점의 NSIGHT 1차 표준을 정의합니다. 피크 **720 TPS**·p95 **3초 이하**·8 vCPU / 32GB 기준으로 권고값을 제시하며, 최종값은 선도개발·성능시험(360/720/1080) 실측으로 확정합니다.
