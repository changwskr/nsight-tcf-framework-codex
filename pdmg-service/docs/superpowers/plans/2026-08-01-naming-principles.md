...
**Goal:** Replace the damaged `docs/네이밍규칙.md` with a comprehensive UTF-8 `docs/네이밍원칙.md` covering Java and Pdmk architectural naming.

**Architecture:** Use one canonical document organized from general Java rules to Pdmk-specific compatibility rules. Separate mandatory rules, recommendations, prohibited forms, and legacy exceptions so new-code reviews have an unambiguous standard without forcing unrelated source renames.
...
- Produces: One review-ready naming standard for developers adding or reviewing `pdmk-service` code.
...
- [ ] **Step 4: Write Pdmk and TCF-specific rules**
...
Log files: project prefix plus purpose, such as pk_framework.log and pk_service.log
```

Explain that the proposed `pk_` log prefix is the naming principle, while any operational collector contract must be checked before changing deployed names.
