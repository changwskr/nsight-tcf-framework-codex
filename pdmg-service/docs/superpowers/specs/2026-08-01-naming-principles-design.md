...
Create `docs/네이밍원칙.md` as the single authoritative naming and architecture convention for `pdmk-service`, consolidating and replacing the encoding-damaged `docs/네이밍규칙.md`.
...
- Document Pdmk screen/program naming, including `mpcoa9999`, `nhnis.mp.co.a`, `DtoIn`, `DtoOut`, and `ListResponseDto` compatibility conventions.
...
- Modifying `pdmk-ui` or projects outside `pdmk-service`.
