...
Create `docs/CRUD서비스프롬프트가이드.md` as a reusable Korean guide for prompting an AI to design, implement, test, and verify CRUD features in `pdmk-service`.

## Audience and Scope

- Audience: Developers and analysts who know the business data but may not know every Spring/MyBatis implementation detail.
- Scope: `pdmk-service` backend only; `pdmk-ui` is excluded.
...
- It prohibits unrelated refactoring, secret exposure, guessing unknown schemas, and changes outside `pdmk-service`.
