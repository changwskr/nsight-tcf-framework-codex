...
**Goal:** Create a reusable Korean Markdown guide that helps users prompt an AI to implement production-quality CRUD features in `pdmp-service`.

**Architecture:** Organize the guide around a requirements intake form, a staged workflow, and copy-ready prompts. Encode current PDMP TCF, Spring transaction, MyBatis, error, testing, and verification conventions directly in the prompts so important requirements are not left to AI inference.
...
- Cover `pdmp-service` backend only; do not include `pdmp-ui` implementation prompts.
...
Each prompt must restrict changes to `pdmp-service`, preserve user changes, and stop for approval when business information is missing.
