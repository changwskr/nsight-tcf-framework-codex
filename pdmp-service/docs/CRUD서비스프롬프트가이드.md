# PDMP Service CRUD 프롬프트 가이드

이 문서는 AI에게 `pdmp-service`의 CRUD 기능을 정확하고 안전하게 요청하기 위한 실전 가이드다. 단순히 “CRUD를 만들어줘”라고 요청하지 않고, 업무 규칙·DB 계약·TCF 전문·트랜잭션·오류·테스트까지 명확하게 전달하는 것을 목표로 한다.
...
```text
작업 대상: pdmp-service만
작업 제외: pdmp-ui와 상위 저장소의 다른 프로젝트
기준 문서: docs/네이밍원칙.md
참조 구현: mpcoa9999 Controller/Service/DAO/DTO/MyBatis
```
...
Java 기본 패키지: [예: nhnis.mp.co.a]
API 기본 경로: [예: /api/mp/co/a/9999]
작업 범위: [전체 CRUD / 목록 / 상세 / 등록 / 수정 / 삭제]
참조할 기존 프로그램: [예: mpcoa9999]
```
...
| 동작 | serviceId | transactionCode | processingType | serviceName | timeout |
|---|---|---|---|---|---|
| 목록 | `[MP.Domain.list]` | `[MP-INQ-0001]` | `INQUIRY` | `[목록 조회]` | `[초 또는 미지정]` |
| 상세 | `[MP.Domain.detail]` | `[MP-INQ-0002]` | `INQUIRY` | `[상세 조회]` | `[초 또는 미지정]` |
| 등록 | `[MP.Domain.create]` | `[MP-REG-0001]` | `[확정 enum]` | `[등록]` | `[초 또는 미지정]` |
| 수정 | `[MP.Domain.update]` | `[MP-UPD-0001]` | `[확정 enum]` | `[수정]` | `[초 또는 미지정]` |
| 삭제 | `[MP.Domain.delete]` | `[MP-DEL-0001]` | `[확정 enum]` | `[삭제]` | `[초 또는 미지정]` |
...
`processingType`에 필요한 enum 값이 현재 코드에 없다면 AI가 임의로 만들지 말고 먼저 추가 여부를 질문하게 한다.
...
5. DTO와 검증 구현
6. DAO와 MyBatis 구현
7. Service 업무·트랜잭션 구현
8. Controller와 TCF 연결
9. 전체 테스트·참조·SQL 검증
10. 변경 파일과 검증 결과 보고
```
...
```text
pdmp-service에 CRUD 기능을 추가하려고 한다. 아직 코드를 수정하지 마라.

다음을 먼저 조사해 보고해라.
- docs/네이밍원칙.md
- mpcoa9999의 Controller, Service, DAO, DTO, MyBatis XML
- TCFAspect, STF, ETF, @TcfTransaction
- StandardRequestDto, StandardResponseDto, BizException
- RdwDataSourceConfig와 트랜잭션 구성
- exceptionCode.yml과 H2 schema/data 구조

작업 범위는 pdmp-service뿐이다. 현재 사용자 변경을 보존하고 관련 없는 파일은 건드리지 마라.
...
DB 컬럼, PK, 중복 기준, 삭제 방식, TCF 거래 코드, timeout, 오류 코드,
...
아직 구현하지 말고 다음을 설계해라.
- 요청·응답 DTO와 필드
- API 경로와 @TcfTransaction 메타데이터
- Controller → Service → DAO/MyBatis 흐름
- 읽기/쓰기 트랜잭션과 rollback 범위
- 목록 페이징과 정렬
- 필수값, 중복, 미존재, 동시 수정 오류
- Oracle/H2 SQL 차이
- 단위·통합 테스트 시나리오
...
- 등록·수정·삭제는 rollback 가능한 @Transactional을 사용한다.
- timeout은 [확정값/미지정]을 그대로 적용하고 임의 값을 만들지 않는다.
- 여러 쓰기 SQL은 하나의 트랜잭션으로 묶는다.
...
```text
Controller를 현재 TCF 계약에 맞게 구현해라.
- StandardRequestDto<T>로 요청을 받고 StandardResponseDto<T>를 반환한다.
- 각 메서드에 확정된 @TcfTransaction 메타데이터를 적용한다.
- Controller는 HTTP 변환과 Service 호출만 담당한다.
- 성공 header/result 조립과 실패 전문은 ETF 흐름을 사용한다.
- 인증·권한 요구를 기존 Security 구조와 맞춘다.

API 경로, 요청 JSON, 성공/실패 응답 예시를 함께 제시해라.
```
...
```text
pdmp-service에 다음 CRUD 서비스를 설계하고 구현해라.

[범위]
- 작업 프로젝트: pdmp-service만
- pdmp-ui와 다른 프로젝트는 수정 금지
- 사용자 기존 변경 보존
- 관련 없는 리팩터링 금지
- 기준: docs/네이밍원칙.md와 현재 mpcoa9999/TCF 구조
...
[TCF]
- 목록: [serviceId / transactionCode / processingType / serviceName]
- 상세: [serviceId / transactionCode / processingType / serviceName]
- 등록: [serviceId / transactionCode / processingType / serviceName]
- 수정: [serviceId / transactionCode / processingType / serviceName]
- 삭제: [serviceId / transactionCode / processingType / serviceName]

[트랜잭션]
- 조회: @Transactional(readOnly = true)
- 쓰기: rollback 가능한 @Transactional
- timeout: [초 또는 미지정]
- timeout 값을 추측하지 말 것
...
[구현 계약]
- 요청은 StandardRequestDto<T>, 응답은 StandardResponseDto<T>
- Controller는 HTTP 변환과 Service 호출만 담당
- Service는 업무 규칙과 트랜잭션 담당
- DAO/MyBatis는 영속성 담당
...
```text
pdmp-service의 [프로그램ID] 목록 조회를 구현해라.
검색조건은 [조건], 정렬은 [순서], 페이징은 pageNo [1], pageSize [20], 최대 [100]이다.
조회는 @Transactional(readOnly = true)를 사용하고 0건은 빈 records와 totalCount=0으로 반환한다.
count SQL과 목록 SQL의 조건을 일치시키고 Oracle/H2에서 검증해라.
StandardRequestDto/StandardResponseDto와 @TcfTransaction [메타데이터]를 적용하고 테스트 결과를 보고해라.
```

### 6.2 상세 조회

```text
pdmp-service의 [프로그램ID] 상세 조회를 구현해라.
조회키는 [PK 전체]이고 누락은 [오류 코드], 결과 없음은 [오류 코드] BizException으로 처리한다.
@Transactional(readOnly = true), 표준 전문, @TcfTransaction [메타데이터]를 적용한다.
...
```text
pdmp-service의 [프로그램ID] 등록을 구현해라.
필수값은 [필드], 중복 기준은 [업무키], 중복 오류는 [코드]다.
쓰기 @Transactional(timeout=[확정값])을 적용하고 후속 SQL 실패 시 전체 rollback되게 한다.
DAO/MyBatis insert, exceptionCode.yml, @TcfTransaction [메타데이터]와 중복/rollback 테스트를 포함해라.
```
...
```text
pdmp-service의 [프로그램ID] 수정을 구현해라.
PK는 [필드], 수정 가능 필드는 [필드], 동시성 기준은 [버전/수정시각/미사용]이다.
update row count가 0이면 [미존재/충돌 오류 코드]를 발생시킨다.
PK 없는 UPDATE를 금지하고 쓰기 트랜잭션, @TcfTransaction, 정상/0건/rollback 테스트를 검증해라.
```

### 6.5 삭제

```text
pdmp-service의 [프로그램ID] 삭제를 구현해라.
삭제 방식은 [물리/논리], PK는 [필드], 사전 삭제 조건은 [내용]이다.
대상 없음은 [멱등 성공/오류 코드] 정책을 따른다.
PK 없는 DELETE/UPDATE를 금지하고 쓰기 트랜잭션, @TcfTransaction, 삭제불가/0건/rollback 테스트를 포함해라.
```

### 6.6 기존 CRUD 디버깅

```text
pdmp-service의 기존 [프로그램ID] CRUD 문제를 진단해라.
증상: [요청, 기대결과, 실제결과, 로그, 재현절차]

아직 수정하지 말고 다음 순서로 조사해라.
1. Controller/TCF 요청 매핑
2. Service 검증과 트랜잭션
3. DAO 메서드와 MyBatis statement/바인딩
4. Oracle/H2 차이
5. ETF/GlobalExceptionHandler 오류 변환
...
- PK 네 필드 모두 필수
- 결과 없음은 FW0003
- serviceId MP.SalesTip.detail
- transactionCode MP-INQ-0002

등록·수정·삭제:
- 처리유형 enum, 거래코드, 중복/충돌/삭제 오류 코드는 아직 미정
...
```text
pdmp-service의 [프로그램ID] CRUD를 만들어라.
테이블과 PK는 [정의], 목록 조건과 정렬은 [정의], 삭제는 [논리/물리]다.
읽기는 readOnly, 쓰기는 rollback 트랜잭션을 사용하고 timeout은 [값/미지정]이다.
TCF 메타데이터와 오류 코드는 [정의]를 사용한다.
...
```text
mpcoa9999에서는 패키지 구조, 표준 전문, TCF, 페이징, MyBatis 연결 방식만 참고해라.
테이블·컬럼·PK·거래코드·오류코드는 아래 신규 요구사항을 사용하고 기존 값을 복사하지 마라.
[신규 요구사항]
```
...
```text
[등록/수정/삭제] Service 메서드의 Spring DB 트랜잭션 timeout을 4초로 지정해라.
timeout 발생 시 쓰기 전체가 rollback되는 테스트를 추가해라.
slow-transaction-ms는 성능 경고 기준이므로 변경하지 마라.
```
...
- [ ] `pdmp-service`만 변경했는가?
- [ ] 사용자 기존 변경과 관련 없는 파일을 보존했는가?
- [ ] Controller → Service → DAO/MyBatis 의존 방향을 지켰는가?
- [ ] `docs/네이밍원칙.md`를 따르는가?

### API·DTO·TCF

- [ ] 요청/응답이 `StandardRequestDto<T>`와 `StandardResponseDto<T>`인가?
- [ ] DTO 필드와 JSON 필드가 camelCase로 맞는가?
- [ ] 각 API의 `@TcfTransaction` 값이 확정 요구사항과 맞는가?
- [ ] Controller에 업무 규칙이나 SQL이 들어가지 않았는가?

### 트랜잭션

- [ ] 조회가 `@Transactional(readOnly = true)`인가?
- [ ] 등록·수정·삭제가 쓰기 트랜잭션인가?
...
```text
첨부한 [CRUD 요구사항 문서/테이블 정의서]를 기준으로 pdmp-service CRUD를 구현해라.

반드시 다음을 지켜라.
- 작업 범위는 pdmp-service만
- docs/네이밍원칙.md와 현재 mpcoa9999/TCF 구조 준수
- 모르는 스키마·업무·거래코드·timeout은 추측하지 말고 질문
- StandardRequestDto/StandardResponseDto와 @TcfTransaction 적용
- 조회 readOnly, 쓰기 rollback 트랜잭션 적용
- BizException, exceptionCode.yml, ETF 오류 흐름 유지
