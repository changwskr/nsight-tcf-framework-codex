# Pdmk Service CRUD 프롬프트 가이드

이 문서는 AI에게 `pdmk-service`의 CRUD 기능을 정확하고 안전하게 요청하기 위한 실전 가이드다. 단순히 “CRUD를 만들어줘”라고 요청하지 않고, 업무 규칙·DB 계약·TCF 전문·트랜잭션·오류·테스트까지 명확하게 전달하는 것을 목표로 한다.
...
```text
작업 대상: pdmk-service만
작업 제외: pdmk-ui와 상위 저장소의 다른 프로젝트
기준 문서: docs/네이밍원칙.md
...
```text
pdmk-service에 CRUD 기능을 추가하려고 한다. 아직 코드를 수정하지 마라.
...
작업 범위는 pdmk-service뿐이다. 현재 사용자 변경을 보존하고 관련 없는 파일은 건드리지 마라.
...
```text
pdmk-service에 다음 CRUD 서비스를 설계하고 구현해라.

[범위]
- 작업 프로젝트: pdmk-service만
- pdmk-ui와 다른 프로젝트는 수정 금지
- 사용자 기존 변경 보존
- 관련 없는 리팩터링 금지
...
```text
pdmk-service의 [프로그램ID] 목록 조회를 구현해라.
...
```text
pdmk-service의 [프로그램ID] 상세 조회를 구현해라.
...
```text
pdmk-service의 기존 [프로그램ID] CRUD 문제를 진단해라.
...
```text
pdmk-service의 [프로그램ID] CRUD를 만들어라.
...
```text
첨부한 [CRUD 요구사항 문서/테이블 정의서]를 기준으로 pdmk-service CRUD를 구현해라.

반드시 다음을 지켜라.
- 작업 범위는 pdmk-service만
